...
const fail = (message) => {
  ...
  process.stderr.write(`ERROR [research-registry-boundary] ${message}\n`);
};
...
for (const artifact of [
  ...
  if (!fs.existsSync(artifact))
    fail(`missing research-registry artifact ${artifact}`);
}

if (failures.length > 0) process.exit(1);
process.stdout.write(
  `Research-registry strategy capability and Python runtime boundary passed (${files.length} Go files)\n`,
);
