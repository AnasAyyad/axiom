...
const fail = (message) => {
  ...
  process.stderr.write(`ERROR [b8-console-boundary] ${message}\n`);
};
...
const backendFiles = [
  "internal/api/console/b8_read.go",
  "internal/api/console/b8_commands.go",
  "internal/storage/postgres/b8_exchange_strategy.go",
  "internal/storage/postgres/b8_opportunities.go",
  "internal/storage/postgres/b8_inventory_rebalancing.go",
  "internal/storage/postgres/b8_research_commands.go",
  "internal/storage/postgres/b8_research_read.go",
];
...
const ui = [
  ...
  read("web/src/api/b8Validation.ts"),
].join("\n");
for (const required of [
...
for (const artifact of [
  "internal/storage/postgres/queries/b8_console.sql",
  "internal/storage/postgres/b8_console_integration_test.go",
  "web/src/app/MultiExchangePages.test.tsx",
  "web/tests/e2e/a11-workflow.spec.ts",
  "docs/adr/0019-b8-generic-multi-exchange-console.md",
  "docs/api/b8-console.md",
  "docs/releases/evidence/b8-local-validation.md",
]) {
  if (!fs.existsSync(artifact)) fail(`B8 artifact missing ${artifact}`);
}

if (failures.length > 0) process.exit(1);
process.stdout.write(
  `B8 generic console, simulation-only commands, advisory lock, and no-production-execution boundary passed (${backendFiles.length} backend files)\n`,
);
