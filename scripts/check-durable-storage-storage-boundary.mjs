...
function fail(message) {
  ...
  process.stderr.write(`ERROR [durable-storage-boundary] ${message}\n`);
}
...
process.stdout.write(
  `Durable storage boundary passed (${migrationNames.length} migrations, ${requiredTables.length} required tables, Parquet/Zstd codec, integer capacity policy, authenticated 14-generation backup retention)\n`,
);
