...
requireTokens("cmd/d6-certify/main.go", [
  "AXIOM_D6_FINAL_CERTIFICATION_ENABLED",
...
const makefile = requireTokens("Makefile", [
  "d6-certification-model-qualify:",
  "d6-traceability-qualify:",
  "d6-security-qualify:",
  "d6-final-certification:",
  "v1d-d6-local-qualify:",
]);
const aggregate =
  makefile.match(/v1d-d6-local-qualify:[\s\S]*?(?=\n\S|$)/)?.[0] ?? "";
if (aggregate.split("##")[0].includes("soak")) {
  throw new Error("D6 local aggregate must not invoke a soak-named target");
}
...
console.log(
  "V1D D6 certification, evidence, and documentation boundary passed",
);
