...
// Command generate_v1c_canary_config emits one complete, immutable V1C graph
...
func main() {
	...
	configuration, err := config.DefaultV1CConfiguration(mode)
	if err != nil {
		fail()
	}
...
func fail() {
	fmt.Fprintln(os.Stderr, "v1c_canary_configuration_generation_failed")
	os.Exit(1)
}
