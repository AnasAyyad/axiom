...
const fail = (message) => {
  ...
  process.stderr.write(`ERROR [mean-reversion-boundary] ${message}\n`);
};
...
if (
  ...
  configuration.mean_reversion?.strategy_version !== "mean-reversion@1.0.0" ||
  ...
  fail("reviewed mean-reversion configuration is incomplete or unsafe");
}
...
for (const artifact of [
  ...
  "internal/storage/postgres/queries/mean_reversion_mean_reversion.sql",
  "internal/storage/postgres/mean_reversion_repository.go",
  "docs/strategies/mean-reversion.md",
]) {
  if (!fs.existsSync(artifact))
    fail(`missing mean-reversion artifact ${artifact}`);
}

if (failures.length > 0) process.exit(1);
process.stdout.write(
  `Mean-reversion strategy, persistence, configuration, and Python runtime boundary passed (${files.length} Go files, ${parameters.length} parameters)\n`,
);
