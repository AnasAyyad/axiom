...
NM="${NM:-/usr/bin/nm}"
RG="${RG:-rg}"
TEMP_DIR="$(mktemp -d "${TMPDIR:-/tmp}/axiom-b8-binary.XXXXXX")"
cleanup() {
  rm -rf -- "${TEMP_DIR}"
}
...
if [[ ! -f "${BINARY}" ]]; then
  printf 'ERROR [b8-binary-boundary] compiled platform is missing\n' >&2
  exit 1
fi

"${NM}" --defined-only "${BINARY}" > "${TEMP_DIR}/symbols"
if ! "${RG}" -q 'ScheduleReplayFault|ExportReport' "${TEMP_DIR}/symbols"; then
  printf 'ERROR [b8-binary-boundary] B8 command boundary is not linked\n' >&2
  exit 1
fi
...
if "${RG}" -a -i -q --regexp "${forbidden_pattern}" -- "${BINARY}" ||
  ...
  printf 'ERROR [b8-binary-boundary] prohibited production execution capability linked\n' >&2
  exit 1
fi

printf 'B8 platform simulation-only command binary boundary passed\n'
