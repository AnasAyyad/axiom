...
requireTokens("crypto_bot_v1_codex_spec.md", [
  ...
  "B2 market-data and C6 sandbox verdicts remain independent",
]);
...
if (compose.includes("backup_data:")) {
  throw new Error("Compose-managed backup_data volume survived D5");
}
...
requireTokens("internal/bootstrap/recorder_pressure.go", [
  ...
  "NewD5StoragePressureStore",
]);
requireTokens("internal/storage/postgres/d5_pressure_gates.go", [
  "source_instance<>'migration-bootstrap'",
  "level='NORMAL'",
  "interval '2 minutes'",
]);
requireTokens("internal/storage/postgres/a11_console_shadow.go", [
  "breaker_kind='disk_failure'",
  "level='NORMAL'",
  "source_instance<>'migration-bootstrap'",
]);
requireTokens("internal/bootstrap/a11_worker_pipeline.go", [
  "orderedOfflineWorkers{lifecycleWorker, reportWorker, worker}",
]);
for (const path of [
  "internal/storage/postgres/d1_console_access.go",
  "internal/storage/postgres/d1_console_actions.go",
  "internal/storage/postgres/d4_report_worker.go",
]) {
  requireTokens(path, ["d5HeavyWorkAllowed"]);
}
...
requireTokens("internal/qualification/d5/runner.go", [
  "validatePreflight",
...
requireTokens("internal/qualification/d5/files.go", [
  "events[index].RunID != runID",
  "d5_fault_evidence_run_mismatch",
]);
requireTokens("cmd/d5-readiness/main.go", [
  "AXIOM_D5_READINESS_ENABLED",
  "formal build identity mismatch",
  "AXIOM_D5_TEST_MANIFEST_FILE",
]);

const schedule = read("deploy/config/d5-fault-schedule-v1.json");
const scheduleHash = createHash("sha256").update(schedule).digest("hex");
const manifest = JSON.parse(read("deploy/config/d5-test-manifest-v1.json"));
...
if (
  ...
  !exactSet(manifest.independent_verdicts_not_replaced, [
    "B2 market-data qualification",
    "C6 sandbox order and reconciliation qualification",
  ])
) {
  throw new Error("D5 manifest duration or fault schedule identity drifted");
}

for (const path of [
  "deploy/config/d5-run.example.json",
  "deploy/config/d5-preflight.example.json",
  "deploy/config/d5-sample.example.json",
  "deploy/config/d5-fault-evidence.example.json",
]) {
...
for (const path of [
  "internal/qualification/d5/model.go",
  "internal/qualification/d5/runner.go",
  "cmd/d5-readiness/main.go",
]) {
  const source = read(path);
  for (const forbidden of [
    ...
    if (source.includes(forbidden)) {
      throw new Error(
        `${path} crosses the D5 observation-only boundary with ${forbidden}`,
      );
    }
  }
}
...
console.log("V1D D5 hardening, lifecycle, and readiness boundary passed");
