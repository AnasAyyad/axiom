...
const traceabilityRows = rows.filter((row) =>
  row.fields[0].startsWith("AX-V1A-A00-"),
);
if (traceabilityRows.length !== 37)
  failures.push("expected 37 A0 rows, found " + traceabilityRows.length);
...
if (plan !== undefined) {
  const planOwnerConsole = plan.match(
  ...
  if (!planOwnerConsole) {
    ...
    const planEndpoints = [
      ...planOwnerConsole[1].matchAll(/^\s*-\s+(GET|POST)\s+(\/\S+)\s*$/gm),
