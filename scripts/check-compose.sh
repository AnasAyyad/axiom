...
expected="$(printf '%s\n' api backup backtest-worker binance-sandbox-canary binance-sandbox-engine binance-testnet-egress bybit-demo-egress bybit-public-egress bybit-sandbox-canary bybit-sandbox-engine caddy engine-shadow grafana migrate operational-readiness-observer postgres prometheus recorder restore | sort)"
