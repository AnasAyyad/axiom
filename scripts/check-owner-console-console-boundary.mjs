...
for (const [method, path] of required) {
  if (openapi.paths?.[path]?.[method] === undefined)
    throw new Error(
      `missing owner-console contract ${method.toUpperCase()} ${path}`,
    );
}
...
for (const label of [
  ...
  if (!shell.includes(label))
    throw new Error(`persistent owner-console safety label missing: ${label}`);
}
for (const token of ['?? "shadow"', "mode.toUpperCase()", "· VIRTUAL"]) {
  if (!shell.includes(token))
    throw new Error(
      `persistent owner-console shadow/virtual label omits: ${token}`,
    );
}
...
for (const name of secretNames) {
  ...
  if (JSON.stringify(consumers) !== JSON.stringify(allowed))
    throw new Error(
      `owner-console secret ${name} consumers = ${consumers.join(",")}`,
    );
}
...
console.log(
  `Owner-console contract and console boundary passed (${required.length} required operations)`,
);
