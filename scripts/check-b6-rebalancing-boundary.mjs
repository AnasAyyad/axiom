...
const fail = (message) => {
  ...
  process.stderr.write(`ERROR [b6-rebalancing-boundary] ${message}\n`);
};
...
if (
  ...
  fail("reviewed B6 configuration is incomplete or unsafe");
}
...
for (const artifact of [
  "internal/storage/postgres/queries/b6_rebalancing.sql",
  "internal/storage/postgres/b6_repository.go",
  "internal/storage/postgres/b6_rebalancing_integration_test.go",
  "docs/strategies/advisory-rebalancing.md",
  "docs/releases/evidence/b6-local-validation.md",
]) {
  if (!fs.existsSync(artifact)) fail(`missing B6 artifact ${artifact}`);
}

if (failures.length > 0) process.exit(1);
process.stdout.write(
  `B6 deterministic advisory optimizer and no-execution boundary passed (${files.length} Go files, ${parameters.length} parameters)\n`,
);
