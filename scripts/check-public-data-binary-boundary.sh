...
GO="${GO:-go}"
RG="${RG:-rg}"
TEMP_DIR="$(mktemp -d "${TMPDIR:-/tmp}/axiom-public-data-binary.XXXXXX")"
cleanup() {
  rm -rf -- "${TEMP_DIR}"
}
...
for required_dependency in \
  ...
  if [[ "${dependencies}" != *"${required_dependency}"* ]]; then
    printf 'ERROR [public-data-binary-boundary] public recorder dependency absent: %s\n' \
      "${required_dependency}" >&2
    exit 1
  fi
done
if [[ "${dependencies}" == *'axiom/internal/exchanges/emulator'* ]]; then
  printf 'ERROR [public-data-binary-boundary] emulator linked into platform\n' >&2
  exit 1
fi
...
for required_literal in \
  ...
  if ! "${RG}" -a -F -q -- "${required_literal}" "${TEMP_DIR}/platform"; then
    printf 'ERROR [public-data-binary-boundary] compiled public policy absent: %s\n' \
      "${required_literal}" >&2
    exit 1
  fi
done
for forbidden_literal in \
  ...
  if "${RG}" -a -F -q -- "${forbidden_literal}" "${TEMP_DIR}/platform"; then
    printf 'ERROR [public-data-binary-boundary] forbidden Binance origin linked: %s\n' \
      "${forbidden_literal}" >&2
    exit 1
  fi
done

printf 'Public-data platform production-public dependency and binary boundary passed\n'
