...
const fail = (message) => {
  ...
  process.stderr.write(
    `ERROR [cross-exchange-arbitrage-boundary] ${message}\n`,
  );
};
...
if (
  ...
  crossExchange.strategy_version !== "cross-exchange-arbitrage@1.0.0" ||
  ...
  fail("reviewed cross-exchange configuration is incomplete or unsafe");
}
...
for (const invariant of [
  ...
  "claim_b5_resources", // semantic-naming: historical-reference
  "settle_b5_claim_group", // semantic-naming: historical-reference
  "close_b5_claim_group", // semantic-naming: historical-reference
...
for (const artifact of [
  "internal/storage/postgres/queries/cross_exchange_arbitrage_cross_exchange.sql",
  "internal/storage/postgres/cross_exchange_arbitrage_repository.go",
  "internal/storage/postgres/cross_exchange_arbitrage_cross_exchange_integration_test.go",
  "docs/strategies/cross-exchange-arbitrage.md",
  "docs/releases/evidence/b5-local-validation.md",
]) {
  if (!fs.existsSync(artifact)) {
    fail(`missing cross-exchange artifact ${artifact}`);
  }
}

if (failures.length > 0) process.exit(1);
process.stdout.write(
  `Cross-exchange coherent views, closed-cycle economics, claims, persistence, and runtime boundary passed (${files.length} Go files, ${parameters.length} parameters)\n`,
);
