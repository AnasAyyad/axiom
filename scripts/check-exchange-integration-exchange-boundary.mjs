...
function fail(message) {
  ...
  process.stderr.write(`ERROR [exchange-integration-boundary] ${message}\n`);
}
...
for (const feature of [
  ...
  if (!capabilitySource.includes(`unsupported(exchangecontracts.${feature})`))
    fail(`Binance public-data boundary does not explicitly reject ${feature}`);
}
...
const reviewedSandboxRuntimeAuthenticatedFiles = new Set([
...
for (const file of goFiles("internal/exchanges")) {
  if (
    ...
    reviewedSandboxRuntimeAuthenticatedFiles.has(file)
  )
    continue;
...
process.stdout.write(
  `Exchange-integration boundary validated (${expectedFixtures.length} fixtures)\n`,
);
