...
func TestPortfolioRiskRealAllocatorAndRiskComposeThroughSharedStrategyExecutionPipeline(t *testing.T) {
	...
	if err != nil || result.Ordinal != 1 || len(result.Decision) == 0 || processor.Metrics().TotalNetReturn != "unavailable" {
		t.Fatalf("portfolio and risk shared pipeline = %#v %v", result, err)
	}
	...
	if balance.Available.String() != "399.9" || balance.Reserved.String() != "100.1" {
		t.Fatalf("portfolio and risk pipeline ownership = %#v", balance)
	}
}

func TestPortfolioRiskPipelineReleasesExclusiveClaimsAfterRiskRejection(t *testing.T) {
	owned := pipelinePortfolio(t)
	liquidity := portfolio.NewLiquidityPool()
	quantity, _ := domain.ParseQuantity("1")
	if err := liquidity.Open("combined-book", quantity); err != nil {
...
func pipelineCandidate(t *testing.T) backtest.Candidate {
	...
	payload, err := json.Marshal(portfolio.Candidate{ID: "pipeline-decision", Strategy: portfolio.TrendStrategy,
