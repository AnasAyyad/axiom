...
func (value decimal) squareRoot() (decimal, error) {
	if value.value.Sign() < 0 {
		return decimal{}, strategyError(ReasonInvalidSizing)
	}
	var result apd.Decimal
	if _, err := calculationContext.Sqrt(&result, &value.value); err != nil {
		return decimal{}, strategyError(ReasonInvalidSizing)
	}
	return quantize(decimal{value: result}, apd.RoundHalfEven)
}
...
func absolute(value decimal) decimal {
	if value.value.Sign() >= 0 {
		return value
	}
	value.value.Neg(&value.value)
	return value
}
