...
func NewRecordedSagaAllocator(fence runtimecore.FencingToken) (*RecordedSagaAllocator, error) {
	if fence == 0 {
		return nil, strategyError("recorded_saga_allocator_invalid")
	}
	return &RecordedSagaAllocator{fence: fence, active: make(map[string]*AtomicSagaAllocator)}, nil
}
...
func (allocator *RecordedSagaAllocator) AllocateSaga(ctx context.Context, candidate backtest.SagaCandidate) (backtest.SagaAllocation, error) {
	...
	claims, err := NewRecordedSagaClaimSet(value.Input)
	if err != nil {
		return backtest.SagaAllocation{}, err
	}
	delegate, err := NewAtomicSagaAllocator(claims, allocator.fence)
	if err != nil {
		return backtest.SagaAllocation{}, err
	}
