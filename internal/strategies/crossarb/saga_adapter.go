...
// SagaStrategyAdapter evaluates every exact Cross-Exchange candidate from one
// canonical Input. It performs no allocation, risk, or exchange dispatch.
...
// EvaluateSaga decodes only immutable replay input and returns the complete,
// two-direction candidate set to the shared allocation stage.
...
// AtomicSagaAllocator reserves every two-venue balance, fee, liquidity, and
// recovery resource for one deterministic highest-ranked candidate.
...
func NewAtomicSagaAllocator(
	...
	if claims == nil || fence == 0 {
		return nil, strategyError("saga_allocator_invalid")
	}
	return &AtomicSagaAllocator{claims: claims, fence: fence}, nil
}

// AllocateSaga ranks all accepted candidates by worst-case closed-cycle
// profit, expected closed-cycle profit, then immutable ID. It reserves the
// first complete candidate and never leaves partial claims on a conflict.
func (allocator *AtomicSagaAllocator) AllocateSaga(
	...
	for _, candidate := range rankedCandidates(candidates.Candidates) {
		...
		group, claimErr := ClaimCandidate(allocator.claims, candidate, reservation, allocator.fence,
			candidates.Input.LogicalTime)
		if claimErr != nil {
			continue
		}
...
// expired or malformed replay input fails before risk can approve it.
...
// SagaPlanner materializes the exact concurrent virtual execution saga only
...
func (*SagaPlanner) PlanSaga(_ context.Context, approved backtest.SagaApproval) (backtest.SagaPlan, error) {
	...
	saga, _, err := newConcurrentSaga(value.Allocation.Candidate)
	if err != nil {
...
// SagaSimulationInputProvider supplies recorded future books, directives,
// latency, and recovery policy associated with one canonical input. It must
// not substitute a newer live market or account view during replay.
type SagaSimulationInputProvider interface {
	SimulationInput(Input) (Timeline, LatencyDistribution, RecoveryPolicy, error)
}
...
func (broker *SagaSimulationBroker) SubmitSaga(
	...
	timeline, latency, policy, err := broker.inputs.SimulationInput(value.Approval.Allocation.Input)
	if err != nil {
		return backtest.SagaExecution{}, strategyError("saga_simulation_input_invalid")
	}
	result, err := Simulate(value.Approval.Allocation.Candidate, timeline, latency, policy)
