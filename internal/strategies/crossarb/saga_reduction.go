...
// SagaReductionProvider supplies the durable journal, independent economics,
// mode-specific reconciliation evidence, and atomic-claim completion needed
// for one Cross-Exchange reduction.
type SagaReductionProvider interface {
	Journal(Input) (*CrossExchangeJournal, error)
	Attribution(Input, Candidate, SimulationResult) (PortfolioAttribution, error)
	Reconciliation(Input, Candidate, SimulationResult) (SagaReconciliation, error)
	Release(context.Context, backtest.SagaAllocation) error
}
...
// RecordedSagaReductionProvider joins immutable recorded attribution and
// reconciliation evidence to the run-scoped durable journal, reconciler, and
// atomic allocator. It never reads a newer venue or portfolio projection.
...
func (provider *RecordedSagaReductionProvider) Journal(input Input) (*CrossExchangeJournal, error) {
	first, err := sagaJournalFirstOrdinal(input.Ordinal)
	if provider == nil || err != nil {
		return nil, strategyError("saga_reduction_journal_invalid")
	}
	return NewCrossExchangeJournal(provider.journal, JournalContext{RunID: provider.runID,
		PortfolioID: provider.portfolioID, Owner: provider.owner, ConfigurationHash: input.ConfigurationHash,
		RecordedAt: domain.EventTime{UTC: input.Now, Sequence: input.Ordinal}, FirstOrdinal: first})
}

// Attribution returns the independent recorded portfolio attribution. It does
// not derive P&L from a current balance or venue state.
func (provider *RecordedSagaReductionProvider) Attribution(
	input Input, _ Candidate, _ SimulationResult,
) (PortfolioAttribution, error) {
	reduction, err := input.RecordedReduction()
	if provider == nil || err != nil {
		return PortfolioAttribution{}, strategyError("saga_reduction_attribution_invalid")
	}
	return reduction.Attribution, nil
}
...
// Release closes the exact allocation only after the reducer has posted and
// reconciled all evidence successfully.
func (provider *RecordedSagaReductionProvider) Release(
	ctx context.Context, allocation backtest.SagaAllocation,
) error {
	if provider == nil {
		return strategyError("saga_reduction_release_invalid")
	}
	return provider.allocator.CloseSagaAllocation(ctx, allocation, backtest.AllocationReleased)
}
...
func (reducer *SagaReducer) ReduceSaga(
	...
	attribution, err := reducer.provider.Attribution(allocation.Input, allocation.Candidate, executionValue.Simulation)
	if err != nil {
		return backtest.EventResult{}, strategyError("saga_reduction_attribution_invalid")
	}
	transactions, err := journal.Transactions(allocation.Candidate, executionValue.Simulation, attribution)
	if err != nil || len(transactions) == 0 || journal.Post(allocation.Candidate, executionValue.Simulation, attribution) != nil {
	...
	if err = reducer.provider.Release(ctx, allocated); err != nil {
		return backtest.EventResult{}, strategyError("saga_reduction_release_failed")
	}
...
func rankedCandidates(candidates []Candidate) []Candidate {
	...
	sort.SliceStable(ordered, func(left, right int) bool {
		if comparison := ordered[left].Economics.WorstClosedCycleProfit.Compare(
			ordered[right].Economics.WorstClosedCycleProfit); comparison != 0 {
			return comparison > 0
		}
		if comparison := ordered[left].Economics.ExpectedClosedCycleProfit.Compare(
			ordered[right].Economics.ExpectedClosedCycleProfit); comparison != 0 {
			return comparison > 0
		}
...
func sagaReservationID(candidateID string) (domain.ReservationID, error) {
	...
	return domain.NewReservationID("crossarb-saga-" + hex.EncodeToString(digest[:10]))
}
