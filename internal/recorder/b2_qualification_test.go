...
func TestB2DualRecorderContentionPreservesSharedOrdinalContinuity(t *testing.T) {
	base := t.TempDir()
	...
	binanceRecorder := newB2QualificationRecorder(t, filepath.Join(base, "binance"), "binance", ordinals, profile)
	bybitRecorder := newB2QualificationRecorder(t, filepath.Join(base, "bybit"), "bybit", ordinals, profile)
	instrument := recorderInstrument(t)

	const recordsPerExchange = 64
	recordB2QualificationConcurrently(t, instrument, recordsPerExchange, binanceRecorder, bybitRecorder)
	manifests := flushB2QualificationPair(t, binanceRecorder, bybitRecorder)
	...
	tierA, err := BuildTierAManifest("b2-contention-tier-a", time.Date(2026, 7, 27, 0, 0, 0, 0, time.UTC),
...
func recordB2QualificationConcurrently(t *testing.T, instrument domain.Instrument, count int, binanceRecorder, bybitRecorder *Recorder) {
	t.Helper()
	...
	for index := 1; index <= count; index++ {
		...
		for _, current := range []struct {
			...
			go func(exchange string, value *Recorder) {
				defer group.Done()
				errorsByRecord <- recordB2QualificationEvent(value, instrument, exchange, sequence)
...
type b2QualificationFlushResult struct {
	exchange string
	manifest DatasetManifest
	err      error
}

func flushB2QualificationPair(t *testing.T, binanceRecorder, bybitRecorder *Recorder) map[string]DatasetManifest {
	t.Helper()
	flushes := make(chan b2QualificationFlushResult, 2)
	go func() {
		manifest, err := binanceRecorder.Flush()
		flushes <- b2QualificationFlushResult{"binance", manifest, err}
	}()
	go func() {
		manifest, err := bybitRecorder.Flush()
		flushes <- b2QualificationFlushResult{"bybit", manifest, err}
	}()
...
func TestB2TierAFinalizationRejectsCompatibilityMismatch(t *testing.T) {
	base := t.TempDir()
	ordinals := &runtimecore.IngestOrdinals{}
	binanceRecorder := newB2QualificationRecorder(t, filepath.Join(base, "binance"), "binance", ordinals,
		CollectorProfile{Instance: "binance-1", Region: "test-region", MinimumReaderVersion: "dataset-reader.v2"})
	bybitRecorder := newB2QualificationRecorder(t, filepath.Join(base, "bybit"), "bybit", ordinals,
		CollectorProfile{Instance: "bybit-1", Region: "test-region", MinimumReaderVersion: "dataset-reader.v3"})
	instrument := recorderInstrument(t)
	if err := recordB2QualificationEvent(binanceRecorder, instrument, "binance", 1); err != nil {
		t.Fatal(err)
	}
	if err := recordB2QualificationEvent(bybitRecorder, instrument, "bybit", 1); err != nil {
		t.Fatal(err)
	}
	...
	_, err = BuildTierAManifest("b2-mismatch-tier-a", time.Date(2026, 7, 27, 0, 0, 0, 0, time.UTC),
		map[string]string{"binance": binanceRecorder.root, "bybit": bybitRecorder.root},
...
func newB2QualificationRecorder(t *testing.T, root, exchange string, ordinals *runtimecore.IngestOrdinals, profile CollectorProfile) *Recorder {
	t.Helper()
	value, err := NewB2(root, exchange+"-qualification", exchange+"-session", exchange, ordinals,
...
func recordB2QualificationEvent(value *Recorder, instrument domain.Instrument, exchange string, sequence uint64) error {
