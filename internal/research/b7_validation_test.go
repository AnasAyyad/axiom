...
func TestB7PreregistrationIsCanonicalCompleteAndBeforeFinalWindow(t *testing.T) {
	input := completeB7RegistrationInput()
...
func TestB7MultipleTestingAndSharpeAreDeterministicAndTrialDeflated(t *testing.T) {
...
func TestB7StatisticsMatchIndependentResearchGolden(t *testing.T) {
	payload, err := os.ReadFile("../../research/testdata/b7_statistics_golden.json")
	if err != nil {
		t.Fatal(err)
	}
	var golden struct {
	...
	if multipleErr != nil || sharpeErr != nil ||
		...
		t.Fatalf("B7 golden mismatch: %#v %#v %v %v", multiple, sharpe,
			multipleErr, sharpeErr)
	}
}

func TestB7ValidationSuiteQualifiesOnlyTierAPrimaryStatisticalEvidence(t *testing.T) {
	registration, _, err := RegisterExperiment(completeB7RegistrationInput())
	if err != nil {
		t.Fatal(err)
	}
	...
	lowConfidence := completeB7ValidationInput(registration)
...
func TestB7ChampionChallengerReportIsEvidenceNotPromotion(t *testing.T) {
	result := func(name string) ResultSlice {
		return ResultSlice{Name: name, NetReturn: "0.02", MaxDrawdown: "0.03", Trades: 120}
	}
	input := ChampionChallengerInput{ID: "comparison-b7", StrategyFamily: "trend",
...
func BenchmarkB7ValidationSuite(b *testing.B) {
	registration, _, _ := RegisterExperiment(completeB7RegistrationInput())
	input := completeB7ValidationInput(registration)
	b.ReportAllocs()
	for b.Loop() {
...
func FuzzB7MultipleTestingPreservesProbabilityBounds(f *testing.F) {
...
func completeB7RegistrationInput() ExperimentRegistrationInput {
	...
	return ExperimentRegistrationInput{ID: "experiment-b7",
		ResearchGenerationID: "generation-b7-1", StrategyVersionID: "trend-v1",
...
func completeB7ValidationInput(
	...
	confidence, _ := BlockBootstrapMean(
		[]string{"0.01", "0.02", "0.03", "0.04"}, 2, 100, "b7-registered-seed",
	)
	sources := []EvidenceSource{
		{RunID: "backtest-b7", Mode: "backtest", DatasetTier: "tier_a",
			ConfidenceLabel: "formal_tier_a", ResultHash: strings.Repeat("1", 64), Primary: true},
		{RunID: "replay-b7", Mode: "replay", DatasetTier: "tier_a",
			ConfidenceLabel: "formal_tier_a", ResultHash: strings.Repeat("2", 64), Primary: true},
		{RunID: "shadow-b7", Mode: "shadow", DatasetTier: "tier_a",
			ConfidenceLabel: "formal_tier_a", ResultHash: strings.Repeat("3", 64), Primary: true},
	}
	return ValidationSuiteInput{ID: "suite-b7",
