...
func TestB7PromotionRequiresPermissionRecentAuthenticationAndExplicitRequest(t *testing.T) {
	...
	principal := authentication.Principal{UserID: "owner-b7", SessionID: "session-b7",
		Permissions: []string{"research.promote"}, ReauthenticatedAt: now}
	request := PromotionRequest{StrategyVersionID: "trend-v1", EvidenceID: "suite-b7",
		EvidenceHash: strings.Repeat("a", 64), Target: MaturityBacktestValidated,
		ExpectedRevision: 1, IdempotencyKey: "promotion-b7-1", Reason: "registered gates passed"}
	...
	if _, err = service.Promote(context.Background(), principal, request); err == nil {
		t.Fatal("permissionless promotion accepted")
	}
	principal.Permissions = []string{"research.promote"}
	...
	if _, err = service.Promote(context.Background(), principal, request); err == nil {
		t.Fatal("V1B sandbox maturity accepted")
	}
...
func TestB7PromotionReturnsDurablePolicyRejection(t *testing.T) {
	now := time.Date(2026, 7, 24, 10, 0, 0, 0, time.UTC)
	clock, _ := domain.NewReplayClock(now)
	store := &promotionStoreStub{result: PromotionResult{CommandID: "stored-command",
	...
	request := PromotionRequest{StrategyVersionID: "trend-v1", EvidenceID: "suite-b7",
		EvidenceHash: strings.Repeat("a", 64), Target: MaturityBacktestValidated,
		ExpectedRevision: 1, IdempotencyKey: "promotion-b7-1", Reason: "review"}
	result, err := service.Promote(context.Background(), principal, request)
...
func TestB7MaturityTransitionsAreSequentialAndRejectable(t *testing.T) {
	valid := [][2]Maturity{
