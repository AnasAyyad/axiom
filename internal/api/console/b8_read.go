...
func (handler *handler) registerB8Reads(mux *http.ServeMux) {
	routes := []struct {
		pattern string
...
func (handler *handler) exchanges(writer http.ResponseWriter, request *http.Request, _ authentication.Principal) {
	limit, ok := handler.b8PageSize(writer, request)
	if !ok {
		return
	}
...
func (handler *handler) inventory(writer http.ResponseWriter, request *http.Request, _ authentication.Principal) {
	...
	if !validB8InventoryFilters(filters) {
		handler.writeServiceError(writer, request, ErrInvalidRequest)
		return
	}
...
func (handler *handler) b8PageSize(writer http.ResponseWriter, request *http.Request) (int, bool) {
...
func validB8InventoryFilters(filters InventoryFilters) bool {
	values := []struct {
		value string
		limit int
	}{
