...
func (store *SandboxRuntimeDispatcherStore) CreateCanarySession(
	ctx context.Context,
	command sandbox.CanarySessionCommand,
) (sandbox.CanarySession, error) {
	if !validCanarySessionCommand(command) {
		return sandbox.CanarySession{}, fmt.Errorf("sandbox_runtime_canary_session_invalid")
	}
	tx, err := store.pool.BeginTx(ctx, pgx.TxOptions{IsoLevel: pgx.Serializable})
	if err != nil {
		return sandbox.CanarySession{}, fmt.Errorf("sandbox_runtime_canary_session_begin_failed")
	}
	...
	if err = tx.Commit(ctx); err != nil {
		return sandbox.CanarySession{}, fmt.Errorf("sandbox_runtime_canary_session_commit_failed")
	}
	return result, nil
}
...
func selectCanaryAccount(
	...
	if err != nil || epoch <= 0 || cycle <= 0 ||
		...
		return sandbox.CanarySession{}, fmt.Errorf("sandbox_runtime_canary_account_not_ready")
	}
...
INSERT INTO sandbox_runtime_sandbox_sessions(
 ...
 created_at,updated_at
		...
		return fmt.Errorf("sandbox_runtime_canary_session_insert_failed")
	}
	if _, err := tx.Exec(ctx, `
INSERT INTO sandbox_runtime_sandbox_session_accounts(
 session_id,account_id,account_epoch
		...
		return fmt.Errorf("sandbox_runtime_canary_membership_insert_failed")
	}
	return nil
}
...
func (store *SandboxRuntimeDispatcherStore) CanaryAdmission(
	ctx context.Context,
	...
	if session == "" || armID == "" || account == "" || instrument == "" ||
		...
		return sandbox.EligibilitySnapshot{}, sandbox.EntrySafetySnapshot{}, 0,
			fmt.Errorf("sandbox_runtime_canary_admission_invalid")
	}
...
func (store *SandboxRuntimeDispatcherStore) readCanaryAdmissionFacts(
	ctx context.Context,
	session sandbox.SessionID,
	armID string,
	account sandbox.AccountID,
	...
	if err != nil || facts.epoch <= 0 || facts.cycle <= 0 {
		return canaryAdmissionFacts{},
			fmt.Errorf("sandbox_runtime_canary_admission_unavailable")
	}
	return facts, nil
}
...
WHERE account.id=$1
  ...
  AND market_observation.observed_at<=$6
  AND $6-market_observation.observed_at<=interval '250 milliseconds'`
...
func buildCanaryAdmission(
	...
	if json.Unmarshal(facts.eligibilityJSON, &eligibility) != nil ||
		...
		return sandbox.EligibilitySnapshot{}, sandbox.EntrySafetySnapshot{}, 0,
			fmt.Errorf("sandbox_runtime_canary_eligibility_invalid")
	}
...
func (store *SandboxRuntimeDispatcherStore) RecordCanaryEvidence(
	...
	id := "sandbox_runtime-canary-" + evidenceHash[:32]
	tag, err := store.pool.Exec(ctx, `
INSERT INTO sandbox_runtime_canary_evidence(
...
ON CONFLICT (exchange,canary_id,stage) DO NOTHING`,
	...
	if err != nil {
		return "", fmt.Errorf("sandbox_runtime_canary_evidence_insert_failed")
	}
...
WHERE exchange=$1 AND canary_id=$2 AND stage=$4`,
		...
		if err != nil || !same {
			return "", fmt.Errorf("sandbox_runtime_canary_evidence_conflict")
		}
	}
