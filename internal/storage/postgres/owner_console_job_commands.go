...
func (store *OwnerConsoleStore) CreateJob(ctx context.Context, principal authentication.Principal, kind, key string, request any) (generated.JobResource, error) {
	...
	configurationID, datasetID, strategyID, generationID, validRequest := offlineJobReferences(request)
	if !validRequest {
		return generated.JobResource{}, console.ErrInvalidRequest
	}
	...
	existingID, found, err := ownerConsoleExistingJob(ctx, tx, dedupe, hash)
	if err != nil {
		return generated.JobResource{}, err
	}
	if found {
	...
	if err = validateOfflineJobCreate(ctx, tx, principal.UserID, configurationID, datasetID, strategyID, generationID, request); err != nil {
		return generated.JobResource{}, err
	}
	now := store.clock.Now().UTC
	jobID, _ := ownerConsoleIdentifier(kind)
	commandID, _ := ownerConsoleIdentifier("command")
	auditID, _ := ownerConsoleIdentifier("audit")
	if err = insertOwnerConsoleCommand(ctx, tx, commandID, principal, key, hash, "create_"+kind, kind, jobID, "create durable research run", now, auditID, commandID); err != nil {
		return generated.JobResource{}, err
	}
	if _, err = tx.Exec(ctx, `INSERT INTO jobs(id,job_type,idempotency_key,state,payload_hash,created_at,updated_at,owner_user_id,request_payload,max_attempts) VALUES($1,$2,$3,'QUEUED',$4,$5,$5,$6,$7,3)`, jobID, kind, dedupe, hash, now, principal.UserID, string(payload)); err != nil {
		return generated.JobResource{}, ownerConsoleConstraintError(err)
	}
	if _, err = completeOwnerConsoleCommand(ctx, tx, commandID, auditID, principal, "create_"+kind, jobID, hash, map[string]any{"job_id": jobID, "state": "QUEUED"}, now, commandID); err != nil {
		return generated.JobResource{}, err
	}
	if err = tx.Commit(ctx); err != nil {
		return generated.JobResource{}, err
...
func ownerConsoleExistingJob(ctx context.Context, tx pgx.Tx, dedupe, payloadHash string) (string, bool, error) {
...
func validateOfflineJobCreate(ctx context.Context, tx pgx.Tx, owner, configurationID, datasetID, strategyID, generationID string, request any) error {
	var references int
	...
	if replay, ok := request.(generated.ReplayJobRequest); ok && replay.IncidentId != nil {
		if err = validateOwnerConsoleIncidentReplay(ctx, tx, replay); err != nil {
			return err
		}
	}
...
func validateOwnerConsoleIncidentReplay(ctx context.Context, tx pgx.Tx, request generated.ReplayJobRequest) error {
	if request.IncidentId == nil || *request.IncidentId == "" || request.FirstOrdinal == nil || request.LastOrdinal == nil {
		return console.ErrPrecondition
	}
	datasetID, first, last, err := ownerConsoleIncidentReplayWindow(ctx, tx, *request.IncidentId)
	if err != nil {
		return err
	}
...
func offlineJobReferences(request any) (string, string, string, string, bool) {
	switch value := request.(type) {
	case generated.OfflineJobRequest:
		return value.ConfigurationId, value.DatasetId, ownerConsoleStrategyVersionID(string(value.StrategyVersion)), value.ResearchGenerationId,
			value.RootSeedHash != ""
...
func ownerConsoleStrategyVersionID(value string) string {
	switch value {
	case "trend-following@1.0.0":
		return "trend-following-1-0-0"
	case "mean-reversion@1.0.0":
		return "mean-reversion-1-0-0"
	case "triangular-arbitrage@1.0.0":
		return "triangular-arbitrage-1-0-0"
	case "cross-exchange-arbitrage@1.0.0":
		return "cross-exchange-arbitrage-1-0-0"
	case "inventory-rebalancing@1.0.0":
		return "inventory-rebalancing-1-0-0"
	}
	return value
}
