...
func TestOwnerConsoleSafeAuditDetailIsBoundedCanonicalAndExcludesSessionSecrets(t *testing.T) {
	command, targetType, targetID := "resume_risk", "risk", "global"
	reason, state := "owner verified recovery", "applied"
	detail := ownerConsoleSafeAuditDetail(strings.Repeat("a", 64), &command, &targetType, &targetID, &reason, &state,
	...
	oversized := ownerConsoleSafeAuditDetail(strings.Repeat("b", 64), &command, &targetType, &targetID, &reason, &state,
