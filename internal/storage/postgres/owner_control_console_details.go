...
func (store *OwnerConsoleStore) ownerControlConfigurations(
	ctx context.Context,
	position string,
...
ORDER BY id DESC LIMIT $4`, position, query.From, query.To, query.PageSize+1)
	...
	for rows.Next() {
		...
		items = append(items, ownerControlResource(id, "configuration_revision", revision, state, &occurred,
			map[string]any{"configuration_hash": hash, "actor": actor}, map[string]string{}))
	}
	return items, rows.Err()
}

func (store *OwnerConsoleStore) ownerControlQualifications(
	ctx context.Context,
	position string,
	query console.OwnerControlListQuery,
) ([]generated.OwnerControlResource, error) {
...
FROM owner_console_qualification_catalogue catalogue
LEFT JOIN LATERAL (
  SELECT * FROM owner_console_qualification_runs WHERE qualification_id=catalogue.id
...
ORDER BY catalogue.id DESC LIMIT $5`, position, query.Filters["state"], query.From,
	...
	for rows.Next() {
		...
		items = append(items, ownerControlResource(id, "qualification", revision, state, &occurred,
			map[string]any{"name": name, "kind": kind, "duration_seconds": duration,
				"owner_start_required": owner, "latest_run_id": runID},
			map[string]string{"sandbox": ownerControlQualificationLink(id)}))
	}
	return items, rows.Err()
}

// OwnerControlResource returns one explicitly supported detail projection.
func (store *OwnerConsoleStore) OwnerControlResource(
	ctx context.Context,
	kind, id string,
) (generated.OwnerControlResource, error) {
	switch kind {
	case "strategy":
		return store.ownerControlStrategyDetail(ctx, id)
	case "order":
		return store.ownerControlOrderDetail(ctx, id)
	case "command":
		return store.ownerControlCommandDetail(ctx, id)
	}
	return generated.OwnerControlResource{}, console.ErrNotFound
}

func (store *OwnerConsoleStore) ownerControlOrderDetail(ctx context.Context, id string) (generated.OwnerControlResource, error) {
...
FROM orders WHERE id=$1`, id).Scan(&state, &revision, &occurred, &instrument,
	...
	if err != nil {
		return generated.OwnerControlResource{}, err
	}
	return ownerControlResource(id, "order", revision, state, &occurred,
...
func (store *OwnerConsoleStore) ownerControlStrategyDetail(ctx context.Context, id string) (generated.OwnerControlResource, error) {
...
JOIN owner_console_strategy_controls control ON control.strategy_id=definition.id
...
GROUP BY definition.id,control.strategy_id`, id).Scan(
	...
	return ownerControlResource(id, "strategy", revision, runtimeState, &occurred,
...
func (store *OwnerConsoleStore) ownerControlCommandDetail(ctx context.Context, id string) (generated.OwnerControlResource, error) {
...
FROM command_requests WHERE id=$1`, id).Scan(&state, &kind, &target, &revision, &correlation, &occurred)
	...
	result := ownerControlResource(id, "command", revision, state, &occurred,
