...
func (store *OwnerConsoleStore) Instruments(ctx context.Context, cursor string, limit int) (generated.InstrumentPage, error) {
	symbol, id, err := decodeOwnerConsolePairCursor(store.cursor, "instruments", cursor)
	if err != nil {
...
func instrumentPage(store *OwnerConsoleStore, items []generated.Instrument, limit int) generated.InstrumentPage {
	...
	if len(items) > limit {
		...
		next := store.cursor.Encode("instruments", last.Symbol+ownerConsoleCursorSeparator+last.Id)
...
func (store *OwnerConsoleStore) Portfolios(ctx context.Context, cursor string, limit int) (generated.PortfolioPage, error) {
...
func (store *OwnerConsoleStore) Portfolio(ctx context.Context, id string) (generated.PortfolioDetail, error) {
	var mode string
...
func (store *OwnerConsoleStore) Journal(ctx context.Context, portfolioID, cursor string, limit int) (generated.JournalPage, error) {
	scope := "journal:" + portfolioID
	occurred, transactionID, line, err := decodeOwnerConsoleTimeCursor(store.cursor, scope, cursor)
	if err != nil {
    ...
    ORDER BY jt.recorded_at DESC,jt.id DESC,le.line_number LIMIT $5`, portfolioID, nullableOwnerConsoleTime(occurred), transactionID, line, limit+1)
	if err != nil {
		return generated.JournalPage{}, err
	}
	defer rows.Close()
	...
	if len(items) > limit {
		...
		next := encodeOwnerConsoleTimeCursor(store.cursor, scope, last.OccurredAt, last.TransactionId, lastLine)
		page.NextCursor = &next
	}
	return page, rows.Err()
}

func nullableOwnerConsoleTime(value time.Time) any {
	if value.IsZero() {
		return nil
	}
	return value
