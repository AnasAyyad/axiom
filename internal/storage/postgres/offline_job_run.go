...
func (store *OfflineJobStore) attachRun(ctx context.Context, kind string, payload json.RawMessage, claim backtest.JobClaim) error {
	...
	if err != nil || claim.Manifest.RunID.Value() != claim.ID || claim.Manifest.Mode != kind ||
		...
		return fmt.Errorf("owner_console_job_run_identity_invalid")
	}
	...
	if err = tx.QueryRow(ctx, `SELECT coalesce(run_id,''),claim_expires_at FROM jobs WHERE id=$1 AND state IN ('RUNNING','PAUSE_REQUESTED')
		...
		return fmt.Errorf("owner_console_job_lease_lost")
	}
	if linked != "" {
		if err = verifyOwnerConsoleLinkedRun(ctx, tx, linked, manifestHash); err != nil {
			return err
		}
	...
	if err = insertOwnerConsoleRun(ctx, tx, request, claim.Manifest, manifestHash, now); err != nil {
		return err
	}
	renewedLeaseEnd := renewedOfflineJobLease(now, leaseEnd)
	...
	if err != nil {
		return fmt.Errorf("owner_console_job_run_link_failed: %w", err)
	}
	if tag.RowsAffected() != 1 {
		return fmt.Errorf("owner_console_job_run_link_conflict")
	}
	return tx.Commit(ctx)
}

func renewedOfflineJobLease(now, leaseEnd time.Time) time.Time {
	renewed := now.Add(offlineJobLease)
...
func insertOwnerConsoleRun(ctx context.Context, tx pgx.Tx, request ownerConsoleOfflineRequest, manifest backtest.RunManifest,
	manifestHash string, now time.Time) error {
	strategyID := ownerConsoleStrategyVersionID(request.StrategyVersion)
	...
	_, err = tx.Exec(ctx, `INSERT INTO run_manifests(run_id,manifest_hash,code_commit,go_version,architecture,
      ...
      $19,$20,$21,$22,$23,$24)`, manifest.RunID.Value(), manifestHash, manifest.CodeCommit, manifest.Build.GoVersion,
		manifest.Build.Architecture, manifest.Build.OperatingSystem, ownerConsoleSHA256(flags), manifest.Build.GoSumHash,
		...
		manifest.Dataset.NormalizationVersion, ownerConsoleSHA256(segments), manifest.ConfigurationHash,
	...
	if err != nil || tag.RowsAffected() != 1 {
		return fmt.Errorf("owner_console_run_start_conflict")
	}
	return nil
}

func verifyOwnerConsoleLinkedRun(ctx context.Context, tx pgx.Tx, runID, manifestHash string) error {
	var stored string
	if err := tx.QueryRow(ctx, `SELECT manifest_hash FROM run_manifests WHERE run_id=$1`, runID).Scan(&stored); err != nil || stored != manifestHash {
		return fmt.Errorf("owner_console_job_run_manifest_conflict")
	}
	return nil
}

func (store *OfflineJobStore) completeRun(ctx context.Context, id string, result backtest.CanonicalResult,
	...
	if err = tx.QueryRow(ctx, `SELECT run_id,job_type,request_payload FROM jobs WHERE id=$1 AND state='RUNNING' AND claim_owner=$2
	  FOR UPDATE`, id, store.owner).Scan(&runID, &kind, &requestPayload); err != nil || runID == "" {
		return fmt.Errorf("owner_console_job_run_missing")
	}
	request, err := decodeOwnerConsoleOfflineRequest(kind, requestPayload)
	if err != nil || request.ResearchGenerationID == "" || result.ManifestHash == "" {
		return fmt.Errorf("owner_console_job_report_identity_invalid")
	}
	reportID, reportHash, err := insertOwnerConsoleRunEvidenceReport(ctx, tx, id, runID, kind, request, result, now)
	if err != nil {
		return err
	}
	projection, err := json.Marshal(offlineJobResult(result, reportID, reportHash))
	if err != nil {
		return fmt.Errorf("owner_console_job_result_invalid")
	}
	if err = insertOwnerConsoleCanonicalOutputs(ctx, tx, runID, result.Events); err != nil {
		return err
	}
	...
	if err = finishOwnerConsoleRunAndJob(ctx, tx, id, runID, store.owner, projection, now); err != nil {
		return err
	}
	return tx.Commit(ctx)
}

func insertOwnerConsoleRunEvidenceReport(ctx context.Context, tx pgx.Tx, id, runID, kind string, request ownerConsoleOfflineRequest,
	result backtest.CanonicalResult, now time.Time) (string, string, error) {
	metricValues := offlineJobMetrics(result)
...
func finishOwnerConsoleRunAndJob(ctx context.Context, tx pgx.Tx, id, runID, owner string, projection []byte, now time.Time) error {
	...
	if updateErr != nil || runTag.RowsAffected() != 1 {
		return fmt.Errorf("owner_console_run_completion_conflict")
	}
	...
	if err != nil || tag.RowsAffected() != 1 {
		return fmt.Errorf("owner_console_job_completion_conflict")
	}
	return nil
}

func insertOwnerConsoleCanonicalOutputs(ctx context.Context, tx pgx.Tx, runID string, events []backtest.EventResult) error {
	for _, event := range events {
		if event.Ordinal == 0 || event.Ordinal > uint64(^uint64(0)>>1) || !json.Valid(event.Decision) || !json.Valid(event.Orders) ||
			...
			return fmt.Errorf("owner_console_run_output_invalid")
		}
		...
		for _, output := range outputs {
			hash := ownerConsoleSHA256(output.payload)
			...
			if insertErr != nil {
				return fmt.Errorf("owner_console_run_output_persist_failed")
			}
			if tag.RowsAffected() == 0 {
				...
				if err = tx.QueryRow(ctx, `SELECT output_hash::text,canonical_payload FROM run_canonical_outputs
				  WHERE run_id=$1 AND output_kind=$2 AND ordinal=$3`, runID, output.kind, int64(event.Ordinal)).
					...
					return fmt.Errorf("owner_console_run_output_conflict")
				}
			}
		}
	}
...
func (store *OfflineJobStore) failRun(ctx context.Context, id, reason string, now time.Time) error {
	tx, err := store.pool.BeginTx(ctx, pgx.TxOptions{IsoLevel: pgx.Serializable})
	if err != nil {
		return err
	}
	...
	if runID != "" {
		...
		if updateErr != nil || runTag.RowsAffected() != 1 {
			return fmt.Errorf("owner_console_run_failure_conflict")
		}
	}
	...
	if err != nil || tag.RowsAffected() != 1 {
		return fmt.Errorf("owner_console_job_failure_conflict")
	}
	return tx.Commit(ctx)
}
