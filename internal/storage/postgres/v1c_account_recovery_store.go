...
func (store *V1CDispatcherStore) RecordAccountSnapshot(
	ctx context.Context,
	id string,
	snapshot sandbox.AccountSnapshot,
) error {
	if id == "" {
		return fmt.Errorf("v1c_account_snapshot_invalid")
	}
	...
	if err != nil {
		return fmt.Errorf("v1c_account_snapshot_encode_failed")
	}
	tag, err := store.pool.Exec(ctx, `
INSERT INTO v1c_account_snapshots(
...
ON CONFLICT DO NOTHING`,
	...
	if err != nil {
		return fmt.Errorf("v1c_account_snapshot_insert_failed")
	}
	if tag.RowsAffected() == 0 &&
		!store.sameV1CAccountSnapshot(ctx, id, snapshot, string(balances)) {
		return fmt.Errorf("v1c_account_snapshot_identity_conflict")
	}
	return nil
}

func (store *V1CDispatcherStore) sameV1CAccountSnapshot(
	ctx context.Context,
	id string,
	snapshot sandbox.AccountSnapshot,
	balances string,
...
FROM v1c_account_snapshots WHERE id=$1`,
		id,
		snapshot.AccountID,
		snapshot.Epoch,
		balances,
...
func (store *V1CDispatcherStore) RecordAccountReset(
	ctx context.Context,
	...
	if err != nil {
		return fmt.Errorf("v1c_account_reset_begin_failed")
	}
	defer func() { _ = tx.Rollback(context.Background()) }()
	newEpoch, err := advanceV1CResetEpoch(ctx, tx, incident)
	if err != nil {
		return err
	}
	if err = quarantineV1CResetState(ctx, tx, incident); err != nil {
		return err
	}
	if err = insertV1CResetIncident(ctx, tx, incident, newEpoch); err != nil {
		return err
	}
	if _, err = tx.Exec(ctx, `
DELETE FROM v1c_account_leases WHERE account_id=$1`, incident.AccountID); err != nil {
		return fmt.Errorf("v1c_account_reset_lease_release_failed")
	}
	if err = tx.Commit(ctx); err != nil {
		return fmt.Errorf("v1c_account_reset_commit_failed")
	}
	return nil
}
...
VALUES ($1,$2,'exchange_reset',$3)`,
		...
		return 0, fmt.Errorf("v1c_account_reset_open_epoch_failed")
	}
	if _, err := tx.Exec(ctx, `
...
WHERE id=$1`, incident.AccountID, newEpoch, incident.DetectedAt); err != nil {
		return 0, fmt.Errorf("v1c_account_reset_lock_failed")
	}
	return newEpoch, nil
}

func insertV1CResetIncident(
	ctx context.Context,
	tx pgx.Tx,
	incident sandbox.AccountResetIncident,
	newEpoch uint64,
	...
	if _, err := tx.Exec(ctx, `
INSERT INTO v1c_reset_incidents(
 id,account_id,prior_epoch,new_epoch,evidence_hash,state,detected_at
		...
		return fmt.Errorf("v1c_account_reset_incident_insert_failed")
	}
	for _, adjustment := range incident.Adjustments {
		if err := insertV1CExternalAdjustment(ctx, tx, incident, adjustment); err != nil {
			return err
		}
	}
	return nil
...
func insertV1CExternalAdjustment(
	ctx context.Context,
	tx pgx.Tx,
	incident sandbox.AccountResetIncident,
	...
	if _, err := tx.Exec(ctx, `
INSERT INTO v1c_external_adjustments(
 ...
 pnl_effect,recorded_at
		...
		return fmt.Errorf("v1c_account_reset_adjustment_insert_failed")
	}
	return nil
}

func quarantineV1CResetState(
	ctx context.Context,
	tx pgx.Tx,
	incident sandbox.AccountResetIncident,
) error {
	if err := quarantineV1CResetCapacity(ctx, tx, incident); err != nil {
		return err
	}
	return quarantineV1CResetControl(ctx, tx, incident)
}

func quarantineV1CResetCapacity(
	ctx context.Context,
	tx pgx.Tx,
	incident sandbox.AccountResetIncident,
) error {
...
WHERE account_id=$1 AND account_epoch=$2 AND state<>'TERMINAL'`,
		...
		return fmt.Errorf("v1c_account_reset_order_quarantine_failed")
	}
	if _, err := tx.Exec(ctx, `
...
WHERE account_id=$1 AND account_epoch=$2 AND state='ACTIVE'`,
		...
		return fmt.Errorf("v1c_account_reset_reservation_quarantine_failed")
	}
	if _, err := tx.Exec(ctx, `
...
WHERE EXISTS (
  ...
  WHERE outbox.plan_id=plan.id AND outbox.account_id=$1
    AND outbox.account_epoch=$2
		...
		return fmt.Errorf("v1c_account_reset_plan_quarantine_failed")
	}
	return nil
}

func quarantineV1CResetControl(
	ctx context.Context,
	tx pgx.Tx,
	incident sandbox.AccountResetIncident,
) error {
...
WHERE EXISTS (
  ...
  WHERE membership.session_id=arm.sandbox_session_id
    AND membership.account_id=$1 AND membership.account_epoch=$2
		...
		return fmt.Errorf("v1c_account_reset_arm_revoke_failed")
	}
	if _, err := tx.Exec(ctx, `
...
WHERE EXISTS (
  ...
  WHERE membership.session_id=session.id
    AND membership.account_id=$1 AND membership.account_epoch=$2
		...
		return fmt.Errorf("v1c_account_reset_session_degrade_failed")
	}
	return nil
}

var _ sandbox.AccountRecoveryRepository = (*V1CDispatcherStore)(nil)
