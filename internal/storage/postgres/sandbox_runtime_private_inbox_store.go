...
func (store *SandboxRuntimeDispatcherStore) AppendPrivateEvent(
	ctx context.Context,
	outboxID string,
	fence uint64,
	...
	if err != nil {
		return fmt.Errorf("sandbox_runtime_private_event_encode_failed")
	}
	alreadyReduced, err := store.persistSandboxRuntimePrivateInbox(ctx, event, payload, kill)
	if err != nil || alreadyReduced {
		return err
	}
	return store.reduceSandboxRuntimePrivateInbox(ctx, outboxID, fence, event, payload, kill)
}

func (store *SandboxRuntimeDispatcherStore) persistSandboxRuntimePrivateInbox(
	ctx context.Context,
	event sandbox.PrivateEvent,
	payload []byte,
	kill sandbox.KillPoint,
	...
	if err != nil {
		return false, fmt.Errorf("sandbox_runtime_private_event_begin_failed")
	}
	defer func() { _ = inboxTx.Rollback(context.Background()) }()
	tag, err := inboxTx.Exec(ctx, `
INSERT INTO sandbox_runtime_private_inbox(
...
ON CONFLICT (account_id,account_epoch,event_identity) DO NOTHING`,
	...
	if err != nil {
		return false, fmt.Errorf("sandbox_runtime_private_event_insert_failed")
	}
	alreadyReduced := false
	if tag.RowsAffected() == 0 {
		alreadyReduced, err = verifySandboxRuntimeDuplicateInbox(ctx, inboxTx, event, payload)
		if err != nil {
			return false, err
		}
	}
	...
	if err = inboxTx.Commit(ctx); err != nil {
		return false, fmt.Errorf("sandbox_runtime_private_event_inbox_commit_failed")
	}
...
func verifySandboxRuntimeDuplicateInbox(
	ctx context.Context,
	tx pgx.Tx,
	event sandbox.PrivateEvent,
	payload []byte,
...
WHERE account_id=$1 AND account_epoch=$2 AND event_identity=$3`,
	...
	if mismatch {
		return false, fmt.Errorf("sandbox_runtime_private_event_identity_conflict")
	}
	return alreadyReduced, nil
}

func (store *SandboxRuntimeDispatcherStore) reduceSandboxRuntimePrivateInbox(
	ctx context.Context,
	outboxID string,
	fence uint64,
	event sandbox.PrivateEvent,
	...
	if err != nil {
		return fmt.Errorf("sandbox_runtime_private_event_reduce_begin_failed")
	}
	defer func() { _ = reductionTx.Rollback(context.Background()) }()
	var reducedAt *time.Time
	if err = reductionTx.QueryRow(ctx, `
SELECT reduced_at FROM sandbox_runtime_private_inbox WHERE id=$1 FOR UPDATE`,
		event.Identity).Scan(&reducedAt); err != nil {
		return fmt.Errorf("sandbox_runtime_private_event_missing")
	}
	if reducedAt != nil {
		return reductionTx.Commit(ctx)
	}
	if event.OrderEvent != nil {
		if err = reduceSandboxRuntimeOrderEvent(
...
UPDATE sandbox_runtime_private_inbox SET reduced_at=$2 WHERE id=$1`, event.Identity, event.ReceivedAt); err != nil {
		return fmt.Errorf("sandbox_runtime_private_event_reduce_mark_failed")
	}
	if err = kill.Hit(ctx, sandbox.KillBeforeReductionCommit); err != nil {
		return err
	}
	if err = reductionTx.Commit(ctx); err != nil {
		return fmt.Errorf("sandbox_runtime_private_event_commit_failed")
	}
	return kill.Hit(ctx, sandbox.KillAfterReductionCommit)
}

func reduceSandboxRuntimeOrderEvent(
	ctx context.Context,
	tx pgx.Tx,
	outboxID string,
	fence uint64,
	...
	lockedID, record, err := lockSandboxRuntimePrivateOutbox(ctx, tx, outboxID, fence, event)
	if err != nil {
		return err
	}
	if replayed, replayErr := reduceSandboxRuntimeCanonicalReplay(
		ctx, tx, event, payload, kill,
	); replayErr != nil || replayed {
		return replayErr
	}
	orderEvents, err := readSandboxRuntimeOrderHistory(ctx, tx, event)
	if err != nil {
		return err
	}
	...
	state, terminal := sandboxRuntimeOrderState(order.State)
	if err = advanceSandboxRuntimeOutbox(ctx, tx, lockedID, fence, state, terminal, event.ReceivedAt); err != nil {
		return err
	}
	if err = kill.Hit(ctx, sandbox.KillAfterReducerUpdate); err != nil {
		return err
	}
	if err = applySandboxRuntimeOrderReductionEffects(ctx, tx, record, event, payload, order, state, terminal, kill); err != nil {
		return err
	}
	return updateSandboxRuntimePlanState(ctx, tx, record.Submission.PlanID.String())
}

func applySandboxRuntimeOrderReductionEffects(ctx context.Context, tx pgx.Tx, record sandbox.SubmissionOutbox,
	event sandbox.PrivateEvent, payload []byte, order execution.Order, state string, terminal bool,
	kill sandbox.KillPoint,
) error {
	if event.Kind == sandbox.PrivateFillEvent {
		if err := postSandboxRuntimeFill(ctx, tx, event, payload, kill); err != nil {
			return err
		}
		if err := postSandboxStrategyAccounting(ctx, tx, record, event); err != nil {
			return err
		}
	}
	if terminal {
		if err := closeSandboxRuntimeReservation(ctx, tx, event, state, len(order.Fills) > 0, kill); err != nil {
			return err
		}
	}
	var filledOutput *sandbox.FilledOutput
	if state == "FILLED" {
		output, err := sandbox.FilledSubmissionOutput(record.Submission, order)
		if err == nil {
			filledOutput = &output
		}
	}
	return advanceSandboxRuntimeSequentialPlan(ctx, tx, record.Submission.PlanID.String(), record.LegIndex,
		state, filledOutput, event.ReceivedAt)
}

func lockSandboxRuntimePrivateOutbox(
	ctx context.Context,
	tx pgx.Tx,
	outboxID string,
	fence uint64,
...
WHERE account_id=$1 AND account_epoch=$2 AND order_id=$3 FOR UPDATE`,
			...
			return "", sandbox.SubmissionOutbox{}, fmt.Errorf("sandbox_runtime_private_event_order_missing")
		}
	} else {
		var lockedID string
		if err := tx.QueryRow(ctx, `
...
WHERE id=$1 AND account_id=$2 AND account_epoch=$3 AND order_id=$4 FOR UPDATE`,
	...
	var leaseValid bool
	if err := tx.QueryRow(ctx, `
SELECT EXISTS(SELECT 1 FROM sandbox_runtime_account_leases
WHERE account_id=$1 AND fencing_token=$2 AND expires_at>$3)`,
		...
		return "", sandbox.SubmissionOutbox{}, fmt.Errorf("sandbox_runtime_stale_fencing_token")
	}
	record, err := readSandboxRuntimeOutbox(ctx, tx, outboxID)
	return outboxID, record, err
}

func reduceSandboxRuntimeCanonicalReplay(
	ctx context.Context,
	tx pgx.Tx,
	event sandbox.PrivateEvent,
	payload []byte,
	...
	replayed, err := canonicalSandboxRuntimePrivateReplay(ctx, tx, event, payload)
	if err != nil || !replayed {
		return replayed, err
	}
...
func canonicalSandboxRuntimePrivateReplay(
	ctx context.Context,
	tx pgx.Tx,
	event sandbox.PrivateEvent,
	payload []byte,
	...
	if err := tx.QueryRow(ctx, `
SELECT EXISTS(
  SELECT 1 FROM sandbox_runtime_private_inbox
  WHERE account_id=$1 AND account_epoch=$2 AND order_id=$3
    ...
    AND canonical_event=$5::jsonb
		...
		return false, fmt.Errorf("sandbox_runtime_private_event_replay_check_failed")
	}
	return replayed, nil
}

func readSandboxRuntimeOrderHistory(
	ctx context.Context,
	tx pgx.Tx,
	event sandbox.PrivateEvent,
) ([]execution.OrderEvent, error) {
	rows, err := tx.Query(ctx, `
SELECT canonical_event FROM sandbox_runtime_private_inbox
WHERE account_id=$1 AND account_epoch=$2 AND order_id=$3
...
ORDER BY occurred_at,received_at,id`,
	...
	if err != nil {
		return nil, fmt.Errorf("sandbox_runtime_private_event_history_failed")
	}
	defer rows.Close()
	...
	for rows.Next() {
		...
		if rows.Scan(&encoded) != nil || json.Unmarshal(encoded, &orderEvent) != nil {
			return nil, fmt.Errorf("sandbox_runtime_private_event_history_invalid")
		}
