...
func TestPortfolioRiskPostgresPortfolioRiskRecoveryQualification(t *testing.T) {
	dsn := os.Getenv("AXIOM_PORTFOLIO_RISK_TEST_DSN")
	if dsn == "" {
		t.Skip("AXIOM_PORTFOLIO_RISK_TEST_DSN is not set")
	}
	configuration, err := pgxpool.ParseConfig(dsn)
	if err != nil || !strings.HasSuffix(configuration.ConnConfig.Database, "_portfolio_risk_test") {
		t.Fatal("portfolio and risk integration requires a dedicated database ending _portfolio_risk_test")
	}
	...
	if applied, applyErr := ApplyMigrations(ctx, pool); applyErr != nil || applied != len(migrations) {
		t.Fatalf("portfolio and risk migrations = %d %v", applied, applyErr)
	}
	assertPortfolioRiskRolePermissions(t, ctx, pool)
	seedPortfolioRiskReferences(t, ctx, pool)
	repository, err := NewPortfolioRiskRepository(pool)
	if err != nil {
		t.Fatal(err)
	}
	assertPortfolioRiskInitialization(t, ctx, pool, repository)
	winner := assertPortfolioRiskContention(t, ctx, pool, repository)
	assertPortfolioRiskAtomicClose(t, ctx, pool, repository, winner)
	assertPortfolioRiskAtomicPartialFills(t, ctx, pool, repository)
	assertPortfolioRiskRiskReconciliationEvidence(t, ctx, pool)
	assertPortfolioRiskRecoveryEvidence(t, ctx, pool)
}

func assertPortfolioRiskAtomicPartialFills(t *testing.T, ctx context.Context, pool *pgxpool.Pool, repository *PortfolioRiskRepository) {
	t.Helper()
	seedPortfolioRiskExecutionOrder(t, ctx, pool)
	allocation := portfolioRiskAllocationFixture(99)
	allocation.LiquidityRevision = 3
	if err := repository.Reserve(ctx, allocation); err != nil {
		t.Fatal(err)
	}
	first := portfolioRiskFillFixture(false)
	stale := first
	stale.Liquidity.Revision++
	if err := repository.SettleFill(ctx, stale); err == nil {
		t.Fatal("stale partial-fill CAS committed")
	}
	assertPortfolioRiskFillCounts(t, ctx, pool, 0)
	if err := repository.SettleFill(ctx, first); err != nil {
		t.Fatal(err)
	}
	assertPortfolioRiskPartialProjection(t, ctx, pool)
	second := portfolioRiskFillFixture(true)
	if err := repository.SettleFill(ctx, second); err != nil {
		t.Fatal(err)
	}
	assertPortfolioRiskFillCounts(t, ctx, pool, 2)
	...
	if fundsState != "consumed" || fundsRemaining != "0.000000000000000000" ||
		...
		t.Fatalf("final portfolio and risk fill = %s/%s %s/%s %s/%s/%s", fundsState, fundsRemaining,
			liquidityState, liquidityRemaining, usdtAvailable, usdtReserved, btcAvailable)
	}
}

func seedPortfolioRiskExecutionOrder(t *testing.T, ctx context.Context, pool *pgxpool.Pool) {
	t.Helper()
...
type portfolioRiskFillConfig struct {
	final                                                             bool
...
func newPortfolioRiskFillConfig(final bool) portfolioRiskFillConfig {
	nowValue := time.Date(2026, 7, 16, 10, 7, 0, 0, time.UTC)
	config := portfolioRiskFillConfig{final: final, now: pgTimestamp(nowValue), ordinal: 5, debit: "40.04", fillQuantity: "0.4",
...
func portfolioRiskFillFixture(final bool) PortfolioRiskFillSettlementWrite {
	config := newPortfolioRiskFillConfig(final)
	execution := portfolioRiskExecutionFixture(config)
	return PortfolioRiskFillSettlementWrite{Execution: execution,
...
func portfolioRiskExecutionFixture(config portfolioRiskFillConfig) StrategyExecutionAtomicWrite {
	execution := strategyExecutionAtomicFixture()
	...
	if !config.final {
		...
		finalizePortfolioRiskExecutionFixture(&execution)
	}
	return execution
}

func finalizePortfolioRiskExecutionFixture(execution *StrategyExecutionAtomicWrite) {
...
func assertPortfolioRiskFillCounts(t *testing.T, ctx context.Context, pool *pgxpool.Pool, expected int) {
	t.Helper()
	for _, table := range []string{"order_events", "fills", "fill_journal_postings", "run_checkpoints", "outbox_events"} {
		...
		if err := pool.QueryRow(ctx, "SELECT count(*) FROM "+table).Scan(&count); err != nil || count != expected {
			t.Fatalf("portfolio and risk atomic fill count %s = %d %v", table, count, err)
		}
	}
	var journals int
	...
	if journals != expected {
		t.Fatalf("portfolio and risk fill journals = %d", journals)
	}
}

func assertPortfolioRiskPartialProjection(t *testing.T, ctx context.Context, pool *pgxpool.Pool) {
	t.Helper()
	...
	if fundsState != "active" || fundsRemaining != "60.060000000000000000" || liquidityState != "active" ||
		...
		t.Fatalf("partial portfolio and risk projection = %s/%s %s/%s %s/%s/%s", fundsState, fundsRemaining,
			liquidityState, liquidityRemaining, usdtReserved, btcAvailable, candidateState)
	}
}

func assertPortfolioRiskRolePermissions(t *testing.T, ctx context.Context, pool *pgxpool.Pool) {
	t.Helper()
	runtimeRole := testRole("AXIOM_PORTFOLIO_RISK_RUNTIME_ROLE", "axiom_runtime")
	recorderRole := testRole("AXIOM_PORTFOLIO_RISK_RECORDER_ROLE", "axiom_recorder")
	readOnlyRole := testRole("AXIOM_PORTFOLIO_RISK_READONLY_ROLE", "axiom_readonly")
	...
	for _, check := range checks {
		...
		if err := pool.QueryRow(ctx, "SELECT has_table_privilege($1,$2,$3)",
			...
			t.Fatalf("portfolio and risk privilege %#v = %t %v", check, allowed, err)
		}
	}
}

func seedPortfolioRiskReferences(t *testing.T, ctx context.Context, pool *pgxpool.Pool) {
	t.Helper()
	...
	statements := []struct {
		...
		{"INSERT INTO portfolios VALUES ('portfolio-a','Trend initial trend','USDT',$1)", []any{now}},
	...
	for index, statement := range statements {
		if _, err := pool.Exec(ctx, statement.sql, statement.args...); err != nil {
			t.Fatalf("portfolio and risk seed %d failed: %v", index+1, err)
		}
	}
}

func assertPortfolioRiskInitialization(t *testing.T, ctx context.Context, pool *pgxpool.Pool, repository *PortfolioRiskRepository) {
	t.Helper()
	...
	write := PortfolioRiskInitializationWrite{
		...
		Snapshot: generated.InsertPortfolioRiskAccountSnapshotParams{ID: "snapshot-initialization", AccountID: "account-a", Revision: 1,
...
type portfolioRiskWinner struct{ index int }

func assertPortfolioRiskContention(t *testing.T, ctx context.Context, pool *pgxpool.Pool, repository *PortfolioRiskRepository) portfolioRiskWinner {
	t.Helper()
	...
	for index := 0; index < 32; index++ {
		...
		go func(index int) {
			defer group.Done()
			if repository.Reserve(ctx, portfolioRiskAllocationFixture(index)) == nil {
				successes.Add(1)
	...
	group.Wait()
	if successes.Load() != 1 {
		t.Fatalf("portfolio and risk successful reservations = %d", successes.Load())
	}
	...
	if available != "399.900000000000000000" || reserved != "100.100000000000000000" ||
		...
		t.Fatalf("portfolio and risk exclusive projections = %s/%s/%s", available, reserved, liquidity)
	}
	return portfolioRiskWinner{index: int(winner.Load())}
}

func portfolioRiskAllocationFixture(index int) PortfolioRiskAllocationWrite {
	suffix := portfolioRiskDecimal(index)
	...
	return PortfolioRiskAllocationWrite{
...
func assertPortfolioRiskAtomicClose(t *testing.T, ctx context.Context, pool *pgxpool.Pool, repository *PortfolioRiskRepository, winner portfolioRiskWinner) {
	t.Helper()
	suffix := portfolioRiskDecimal(winner.index)
	now := pgTimestamp(time.Date(2026, 7, 16, 10, 3, 0, 0, time.UTC))
	write := PortfolioRiskAllocationClose{
	...
	if available != "500.000000000000000000" || reserved != "0.000000000000000000" ||
		...
		t.Fatalf("portfolio and risk released projections = %s/%s/%s", available, reserved, liquidity)
	}
}

func assertPortfolioRiskRiskReconciliationEvidence(t *testing.T, ctx context.Context, pool *pgxpool.Pool) {
	t.Helper()
	queries := generated.New(pool)
	now := pgTimestamp(time.Date(2026, 7, 16, 10, 4, 0, 0, time.UTC))
	hash := strings.Repeat("c", 64)
	assertPortfolioRiskRiskEvidence(t, ctx, pool, queries, now, hash)
	assertPortfolioRiskReconciliationEvidence(t, ctx, pool, queries, now, hash)
	for _, table := range []string{"risk_policies", "risk_policy_limits", "risk_evaluations", "risk_evaluation_policies",
		...
		if err := pool.QueryRow(ctx, "SELECT count(*) FROM "+table).Scan(&count); err != nil || count != 1 {
			t.Fatalf("portfolio and risk evidence table %s = %d %v", table, count, err)
		}
	}
}

func assertPortfolioRiskRiskEvidence(
	...
	_, err = queries.InsertPortfolioRiskRiskEvaluation(ctx, generated.InsertPortfolioRiskRiskEvaluationParams{ID: "risk-evaluation-a",
...
func assertPortfolioRiskReconciliationEvidence(
	t *testing.T, ctx context.Context, pool *pgxpool.Pool, queries *generated.Queries,
	now pgtype.Timestamptz, hash string,
) {
	t.Helper()
	...
	incident := "incident-a"
	_, err := queries.InsertPortfolioRiskReconciliationCase(ctx, generated.InsertPortfolioRiskReconciliationCaseParams{ID: "case-a",
...
func assertPortfolioRiskRecoveryEvidence(t *testing.T, ctx context.Context, pool *pgxpool.Pool) {
	t.Helper()
	...
	clock := now
	store, err := NewPortfolioRiskRecoveryEvidenceStore(ctx, pool, "recovery-a", func() time.Time {
	...
	if state != "ready_paused" || count != len(sequence) {
		t.Fatalf("portfolio and risk recovery = %s/%d", state, count)
	}
}
...
func portfolioRiskDecimal(value int) string {
	if value == 0 {
		return "zero"
	}
