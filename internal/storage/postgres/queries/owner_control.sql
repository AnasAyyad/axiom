...
-- name: InsertOwnerConsoleSession :one
...
-- name: InsertOwnerConsoleAuditEvent :one
...
-- name: ListOwnerConsoleOutboxAfter :many
SELECT * FROM outbox_events WHERE revision>$1 ORDER BY revision LIMIT $2;

-- name: OldestOwnerConsoleOutboxRevision :one
