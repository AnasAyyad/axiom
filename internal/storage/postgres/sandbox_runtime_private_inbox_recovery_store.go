...
const maximumSandboxRuntimeInboxRecoveryPage = 1000
...
func (store *SandboxRuntimeDispatcherStore) RecoverPrivateInbox(
	ctx context.Context,
	...
	if account == "" || epoch == 0 || fence == 0 {
		return 0, fmt.Errorf("sandbox_runtime_private_inbox_recovery_invalid")
	}
	...
	for index, record := range records {
		if err = store.reduceSandboxRuntimePrivateInbox(
...
func (store *SandboxRuntimeDispatcherStore) readPrivateInboxRecovery(
	ctx context.Context,
	account sandbox.AccountID,
	epoch uint64,
) ([]sandboxRuntimeInboxRecoveryRecord, error) {
...
LIMIT $3`,
		account,
		epoch,
		maximumSandboxRuntimeInboxRecoveryPage+1,
	)
	if err != nil {
		return nil, fmt.Errorf("sandbox_runtime_private_inbox_recovery_query_failed")
	}
	records := make([]sandboxRuntimeInboxRecoveryRecord, 0)
	for rows.Next() {
		record, scanErr := scanSandboxRuntimeInboxRecovery(rows, account, epoch)
		if scanErr != nil {
			rows.Close()
			return nil, scanErr
		}
	...
	if len(records) > maximumSandboxRuntimeInboxRecoveryPage {
		return nil, fmt.Errorf("sandbox_runtime_private_inbox_recovery_overflow")
	}
	return records, nil
}

type sandboxRuntimeInboxRecoveryScanner interface {
	Scan(...any) error
}

type sandboxRuntimeInboxRecoveryRecord struct {
	event   sandbox.PrivateEvent
	payload []byte
}

func scanSandboxRuntimeInboxRecovery(
	row sandboxRuntimeInboxRecoveryScanner,
	account sandbox.AccountID,
	epoch uint64,
) (sandboxRuntimeInboxRecoveryRecord, error) {
	...
	if err := row.Scan(
		...
		return sandboxRuntimeInboxRecoveryRecord{},
			fmt.Errorf("sandbox_runtime_private_inbox_recovery_scan_failed")
	}
	event := sandbox.PrivateEvent{
		Identity:        identity,
		AccountID:       account,
	...
	if err := decodeRecoveredPrivateEvent(&event, payload, orderID); err != nil {
		return sandboxRuntimeInboxRecoveryRecord{}, err
	}
	return sandboxRuntimeInboxRecoveryRecord{
...
func decodeRecoveredPrivateEvent(
	...
	if orderID != nil {
		if err := event.OrderID.UnmarshalText([]byte(*orderID)); err != nil {
			return fmt.Errorf("sandbox_runtime_private_inbox_recovery_invalid")
		}
	}
