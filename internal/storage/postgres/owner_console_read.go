...
// OwnerConsoleStore reads authoritative console projections and owns durable commands.
type OwnerConsoleStore struct {
	pool            *pgxpool.Pool
	cursor          console.CursorCodec
	clock           domain.Clock
	sandbox_runtime *SandboxRuntimeDispatcherStore
}

// NewOwnerConsoleStore constructs the PostgreSQL-backed owner console service boundary.
func NewOwnerConsoleStore(pool *pgxpool.Pool, cursorKey []byte, clock domain.Clock) (*OwnerConsoleStore, error) {
	if pool == nil || clock == nil {
		return nil, fmt.Errorf("owner_console_console_dependencies_missing")
	}
	codec, err := console.NewCursorCodec(cursorKey)
	if err != nil {
		return nil, err
	}
	sandbox_runtime, err := NewSandboxRuntimeDispatcherStore(pool)
	if err != nil {
		return nil, err
	}
	return &OwnerConsoleStore{
		pool: pool, cursor: codec, clock: clock, sandbox_runtime: sandbox_runtime,
	}, nil
}

// SystemStatus returns current durable risk, shadow, and incident state.
func (store *OwnerConsoleStore) SystemStatus(ctx context.Context) (generated.SystemStatus, error) {
	riskState := store.ownerConsoleSystemRiskState(ctx)
	activeID, shadowState := store.ownerConsoleSystemShadowState(ctx)
	var incidents int
	...
	strategy := store.ownerConsoleSystemStrategyActivation(ctx)
	...
	lifecycle := ownerConsoleSystemLifecycle(shadowState, riskState, binanceState)
	...
	readiness := ownerConsoleSystemReadiness(lifecycle)
...
func (store *OwnerConsoleStore) ownerConsoleSystemRiskState(ctx context.Context) string {
	var state string
	_ = store.pool.QueryRow(ctx, `SELECT next_state FROM risk_state_events ORDER BY entity_revision DESC LIMIT 1`).Scan(&state)
	if state == "" {
		return "PAUSED"
	}
	if state == "NORMAL" || state == "CAUTIOUS" {
		return "RESUMED"
	}
	return state
}

func (store *OwnerConsoleStore) ownerConsoleSystemShadowState(ctx context.Context) (*string, string) {
	var id *string
	var state string
	_ = store.pool.QueryRow(ctx, `SELECT id,state FROM shadow_sessions WHERE state IN ('QUEUED','RUNNING','PAUSED','CANCEL_REQUESTED') ORDER BY created_at DESC,id DESC LIMIT 1`).Scan(&id, &state)
	return id, state
}

func (store *OwnerConsoleStore) ownerConsoleSystemStrategyActivation(ctx context.Context) generated.SystemStatusStrategyActivation {
	var count int
	_ = store.pool.QueryRow(ctx, `SELECT count(*) FROM strategy_versions WHERE id='trend-following-1-0-0'`).Scan(&count)
	if count == 1 {
		return generated.SystemStatusStrategyActivation("trend-following@1.0.0")
	}
	return generated.SystemStatusStrategyActivation("unavailable")
}

func ownerConsoleSystemLifecycle(shadowState, riskState, binanceState string) generated.SystemStatusLifecycleState {
	if shadowState == "RUNNING" && riskState == "RESUMED" && binanceState == "healthy" {
		return generated.SystemStatusLifecycleState("RUNNING")
	}
	if binanceState == "stale" || binanceState == "unavailable" {
		return generated.SystemStatusLifecycleState("DEGRADED")
	}
	return generated.SystemStatusLifecycleState("READY_PAUSED")
}

func ownerConsoleSystemReadiness(lifecycle generated.SystemStatusLifecycleState) generated.SystemStatusReadinessState {
	if lifecycle == generated.SystemStatusLifecycleState("DEGRADED") {
		return generated.SystemStatusReadinessState("degraded")
	}
	if lifecycle == generated.SystemStatusLifecycleState("READY_PAUSED") ||
		lifecycle == generated.SystemStatusLifecycleState("RUNNING") {
		return generated.SystemStatusReadinessState("ready")
	}
	return generated.SystemStatusReadinessState("blocked")
}

// BinanceHealth derives a fail-closed public-only view from durable recorder evidence.
func (store *OwnerConsoleStore) BinanceHealth(ctx context.Context) (generated.BinanceHealth, error) {
	now := store.clock.Now().UTC
	var ended time.Time
	var revision int64
...
func (store *OwnerConsoleStore) Risk(ctx context.Context) (generated.RiskStatus, error) {
...
func (store *OwnerConsoleStore) Trend(ctx context.Context) (generated.TrendStatus, error) {
