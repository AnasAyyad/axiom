...
generated_at,revision,coalesce(content,'{}'::jsonb) FROM v1d_reports WHERE id=$1`, id).Scan(&reportType, &state,
...
func d4ExportIncidentEvidence(
	ctx context.Context, tx pgx.Tx, id string, record map[string]string,
) error {
	var alerts, activity, holds, events int64
	if err := tx.QueryRow(ctx, `SELECT
(SELECT count(*) FROM v1d_incident_alert_links WHERE incident_id=$1),
(SELECT count(*) FROM v1d_incident_activity_links WHERE incident_id=$1),
(SELECT count(*) FROM v1d_artifact_holds WHERE reference_id=$1 AND hold_type='incident' AND released_at IS NULL),
(SELECT count(*) FROM v1d_incident_events WHERE incident_id=$1)`, id).Scan(
	...
	if err := tx.QueryRow(ctx, `SELECT
coalesce((SELECT event_hash::text FROM v1d_incident_events WHERE incident_id=$1
  ORDER BY incident_revision DESC LIMIT 1),''),
coalesce((SELECT evidence_hash::text FROM v1d_incident_resolution_evidence
...
FROM v1d_incident_replay_inputs WHERE incident_id=$1`, id).Scan(&dataset, &first, &last, &source)
