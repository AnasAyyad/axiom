...
func (store *V1CDispatcherStore) ReadCanaryOrderStatus(
	ctx context.Context,
	...
	if canaryID == "" ||
		...
		return sandbox.CanaryOrderStatus{}, fmt.Errorf("v1c_canary_status_invalid")
	}
...
JOIN v1c_submission_outbox outbox ON outbox.plan_id=plan.id
...
WHERE plan.id=$1 AND plan.intent_kind='CANARY' AND plan.leg_count=1
  AND account.exchange=$2`,
	...
	if err != nil || epoch <= 0 || attempt < 0 {
		return sandbox.CanaryOrderStatus{}, fmt.Errorf("v1c_canary_status_unavailable")
	}
...
func (store *V1CDispatcherStore) ReadEngineCommandResult(
	ctx context.Context,
	id string,
) (sandbox.EngineCommandState, string, error) {
	if id == "" {
		return "", "", fmt.Errorf("v1c_engine_command_result_invalid")
	}
	var state sandbox.EngineCommandState
	var evidence *string
	if err := store.pool.QueryRow(ctx, `
SELECT state,evidence_hash
FROM v1c_engine_commands
WHERE id=$1`,
		id,
	).Scan(&state, &evidence); err != nil {
		return "", "", fmt.Errorf("v1c_engine_command_result_unavailable")
	}
	if evidence == nil {
		return state, "", nil
	}
...
func (store *V1CDispatcherStore) ReadCanaryEvidence(
	ctx context.Context,
	exchange sandbox.Exchange,
	canaryID string,
	...
	if canaryID == "" ||
		(exchange != sandbox.ExchangeBinance &&
			exchange != sandbox.ExchangeBybit) {
		return nil, fmt.Errorf("v1c_canary_evidence_read_invalid")
	}
...
ORDER BY observed_at,stage`,
	...
	if err != nil {
		return nil, fmt.Errorf("v1c_canary_evidence_read_failed")
	}
	defer rows.Close()
...
func scanCanaryEvidenceRecord(
	...
	if err != nil || epoch <= 0 || cycle <= 0 {
		return sandbox.CanaryEvidenceRecord{},
			fmt.Errorf("v1c_canary_evidence_read_failed")
	}
...
func (store *V1CDispatcherStore) ReadyCanaryRestartCycle(
	ctx context.Context,
	...
	if account == "" || epoch == 0 || priorCycle == 0 ||
		...
		return 0, fmt.Errorf("v1c_canary_restart_invalid")
	}
	var cycle int64
	err := store.pool.QueryRow(ctx, `
SELECT observation.startup_cycle
FROM v1c_engine_observations observation
JOIN v1c_exchange_accounts account ON account.id=observation.account_id
...
WHERE account.id=$1
  ...
  AND EXISTS(
    SELECT 1 FROM v1c_engine_startup_evidence evidence
	...
	if err != nil || cycle <= 0 {
		return 0, fmt.Errorf("v1c_canary_restart_not_ready")
	}
...
func (store *V1CDispatcherStore) CountCanaryCreateEvidence(
	ctx context.Context,
	exchange sandbox.Exchange,
	...
	if (exchange != sandbox.ExchangeBinance &&
		...
		return 0, fmt.Errorf("v1c_canary_create_count_invalid")
	}
	path := "/api/v3/order"
	...
	if err := store.pool.QueryRow(ctx, countCanaryCreateEvidenceSQL,
		...
		return 0, fmt.Errorf("v1c_canary_create_count_failed")
	}
	return count, nil
}
