...
// D4Report returns one report with immutable provenance and hash identity.
func (store *A11ConsoleStore) D4Report(ctx context.Context, id string) (generated.ReportResource, error) {
...
FROM v1d_reports report WHERE report.id=$1`, id).Scan(
...
// D4ReportSchedules returns deterministic bounded schedule projections.
func (store *A11ConsoleStore) D4ReportSchedules(
	ctx context.Context, query console.D1ListQuery,
	...
	position, err := store.cursor.Decode("v1d-d4:report-schedules", query.Cursor)
	if err != nil {
		return generated.ReportSchedulePage{}, err
	}
	items, err := store.d4ScheduleRows(ctx, position, query)
	if err != nil {
		return generated.ReportSchedulePage{}, err
	}
	...
	if len(items) > query.PageSize {
		...
		next := store.cursor.Encode("v1d-d4:report-schedules", page.Items[len(page.Items)-1].Id)
...
func (store *A11ConsoleStore) d4ScheduleRows(
	ctx context.Context, position string, query console.D1ListQuery,
...
FROM v1d_report_schedules WHERE ($1='' OR id<$1) AND ($2='' OR state=$2)
...
// D4Alert returns sanitized delivery and escalation evidence.
func (store *A11ConsoleStore) D4Alert(ctx context.Context, id string) (generated.AlertDetail, error) {
...
occurrences,revision,correlation_id,incident_id,created_at,last_seen_at FROM alerts WHERE id=$1`, id).Scan(
	...
	item.Revision = strconv.FormatInt(revision, 10)
	if item.Deliveries, err = store.d4AlertAttempts(ctx, id); err != nil {
		return generated.AlertDetail{}, err
	}
	item.Escalations, err = store.d4AlertEscalations(ctx, id)
	return item, err
}

func (store *A11ConsoleStore) d4AlertAttempts(ctx context.Context, id string) ([]generated.AlertDeliveryAttempt, error) {
	rows, err := store.pool.Query(ctx, `SELECT id,sink_name,attempt,state,reason_code,
started_at,completed_at,latency_ms FROM v1d_alert_delivery_attempts
...
func (store *A11ConsoleStore) d4AlertEscalations(ctx context.Context, id string) ([]generated.AlertEscalation, error) {
	rows, err := store.pool.Query(ctx, `SELECT id,revision,actor_user_id,reason,escalated_at
FROM v1d_alert_escalations WHERE alert_id=$1 ORDER BY revision`, id)
	if err != nil {
		return nil, err
	}
	defer rows.Close()
...
// D4AlertRoutes exposes labels and state only; endpoint URLs and credentials
// remain runtime configuration and are never selected here.
func (store *A11ConsoleStore) D4AlertRoutes(ctx context.Context) (generated.AlertRoutePage, error) {
...
FROM v1d_alert_routes route LEFT JOIN LATERAL (
  SELECT state,requested_at FROM v1d_alert_route_tests WHERE route_id=route.id
...
var _ console.D4ReadService = (*A11ConsoleStore)(nil)
