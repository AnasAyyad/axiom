...
model_provenance::text,maturity,source_identity,source_revision FROM v1d_reports
WHERE id=$1`, reportID).Scan(&mode, &confidence, &valuation, &models, &maturity,
	...
	content := map[string]any{
		"schema_version": "axiom.report.v1d.d4", "report_id": reportID,
...
func d4ReportCounts(ctx context.Context, tx pgx.Tx, reportType string) (map[string]int64, error) {
	queries := d4CountQueries(reportType)
...
func d4CountQueries(reportType string) map[string]string {
	switch reportType {
	...
	case "sandbox_qualifications":
		return map[string]string{"qualification_runs": `SELECT count(*) FROM v1d_qualification_runs`, "sandbox_submission_plans": `SELECT count(*) FROM v1c_submission_plans`}
	default:
		return map[string]string{"open_incidents": `SELECT count(*) FROM incidents WHERE state<>'resolved'`,
			"failed_qualifications": `SELECT count(*) FROM v1d_qualification_runs WHERE state='FAILED'`,
