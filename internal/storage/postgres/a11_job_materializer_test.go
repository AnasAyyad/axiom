...
func TestA11OfflineRequestRequiresCompleteWindowAndSeed(t *testing.T) {
	valid := []byte(`{"configuration_id":"configuration-a","dataset_id":"dataset-a","root_seed_hash":"` +
...
func TestA11RunManifestRequiresCleanEmbeddedBuildIdentity(t *testing.T) {
	...
	request := a11OfflineRequest{ConfigurationID: "configuration-a", DatasetID: "dataset-a",
		RootSeedHash: hash, StrategyVersion: "trend.v1a.1"}
	...
	if _, err = a11RunManifest("backtest-b", "backtest", request, config.DefaultConfiguration(), strings.Repeat("6", 64), dataset, namespace); err == nil {
