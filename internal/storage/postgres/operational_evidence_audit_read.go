...
var operationalEvidenceSHA256 = regexp.MustCompile(`^[0-9a-f]{64}$`)

type operationalEvidenceAuditLink struct {
	sequence          int64
	previous, stored  string
	authoritativeHash string
}

// OperationalEvidenceAuditVerification explicitly reports whether the immutable sidecar remains
// linked to every authoritative general-audit event.
func (store *OwnerConsoleStore) OperationalEvidenceAuditVerification(ctx context.Context) (generated.AuditVerification, error) {
...
FROM owner_console_audit_chain chain JOIN audit_events audit ON audit.id=chain.audit_event_id
ORDER BY chain.chain_sequence`)
	if err != nil {
		return generated.AuditVerification{}, err
	}
	defer rows.Close()
	links := []operationalEvidenceAuditLink{}
	for rows.Next() {
		var link operationalEvidenceAuditLink
	...
	verdict := verifyOperationalEvidenceAuditLinks(links)
	verdict.VerifiedAt = store.clock.Now().UTC
	return verdict, nil
}

func verifyOperationalEvidenceAuditLinks(links []operationalEvidenceAuditLink) generated.AuditVerification {
	...
	for index, link := range links {
		if link.sequence <= priorSequence || link.previous != prior || link.stored != link.authoritativeHash ||
			!operationalEvidenceSHA256.MatchString(link.stored) {
