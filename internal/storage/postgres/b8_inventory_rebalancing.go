...
const b8InventorySQL = `WITH positions AS (
...
const b8RebalancingSQL = `SELECT recommendation.id,recommendation.method,
...
const b8RebalancingRouteSQL = `SELECT step.step_index,fact.fact_id,fact.fact_version,
...
func (store *A11ConsoleStore) Inventory(
	ctx context.Context, cursor string, limit int, filters console.InventoryFilters,
) (generated.InventoryPage, error) {
	position, err := decodeB8StringCursor(store.cursor, "b8-inventory", cursor)
	if err != nil {
		return generated.InventoryPage{}, err
	}
	rows, err := store.pool.Query(ctx, b8InventorySQL,
	...
	for rows.Next() {
		...
		item.Virtual = true
		item.Quality = b8Quality("cross_exchange_inventory_snapshots", observed,
			generated.QualityEvidenceConfidenceHigh, b8Freshness(now, observed))
	...
	if len(items) > limit {
		...
		items = items[:limit]
		page.Items = items
		next := encodeB8StringCursor(store.cursor, "b8-inventory", items[len(items)-1].Id)
...
func (store *A11ConsoleStore) Rebalancing(
	...
	var err error
	if cursor != "" {
		cursorTime, cursorID, err = decodeB8TimeCursor(store.cursor, "b8-rebalancing", cursor)
		if err != nil {
			return generated.RebalancingPage{}, err
		}
	}
	rows, err := store.pool.Query(ctx, b8RebalancingSQL,
		nullableB8Time(cursorTime), cursorID, limit+1)
	if err != nil {
		return generated.RebalancingPage{}, err
	}
	defer rows.Close()
	items, err := scanB8RebalancingRows(rows, store.clock.Now().UTC, limit+1)
	if err != nil {
		return generated.RebalancingPage{}, err
	}
	snapshot, err := b8SnapshotRevision(ctx, store.pool)
	...
	if len(items) > limit {
		...
		next := encodeB8TimeCursor(store.cursor, "b8-rebalancing", last.RecordedAt, last.Id)
		page.NextCursor = &next
	}
	return page, nil
}

func scanB8RebalancingRows(rows pgx.Rows, now time.Time, capacity int) (
	...
	for rows.Next() {
		...
		item.Quality = b8RebalancingQuality(now, observed, expires, confidence)
		items = append(items, item)
	}
...
func (store *A11ConsoleStore) RebalancingDetail(
	ctx context.Context, id string,
) (generated.RebalancingDetail, error) {
	page, err := store.b8RebalancingByID(ctx, id)
	if err != nil {
		return generated.RebalancingDetail{}, err
	}
	rows, err := store.pool.Query(ctx, b8RebalancingRouteSQL, id)
	if err != nil {
		return generated.RebalancingDetail{}, err
	}
...
func (store *A11ConsoleStore) b8RebalancingByID(
	ctx context.Context, id string,
) (generated.RebalancingSummary, error) {
	var item generated.RebalancingSummary
	var advisory bool
	...
	item.AdvisoryOnly = generated.RebalancingSummaryAdvisoryOnly(advisory)
	item.Revision = strconv.FormatInt(item.RecordedAt.UnixNano(), 10)
	item.Quality = b8RebalancingQuality(store.clock.Now().UTC, observed, expires, confidence)
	return item, nil
}

func b8RebalancingQuality(now, observed, expires time.Time, confidence string) generated.QualityEvidence {
	label := generated.QualityEvidenceConfidence(confidence)
	freshness := b8Freshness(now, observed)
	if !expires.After(now) {
		freshness = generated.QualityEvidenceFreshnessExpired
	}
	result := b8Quality("rebalancing_route_facts", observed, label, freshness)
	expires = expires.UTC()
