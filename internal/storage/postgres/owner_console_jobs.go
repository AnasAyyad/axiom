...
func (store *OwnerConsoleStore) Job(ctx context.Context, id, eventOrdinal string) (generated.JobResource, error) {
	...
	request, requestErr := decodeOwnerConsoleOfflineRequest(kind, requestPayload)
	if requestErr != nil {
	...
	if err = populateRunLabJobEvidence(ctx, store, &item, request, payloadHash, runID); err != nil {
		return generated.JobResource{}, err
	}
	return item, nil
}

func (store *OwnerConsoleStore) populateJobMode(ctx context.Context, item *generated.JobResource, kind, runID, eventOrdinal string) error {
	if kind == "backtest" {
		if eventOrdinal != "" {
	...
	checkpoints, err := store.runLabReplayCheckpoints(ctx, runID)
	if err != nil {
		return err
	}
...
func (store *OwnerConsoleStore) registeredReport(ctx context.Context, generationID string) (*generated.RegisteredResearchReport, error) {
	...
	if err != nil || len(canonical) < 2 || len(canonical) > 1_048_576 ||
		...
		return nil, fmt.Errorf("owner_console_registered_report_invalid")
	}
	...
	item.Benchmarks, err = ownerConsoleResearchSlices(manifest.Benchmarks)
	if err != nil {
		return nil, err
	}
	item.Stress, err = ownerConsoleResearchSlices(manifest.Stress)
	if err != nil {
		return nil, err
	}
...
func ownerConsoleResearchSlices(source []research.ResultSlice) ([]generated.ResearchResultSlice, error) {
...
func (store *OwnerConsoleStore) replayInspection(ctx context.Context, runID, requested string) (*generated.ReplayEventInspection, error) {
	if runID == "" {
		if requested != "" {
	...
	selected, err := ownerConsoleReplayOrdinal(newest, requested)
	if err != nil {
		return nil, err
	}
...
func ownerConsoleReplayOrdinal(newest int64, requested string) (int64, error) {
	if requested == "" {
		return newest, nil
	}
...
func (store *OwnerConsoleStore) loadReplayInspection(ctx context.Context, runID string, count, selected int64) (*generated.ReplayEventInspection, error) {
	...
	for rows.Next() {
		...
		if err = rows.Scan(&kind, &hash, &payload); err != nil || !json.Valid([]byte(payload)) {
			return nil, fmt.Errorf("owner_console_replay_inspection_invalid")
		}
		switch kind {
		case "event":
	...
	if found != 5 || !ownerConsoleReplayInspectionComplete(inspection) {
		return nil, console.ErrNotFound
	}
	return &inspection, nil
}

func ownerConsoleReplayInspectionComplete(inspection generated.ReplayEventInspection) bool {
