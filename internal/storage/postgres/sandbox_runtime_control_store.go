...
func (store *SandboxRuntimeDispatcherStore) CreateSandboxArm(
	ctx context.Context,
	...
	command.Arm = normalizeSandboxRuntimeArmForPersistence(command.Arm)
	tx, err := store.pool.BeginTx(ctx, pgx.TxOptions{IsoLevel: pgx.Serializable})
	if err != nil {
		return sandbox.Arm{}, fmt.Errorf("sandbox_runtime_arm_begin_failed")
	}
	defer func() { _ = tx.Rollback(context.Background()) }()
	state, revision, err := lockSandboxRuntimeArmSession(ctx, tx, command)
	if err != nil {
		return sandbox.Arm{}, err
	}
	accounts, err := lockSandboxRuntimeArmAccounts(ctx, tx, command.Arm.SessionID)
	if err != nil || !sameSandboxRuntimeAccounts(accounts, command.Arm.AccountIDs) {
		return sandbox.Arm{}, fmt.Errorf("sandbox_runtime_arm_accounts_rejected")
	}
	beforeHash := sandboxRuntimeControlStateHash(
		string(command.Arm.SessionID),
		state,
		accounts,
		uint64(revision),
	)
	if err = insertSandboxRuntimeArm(ctx, tx, command); err != nil {
		return sandbox.Arm{}, err
	}
	if err = setSandboxRuntimeArmedState(ctx, tx, command, accounts); err != nil {
		return sandbox.Arm{}, err
	}
	afterHash := sandboxRuntimeControlStateHash(
		string(command.Arm.SessionID), "ARMED", accounts, uint64(revision+1),
	)
	if err = appendSandboxRuntimeArmAudit(
		ctx, tx, command, revision+1, beforeHash, afterHash,
	); err != nil {
		return sandbox.Arm{}, err
	}
	if err = tx.Commit(ctx); err != nil {
		return sandbox.Arm{}, fmt.Errorf("sandbox_runtime_arm_commit_failed")
	}
	return command.Arm, nil
}

func normalizeSandboxRuntimeArmForPersistence(arm sandbox.Arm) sandbox.Arm {
	arm.CreatedAt = arm.CreatedAt.UTC().Truncate(time.Microsecond)
	arm.ExpiresAt = arm.ExpiresAt.UTC().Truncate(time.Microsecond)
	return arm
}

func lockSandboxRuntimeArmSession(
	ctx context.Context,
	tx pgx.Tx,
	command sandbox.ArmCommand,
) (string, int64, error) {
	...
	var revision int64
	err := tx.QueryRow(ctx, `
SELECT state,revision,created_by FROM sandbox_runtime_sandbox_sessions
WHERE id=$1 AND revision=$2 FOR UPDATE`,
	...
	if err != nil || (state != "READY_PAUSED" && state != "PAUSED") ||
		...
		return "", 0, fmt.Errorf("sandbox_runtime_arm_session_rejected")
	}
	return state, revision, nil
}

func insertSandboxRuntimeArm(
	ctx context.Context,
	tx pgx.Tx,
	command sandbox.ArmCommand,
) error {
...
INSERT INTO sandbox_runtime_sandbox_arms(
 ...
 source_hash,reason_hash,created_at,expires_at,revision
		...
		return fmt.Errorf("sandbox_runtime_arm_insert_failed")
	}
	return nil
}

func setSandboxRuntimeArmedState(
	ctx context.Context,
	tx pgx.Tx,
	command sandbox.ArmCommand,
...
WHERE id=$1`, command.Arm.SessionID, command.Arm.CreatedAt); err != nil {
		return fmt.Errorf("sandbox_runtime_arm_session_update_failed")
	}
...
WHERE id=ANY($1) AND state='READY_PAUSED'`,
	...
	if err != nil || tag.RowsAffected() != int64(len(accounts)) {
		return fmt.Errorf("sandbox_runtime_arm_account_update_failed")
	}
	return nil
}

func appendSandboxRuntimeArmAudit(
	ctx context.Context,
	tx pgx.Tx,
	command sandbox.ArmCommand,
	revision int64,
...
func lockSandboxRuntimeArmAccounts(
	ctx context.Context,
	tx pgx.Tx,
...
func sameSandboxRuntimeAccounts(left, right []sandbox.AccountID) bool {
...
// reconciliation.
func (store *SandboxRuntimeDispatcherStore) RiskUnlock(
	ctx context.Context,
	...
	tx, err := store.pool.BeginTx(ctx, pgx.TxOptions{IsoLevel: pgx.Serializable})
	if err != nil {
		return fmt.Errorf("sandbox_runtime_risk_unlock_begin_failed")
	}
	defer func() { _ = tx.Rollback(context.Background()) }()
	state, revision, err := lockSandboxRuntimeRiskUnlockAccount(ctx, tx, command)
	if err != nil {
		return err
	}
	if err = requireSandboxRuntimeCleanReconciliation(ctx, tx, command); err != nil {
		return err
	}
	if err = insertSandboxRuntimeRiskUnlock(ctx, tx, command, state); err != nil {
		return err
	}
	if _, err = tx.Exec(ctx, `
UPDATE sandbox_runtime_exchange_accounts
...
WHERE id=$1`, command.AccountID, command.Now); err != nil {
		return fmt.Errorf("sandbox_runtime_risk_unlock_update_failed")
	}
	if err = appendSandboxRuntimeRiskUnlockAudit(ctx, tx, command, state, revision); err != nil {
		return err
	}
	if err = tx.Commit(ctx); err != nil {
		return fmt.Errorf("sandbox_runtime_risk_unlock_commit_failed")
	}
	return nil
}

func lockSandboxRuntimeRiskUnlockAccount(
	ctx context.Context,
	tx pgx.Tx,
	command sandbox.RiskUnlockCommand,
) (string, int64, error) {
	...
	err := tx.QueryRow(ctx, `
SELECT state,revision,current_epoch FROM sandbox_runtime_exchange_accounts
WHERE id=$1 AND revision=$2 FOR UPDATE`,
	...
	if err != nil || (state != "LOCKED" && state != "QUARANTINED") ||
		...
		return "", 0, fmt.Errorf("sandbox_runtime_risk_unlock_account_rejected")
	}
	return state, revision, nil
}

func requireSandboxRuntimeCleanReconciliation(
	ctx context.Context,
	tx pgx.Tx,
	command sandbox.RiskUnlockCommand,
) error {
	var clean bool
	err := tx.QueryRow(ctx, `
SELECT state='clean' FROM sandbox_runtime_reconciliations
WHERE id=$1 AND account_id=$2 AND account_epoch=$3 FOR SHARE`,
	...
	if err != nil || !clean {
		return fmt.Errorf("sandbox_runtime_risk_unlock_reconciliation_rejected")
	}
	return nil
}

func insertSandboxRuntimeRiskUnlock(
	ctx context.Context,
	tx pgx.Tx,
	command sandbox.RiskUnlockCommand,
	priorState string,
...
INSERT INTO sandbox_runtime_risk_unlocks(
 ...
 unlocked_at,revision
		...
		return fmt.Errorf("sandbox_runtime_risk_unlock_insert_failed")
	}
	return nil
}

func appendSandboxRuntimeRiskUnlockAudit(
	ctx context.Context,
	tx pgx.Tx,
	command sandbox.RiskUnlockCommand,
	priorState string,
...
func sandboxRuntimeControlStateHash(
...
func stableSandboxRuntimeHash(values ...string) string {
	digest := sha256.Sum256([]byte(strings.Join(values, "\x00")))
	return hex.EncodeToString(digest[:])
}

var _ sandbox.SandboxControlStore = (*SandboxRuntimeDispatcherStore)(nil)
