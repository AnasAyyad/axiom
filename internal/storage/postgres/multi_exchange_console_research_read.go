...
const multiExchangeConsoleChampionChallengerSQL = `SELECT report.id,report.champion_strategy_version_id,
...
// ChampionChallenger returns immutable research promotion comparison reports.
func (store *OwnerConsoleStore) ChampionChallenger(
	...
	var err error
	if cursor != "" {
		cursorTime, cursorID, err = decodeMultiExchangeConsoleTimeCursor(store.cursor, "multi_exchange_console-champion-challenger", cursor)
		if err != nil {
			return generated.ChampionChallengerPage{}, err
		}
	}
	rows, err := store.pool.Query(ctx, multiExchangeConsoleChampionChallengerSQL,
	...
	for rows.Next() {
		item, scanErr := scanMultiExchangeConsoleChampion(rows)
		if scanErr != nil {
	...
	if len(items) > limit {
		...
		next := encodeMultiExchangeConsoleTimeCursor(store.cursor, "multi_exchange_console-champion-challenger", last.CreatedAt, last.Id)
		page.NextCursor = &next
	}
	return page, nil
}

func scanMultiExchangeConsoleChampion(row multiExchangeConsoleRowScanner) (generated.ChampionChallengerReport, error) {
...
func (store *OwnerConsoleStore) ReplayFaults(
	...
	FROM multi_exchange_console_replay_fault_schedules WHERE replay_id=$1 ORDER BY schedule_revision`, replayID)
	if err != nil {
		return generated.ReplayFaultPage{}, err
	}
	defer rows.Close()
	...
	err = store.pool.QueryRow(ctx, `SELECT revision FROM multi_exchange_console_replay_fault_schedule_states
