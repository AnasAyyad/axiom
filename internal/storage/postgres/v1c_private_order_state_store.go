...
func advanceV1COutbox(
	...
	tag, err := tx.Exec(ctx, `
UPDATE v1c_submission_outbox SET state=$3,order_state=$4,updated_at=$5
...
ON CONFLICT DO NOTHING`,
	...
	if err != nil {
		return fmt.Errorf("v1c_fill_post_failed")
	}
	if tag.RowsAffected() == 0 && !sameV1CFill(ctx, tx, event, payload) {
		return fmt.Errorf("v1c_fill_identity_conflict")
	}
...
WHERE order_id=$1 AND state='ACTIVE'`,
		...
		return fmt.Errorf("v1c_reservation_release_failed")
	}
	return kill.Hit(ctx, sandbox.KillAfterReservationRelease)
}

func updateV1CPlanState(
	ctx context.Context,
	tx pgx.Tx,
	planID string,
) error {
	legs, terminal, filled, recovery, err := readV1CPlanState(ctx, tx, planID)
	if err != nil || legs == 0 {
		return fmt.Errorf("v1c_plan_state_read_failed")
	}
	next, disposition := deriveV1CPlanState(legs, terminal, filled, recovery)
	if _, err := tx.Exec(ctx, `
UPDATE v1c_submission_plans
...
WHERE id=$1 AND (state<>$2 OR coalesce(final_disposition,'')<>$3)`,
		...
		return fmt.Errorf("v1c_plan_state_update_failed")
	}
	return nil
}

func readV1CPlanState(
	ctx context.Context,
	tx pgx.Tx,
	planID string,
...
SELECT count(*),
       ...
       coalesce(bool_or(state='UNKNOWN' OR order_state='RECOVERY_REQUIRED'),false)
...
func v1cOrderState(state execution.OrderState) (string, bool) {
	switch state {
