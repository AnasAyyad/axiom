...
const b8ExchangesSQL = `SELECT exchange.id,exchange.name,
...
const b8StrategiesSQL = `SELECT version.id,definition.family,definition.name,
  ...
  coalesce((SELECT role FROM (
    SELECT 'champion'::text role,report.created_at FROM b7_champion_challenger_reports report
      WHERE report.champion_strategy_version_id=version.id
    UNION ALL
    SELECT 'challenger'::text role,report.created_at FROM b7_champion_challenger_reports report
...
LEFT JOIN LATERAL (
  ...
  FROM b7_validation_suites WHERE strategy_version_id=version.id
...
// the retained Binance-specific A11 alias.
func (store *A11ConsoleStore) Exchanges(
	ctx context.Context,
	cursor string,
	limit int,
) (generated.ExchangePage, error) {
	position, err := decodeB8StringCursor(store.cursor, "b8-exchanges", cursor)
	if err != nil {
		return generated.ExchangePage{}, err
	}
	rows, err := store.pool.Query(ctx, b8ExchangesSQL, position, limit+1)
	if err != nil {
		return generated.ExchangePage{}, err
	}
	defer rows.Close()
	...
	for rows.Next() {
		...
		items = append(items, b8ExchangeSummary(
	...
	if len(items) > limit {
		...
		items = items[:limit]
		page.Items = items
		next := encodeB8StringCursor(store.cursor, "b8-exchanges", items[len(items)-1].Id)
		page.NextCursor = &next
	}
	return page, nil
}

func b8ExchangeSummary(id, name string, observed, now time.Time, revision int64,
	...
	quality := b8Quality("market_data_segments", observed, confidence, b8Freshness(now, observed))
...
func (store *A11ConsoleStore) Strategies(
	ctx context.Context,
	cursor string,
	limit int,
) (generated.StrategyPage, error) {
	position, err := decodeB8StringCursor(store.cursor, "b8-strategies", cursor)
	if err != nil {
		return generated.StrategyPage{}, err
	}
	rows, err := store.pool.Query(ctx, b8StrategiesSQL, position, limit+1)
	if err != nil {
		return generated.StrategyPage{}, err
	}
	defer rows.Close()
	...
	for rows.Next() {
		item, scanErr := scanB8Strategy(rows)
		if scanErr != nil {
	...
	if len(items) > limit {
		...
		items = items[:limit]
		page.Items = items
		next := encodeB8StringCursor(store.cursor, "b8-strategies", items[len(items)-1].Id)
		page.NextCursor = &next
	}
	return page, nil
}

type b8RowScanner interface{ Scan(...any) error }

func scanB8Strategy(row b8RowScanner) (generated.StrategySummary, error) {
	...
	item.Maturity = generated.StrategySummaryMaturity(normalizeB8Maturity(maturity))
	item.EvidenceRole = generated.StrategySummaryEvidenceRole(role)
	item.Confidence = generated.StrategySummaryConfidence(normalizeB8Confidence(confidence))
	item.Viability = generated.StrategySummaryViability(normalizeB8Viability(viability))
...
func normalizeB8Maturity(value string) string {
	switch value {
...
func normalizeB8Confidence(value string) string {
	switch value {
...
func normalizeB8Viability(value string) string {
	switch value {
