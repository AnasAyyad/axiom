...
func applyOwnerControlAlert(
	...
	if command.Action == "escalate" {
		return applyOperationalEvidenceAlertEscalation(ctx, tx, principal, command, now)
	}
...
func applyOwnerControlReport(
	...
	return queueOperationalEvidenceReport(ctx, tx, principal, command, commandID, now, nil, nil)
}

func applyOwnerControlExportDelete(
	ctx context.Context,
	tx pgx.Tx,
	principal authentication.Principal,
	command console.OwnerControlCommand,
	...
	var holds int
	err := tx.QueryRow(ctx, `SELECT deleted_at FROM owner_console_export_artifacts
	...
	if err = tx.QueryRow(ctx, `SELECT count(*)::integer FROM owner_console_artifact_holds
	...
	if _, err = tx.Exec(ctx, `
UPDATE owner_console_export_artifacts SET content=NULL,deleted_at=$2,deletion_reason=$3 WHERE id=$1`,
...
func applyOwnerControlArtifactHold(
	ctx context.Context,
	tx pgx.Tx,
	principal authentication.Principal,
	command console.OwnerControlCommand,
	...
	if err := tx.QueryRow(ctx, `SELECT EXISTS(
      SELECT 1 FROM owner_console_export_artifacts WHERE id=$1 AND deleted_at IS NULL
    )`, command.TargetID).Scan(&available); err != nil || !available {
		return nil, console.ErrPrecondition
	}
	holdID, _ := ownerConsoleIdentifier("artifact-hold")
	if _, err := tx.Exec(ctx, `
INSERT INTO owner_console_artifact_holds(
...
func applyOwnerControlIncident(
	ctx context.Context,
	tx pgx.Tx,
	principal authentication.Principal,
	command console.OwnerControlCommand,
	...
	return applyOperationalEvidenceIncidentTransition(ctx, tx, principal, command, now)
}

func applyOwnerControlConfigurationActivation(
	ctx context.Context,
	tx pgx.Tx,
	principal authentication.Principal,
	command console.OwnerControlCommand,
...
func applyOwnerControlLabControl(
	ctx context.Context,
	tx pgx.Tx,
	principal authentication.Principal,
	command console.OwnerControlCommand,
...
SELECT state,job_type,progress_revision FROM jobs WHERE id=$1 FOR UPDATE`, command.TargetID).Scan(
	...
	next, reproduce, err := ownerControlLabTransition(command.Action, state)
	if err != nil {
		return nil, err
	}
	if reproduce {
		return applyOwnerControlLabReproduce(ctx, tx, principal, command, jobType, now)
	}
	if _, err = tx.Exec(ctx, `
...
func ownerControlLabTransition(action, state string) (string, bool, error) {
	switch action {
	case "pause":
		if state == "RUNNING" {
...
func applyOwnerControlLabReproduce(
	ctx context.Context,
	tx pgx.Tx,
	principal authentication.Principal,
	command console.OwnerControlCommand,
...
SELECT request_payload,payload_hash,max_attempts FROM jobs WHERE id=$1`, command.TargetID).Scan(
	...
	newID, _ := ownerConsoleIdentifier("reproduction")
	if _, err := tx.Exec(ctx, `
INSERT INTO jobs(
  ...
  owner_user_id,request_payload,max_attempts
		...
		ownerConsoleDedupe(principal.UserID, command.IdempotencyKey+":reproduction"),
