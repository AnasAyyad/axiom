...
func (store *SandboxRuntimeDispatcherStore) RecordRotationReconciliation(
	ctx context.Context,
	id string,
	...
	if err != nil {
		return sandbox.CredentialRotation{}, fmt.Errorf("sandbox_runtime_rotation_reconciliation_begin_failed")
	}
	defer func() { _ = tx.Rollback(context.Background()) }()
	var accountID string
	if err = tx.QueryRow(ctx, `
SELECT account_id FROM sandbox_runtime_credential_rotations
...
FOR UPDATE`, id, expectedRevision).Scan(&accountID); err != nil ||
		...
		return sandbox.CredentialRotation{}, fmt.Errorf("sandbox_runtime_rotation_reconciliation_failed")
	}
...
WHERE id=$1 AND revision=$2 AND stage='RESTART_VALIDATED' AND account_id=$5`,
	...
	if err = tx.Commit(ctx); err != nil {
		return sandbox.CredentialRotation{}, fmt.Errorf("sandbox_runtime_rotation_reconciliation_commit_failed")
	}
	return store.readCredentialRotation(ctx, id)
}

// CompleteCredentialRotation returns a reconciled account to READY_PAUSED.
func (store *SandboxRuntimeDispatcherStore) CompleteCredentialRotation(
	ctx context.Context,
	id string,
	expectedRevision uint64,
	now time.Time,
	...
	if id == "" || expectedRevision == 0 || now.IsZero() || now.Location() != time.UTC {
		return sandbox.CredentialRotation{}, fmt.Errorf("sandbox_runtime_rotation_complete_failed")
	}
	tx, err := store.pool.BeginTx(ctx, pgx.TxOptions{IsoLevel: pgx.Serializable})
	if err != nil {
		return sandbox.CredentialRotation{}, fmt.Errorf("sandbox_runtime_rotation_complete_begin_failed")
	}
	defer func() { _ = tx.Rollback(context.Background()) }()
	if err = completeSandboxRuntimeRotationTransaction(ctx, tx, id, expectedRevision, now); err != nil {
		return sandbox.CredentialRotation{}, err
	}
	if err = tx.Commit(ctx); err != nil {
		return sandbox.CredentialRotation{}, fmt.Errorf("sandbox_runtime_rotation_complete_commit_failed")
	}
	return store.readCredentialRotation(ctx, id)
}

func completeSandboxRuntimeRotationTransaction(
	ctx context.Context,
	tx pgx.Tx,
	id string,
	expectedRevision uint64,
	...
	completion, err := lockSandboxRuntimeRotationForCompletion(ctx, tx, id, expectedRevision)
	if err != nil {
		return fmt.Errorf("sandbox_runtime_rotation_complete_failed")
	}
	tag, err := tx.Exec(ctx, `
UPDATE sandbox_runtime_credential_rotations
...
WHERE id=$1 AND state='LOCKED' AND revision=$3`,
	...
	if err != nil || tag.RowsAffected() != 1 {
		return fmt.Errorf("sandbox_runtime_rotation_ready_failed")
	}
	return appendSandboxRuntimeRotationCompletionAudit(ctx, tx, id, expectedRevision, completion, now)
}

func appendSandboxRuntimeRotationCompletionAudit(
	ctx context.Context,
	tx pgx.Tx,
	id string,
	expectedRevision uint64,
	completion sandboxRuntimeRotationCompletionState,
	now time.Time,
) error {
...
type sandboxRuntimeRotationCompletionState struct {
	accountID       string
	actorUserID     string
...
func lockSandboxRuntimeRotationForCompletion(
	ctx context.Context,
	tx pgx.Tx,
	id string,
	expectedRevision uint64,
) (sandboxRuntimeRotationCompletionState, error) {
	var state sandboxRuntimeRotationCompletionState
	err := tx.QueryRow(ctx, `
...
func (store *SandboxRuntimeDispatcherStore) advanceRotation(
	ctx context.Context,
	id string,
	expectedRevision uint64,
	prior, next string,
...
WHERE id=$1 AND revision=$2 AND stage=$5`,
	...
	if err != nil || tag.RowsAffected() != 1 {
		return sandbox.CredentialRotation{}, fmt.Errorf("sandbox_runtime_rotation_transition_failed")
	}
	return store.readCredentialRotation(ctx, id)
}

func (store *SandboxRuntimeDispatcherStore) readCredentialRotation(
	ctx context.Context,
	id string,
) (sandbox.CredentialRotation, error) {
...
FROM sandbox_runtime_credential_rotations WHERE id=$1`, id).Scan(
	...
	if err != nil {
		return sandbox.CredentialRotation{}, fmt.Errorf("sandbox_runtime_rotation_read_failed")
	}
...
var _ sandbox.CredentialRotationStore = (*SandboxRuntimeDispatcherStore)(nil)
