...
func TestResearchPromotionPostgresCleanInstallQualification(t *testing.T) {
	ctx, pool := openResearchPromotionTestDatabase(t, "AXIOM_RESEARCH_PROMOTION_TEST_DSN")
	defer pool.Close()
	assertPostgres18(t, ctx, pool)
	assertEmptyTestDatabase(t, ctx, pool)
	applyTriangularArbitrageMigrationPrefix(t, ctx, pool, 19)
	assertResearchPromotionSchemaAndPromotion(t, ctx, pool)
}

func TestResearchPromotionPostgresInventoryRebalancingToResearchPromotionUpgradeQualification(t *testing.T) {
	ctx, pool := openResearchPromotionTestDatabase(t, "AXIOM_RESEARCH_PROMOTION_UPGRADE_TEST_DSN")
	defer pool.Close()
	assertPostgres18(t, ctx, pool)
	assertEmptyTestDatabase(t, ctx, pool)
	applyTriangularArbitrageMigrationPrefix(t, ctx, pool, 18)
	assertResearchPromotionUpgradeSentinel(t, ctx, pool)
	...
	if err != nil || !changed {
		t.Fatalf("rebalancing-to-research-promotion migration changed=%t error=%v", changed, err)
	}
	var sentinel int
	if err = pool.QueryRow(ctx,
		`SELECT count(*) FROM rebalancing_fact_sets WHERE id='research_promotion-upgrade-sentinel'`,
	).Scan(&sentinel); err != nil || sentinel != 1 {
		t.Fatalf("inventory rebalancing upgrade sentinel=%d error=%v", sentinel, err)
	}
	assertResearchPromotionSchemaAndPromotion(t, ctx, pool)
}

func openResearchPromotionTestDatabase(t *testing.T, environment string) (context.Context, *pgxpool.Pool) {
	t.Helper()
	...
	if err != nil || !strings.HasSuffix(configuration.ConnConfig.Database, "_research_promotion_test") {
		t.Fatal("research promotion integration requires a dedicated database ending _research_promotion_test")
	}
...
func assertResearchPromotionUpgradeSentinel(t *testing.T, ctx context.Context, pool *pgxpool.Pool) {
	t.Helper()
...
) VALUES ('configuration-research_promotion-upgrade',700,$1,'{}','research_promotion-upgrade',$2)`, hash, now); err != nil {
		t.Fatal(err)
	}
...
) VALUES ('research_promotion-upgrade-sentinel','configuration-research_promotion-upgrade',$1,
'rebalancing-fact.v1','rebalancing-cost.v1',$1,$2)`, hash, now); err != nil {
		t.Fatal(err)
	}
}

func assertResearchPromotionSchemaAndPromotion(
	t *testing.T,
	ctx context.Context,
	pool *pgxpool.Pool,
) {
	...
	principal := researchPromotionQualificationPrincipal(t, ctx, pool, now)
	seedResearchRegistryReferences(t, ctx, pool)
	repository, err := NewResearchPromotionRepository(pool)
	if err != nil {
		t.Fatal(err)
	}
	champion := seedResearchPromotionResearchFixture(t, ctx, pool, repository, now, "champion", 1)
	challenger := seedResearchPromotionResearchFixture(t, ctx, pool, repository, now, "challenger", 2)
	assertResearchPromotionComparison(t, ctx, pool, repository, champion, challenger, now)
	assertResearchPromotionLowConfidenceDenied(t, ctx, repository, principal, clock, champion)
	assertResearchPromotionSequentialPromotion(t, ctx, repository, principal, clock, champion)
	assertResearchPromotionConcurrentPromotion(t, ctx, pool, repository, principal, clock, challenger)
	assertResearchPromotionDatabaseAuthAndImmutability(
		t, ctx, pool, repository, principal, challenger, now,
	)
	assertResearchPromotionRoleMatrix(t, ctx, pool)
}

func researchPromotionQualificationPrincipal(
	t *testing.T,
	ctx context.Context,
	pool *pgxpool.Pool,
	now time.Time,
	...
	t.Helper()
	const userID = "research_promotion-qualification-owner"
	const sessionID = "research_promotion-qualification-session"
	if _, err := pool.Exec(ctx, `INSERT INTO users(
id,email,password_hash,status,created_at,normalized_email,password_changed_at
) VALUES ($1,'research_promotion-owner@example.test','qualification-hash','active',$2,
  'research_promotion-owner@example.test',$2)`, userID, now); err != nil {
		t.Fatal(err)
	}
  ...
  $3::timestamptz,$3::timestamptz+interval '30 minutes',$3::timestamptz,1)`,
	...
	return authentication.Principal{
		UserID: userID, Email: "research_promotion-owner@example.test", SessionID: sessionID,
		ReauthenticatedAt: now, SessionRevision: 1,
	}
}

type researchPromotionFixture struct {
...
func seedResearchPromotionResearchFixture(
	t *testing.T,
	ctx context.Context,
	pool *pgxpool.Pool,
	repository *ResearchPromotionRepository,
	now time.Time,
	suffix string,
	version int64,
) researchPromotionFixture {
	t.Helper()
	registration, registrationBytes, err := research.RegisterExperiment(
		researchPromotionRegistrationInput(now, suffix, uint32(version)),
	)
	if err != nil {
		t.Fatal(err)
	}
	seedResearchPromotionRegisteredGeneration(t, ctx, pool, registration, registrationBytes, version, now)
	...
	suite, suiteBytes, err := research.BuildValidationSuite(
		registration, researchPromotionValidationInput(registration, now, suffix),
	)
	if err != nil {
		t.Fatal(err)
	}
	evidenceHash := researchRegistryPayloadHash([]byte("evidence-" + suffix))
	if err = repository.RecordValidationSuite(
		ctx, registration.ID, suiteBytes, suite.ManifestHash, evidenceHash,
	); err != nil {
		t.Fatal(err)
	}
	return researchPromotionFixture{registration: registration, suite: suite,
		suiteBytes: suiteBytes, evidenceHash: evidenceHash}
}

func seedResearchPromotionRegisteredGeneration(
	t *testing.T,
	ctx context.Context,
	pool *pgxpool.Pool,
	registration research.ExperimentRegistrationManifest,
...
VALUES ('trend-research_promotion','Trend research promotion','trend') ON CONFLICT (id) DO NOTHING`); err != nil {
		t.Fatal(err)
	}
...
) VALUES ($1,'trend-research_promotion',$2,$3,'research',$4,$3,$5)`,
		registration.StrategyVersionID, version, hash, registration.RegisteredAt, canonical); err != nil {
		t.Fatal(err)
	}
	insertResearchPromotionExperimentAndRuns(t, ctx, pool, registration, now)
}

func insertResearchPromotionExperimentAndRuns(
	t *testing.T,
	ctx context.Context,
	pool *pgxpool.Pool,
	registration research.ExperimentRegistrationManifest,
	...
	_, err := queries.InsertResearchRegistryExperimentRegistration(ctx,
		generated.InsertResearchRegistryExperimentRegistrationParams{
			ID: registration.ID, StrategyVersionID: registration.StrategyVersionID,
			ConfigurationID: "configuration-research_registry", DatasetID: "dataset-public-data-formal-pending",
	...
	if _, err = queries.InsertResearchGeneration(ctx, generated.InsertResearchGenerationParams{
		...
		Generation:       int32(registration.Generation),
		FinalWindowHash:  researchRegistryPayloadHash([]byte(registration.ID + "-final")),
		RegistrationHash: registration.RegistrationHash,
		RegisteredAt:     pgTimestamp(registration.RegisteredAt),
	}); err != nil {
		t.Fatal(err)
	}
	insertResearchPromotionRunsAndConsumption(t, ctx, pool, registration, now)
}

func insertResearchPromotionRunsAndConsumption(
	t *testing.T,
	ctx context.Context,
	pool *pgxpool.Pool,
	registration research.ExperimentRegistrationManifest,
...
) VALUES ($1,$2,'configuration-research_registry',$3,'dataset-public-data-formal-pending',$4,$4,
...
SET state='completed',completed_at=$2 WHERE id=$1`, runID, now.Add(-time.Minute)); err != nil {
	...
	consumptionHash := researchRegistryPayloadHash([]byte("consumption-" + registration.ID))
...
func researchPromotionRegistrationInput(
	now time.Time,
	suffix string,
	generation uint32,
	...
	return research.ExperimentRegistrationInput{
		ID:                   "experiment-research_promotion-" + suffix,
		ResearchGenerationID: "generation-research_promotion-" + suffix,
		StrategyVersionID:    fmt.Sprintf("trend-research_promotion-v%d", generation),
...
func researchPromotionValidationInput(
	registration research.ExperimentRegistrationManifest,
	now time.Time,
	suffix string,
	...
	confidence, _ := research.BlockBootstrapMean(
		[]string{"0.01", "0.02", "0.03", "0.04"}, 2, 100, "research_promotion-"+suffix,
	)
	...
	return research.ValidationSuiteInput{
		ID: "suite-research_promotion-" + suffix, ResearchGenerationID: registration.ResearchGenerationID,
		StrategyVersionID: registration.StrategyVersionID, StrategyFamily: "trend",
		PreregistrationHash:      registration.RegistrationHash,
		FinalTestConsumptionHash: researchRegistryPayloadHash([]byte("consumption-" + registration.ID)),
...
func assertResearchPromotionComparison(
	t *testing.T,
	ctx context.Context,
	pool *pgxpool.Pool,
	repository *ResearchPromotionRepository,
	champion researchPromotionFixture,
	challenger researchPromotionFixture,
	now time.Time,
) {
	t.Helper()
	result := func(name string) research.ResultSlice {
	...
	report, canonical, err := research.BuildChampionChallengerReport(
		research.ChampionChallengerInput{ID: "comparison-research_promotion",
	...
	if _, err = pool.Exec(ctx,
		`DELETE FROM research_promotion_champion_challenger_reports WHERE id='comparison-research_promotion'`,
	); err == nil {
		t.Fatal("immutable champion/challenger report deleted")
	}
}

func assertResearchPromotionLowConfidenceDenied(
	t *testing.T,
	ctx context.Context,
	repository *ResearchPromotionRepository,
	principal authentication.Principal,
	clock *domain.ReplayClock,
	fixture researchPromotionFixture,
) {
	t.Helper()
	input := researchPromotionValidationInput(fixture.registration, clock.Now().UTC, "low")
	input.ID = "suite-research_promotion-low"
	...
	if err = repository.RecordValidationSuite(ctx, fixture.registration.ID, canonical,
		suite.ManifestHash, researchRegistryPayloadHash([]byte("low-evidence"))); err != nil {
		t.Fatal(err)
	}
	...
	_, err = service.Promote(ctx, principal, research.PromotionRequest{
		...
		ExpectedRevision: 1, IdempotencyKey: "research_promotion-low-confidence",
...
func assertResearchPromotionSequentialPromotion(
	t *testing.T,
	ctx context.Context,
	repository *ResearchPromotionRepository,
	principal authentication.Principal,
	...
	for index, target := range targets {
		request := research.PromotionRequest{
			...
			IdempotencyKey: fmt.Sprintf("research_promotion-sequential-%d", index+1),
...
func assertResearchPromotionConcurrentPromotion(
	t *testing.T,
	ctx context.Context,
	pool *pgxpool.Pool,
	repository *ResearchPromotionRepository,
	...
	for index := range results {
		...
		go func(index int) {
			...
			_, results[index] = service.Promote(ctx, principal, research.PromotionRequest{
				...
				IdempotencyKey: fmt.Sprintf("research_promotion-concurrent-%d", index),
...
func assertResearchPromotionDatabaseAuthAndImmutability(
	t *testing.T,
	ctx context.Context,
	pool *pgxpool.Pool,
	repository *ResearchPromotionRepository,
	...
	result, err := repository.ApplyPromotion(ctx, research.PromotionCommand{
		PromotionRequest: research.PromotionRequest{
			...
			IdempotencyKey: "research_promotion-invalid-session", Reason: "database auth check"},
		CommandID:   "promotion-research_promotion-invalid-session",
	...
	result, err = repository.ApplyPromotion(ctx, research.PromotionCommand{
		PromotionRequest: research.PromotionRequest{
			...
			EvidenceID:        fixture.suite.ID, EvidenceHash: fixture.suite.ManifestHash,
			Target: research.MaturityReplayValidated, ExpectedRevision: 2,
			IdempotencyKey: "research_promotion-stale-command-time", Reason: "database time check"},
		CommandID:   "promotion-research_promotion-stale-command-time",
	...
	for _, statement := range []string{
		`UPDATE research_promotion_validation_suites SET confidence_label='rejected' WHERE id='suite-research_promotion-champion'`,
		`DELETE FROM strategy_maturity_commands WHERE id='promotion-research_promotion-invalid-session'`,
	} {
		if _, err = pool.Exec(ctx, statement); err == nil {
			t.Fatalf("immutable research promotion evidence mutated: %s", statement)
		}
	}
}

func assertResearchPromotionRoleMatrix(t *testing.T, ctx context.Context, pool *pgxpool.Pool) {
	t.Helper()
	runtimeRole := testRole("AXIOM_RESEARCH_PROMOTION_RUNTIME_ROLE", "axiom_app")
	recorderRole := testRole("AXIOM_RESEARCH_PROMOTION_RECORDER_ROLE", "axiom_recorder")
	readOnlyRole := testRole("AXIOM_RESEARCH_PROMOTION_READONLY_ROLE", "axiom_readonly")
...
has_table_privilege($1,'research_promotion_validation_suites','INSERT'),
has_table_privilege($1,'strategy_maturity_states','UPDATE'),
has_function_privilege($1,'apply_research_promotion_maturity_promotion(text,text,text,sha256_hex,text,bigint,text,text,text,sha256_hex,text,timestamptz)','EXECUTE'),
has_table_privilege($2,'research_promotion_validation_suites','SELECT'),
...
has_table_privilege($3,'strategy_maturity_events','INSERT')`,
	...
	if err != nil || !runtimeInsert || runtimeStateUpdate || !runtimeExecute ||
		recorderSelect || !readOnlySelect || readOnlyInsert {
		t.Fatalf("research promotion role matrix insert=%t update=%t execute=%t recorder=%t read=%t write=%t error=%v",
