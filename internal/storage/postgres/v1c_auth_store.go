...
// V1CAuthenticationStore owns atomic TOTP replay prevention, one-use
// authorizations, high-risk audit linkage, and revoke-all session control.
type V1CAuthenticationStore struct{ pool *pgxpool.Pool }

// NewV1CAuthenticationStore constructs the durable high-risk authorization store.
func NewV1CAuthenticationStore(pool *pgxpool.Pool) (*V1CAuthenticationStore, error) {
	if pool == nil {
		return nil, fmt.Errorf("v1c_authentication_pool_missing")
	}
	return &V1CAuthenticationStore{pool: pool}, nil
}

// CreateSandboxAuthorization atomically advances TOTP replay state, grant, and audit.
func (store *V1CAuthenticationStore) CreateSandboxAuthorization(
	ctx context.Context,
	write authentication.NewSandboxAuthorization,
) error {
	if err := validateV1CAuthorizationWrite(write); err != nil {
		return err
	}
	tx, err := store.pool.BeginTx(ctx, pgx.TxOptions{IsoLevel: pgx.ReadCommitted})
	if err != nil {
		return fmt.Errorf("v1c_authorization_begin_failed")
	}
	defer func() { _ = tx.Rollback(context.Background()) }()
	if err = persistV1CAuthorization(ctx, tx, write); err != nil {
		return err
	}
	if err = tx.Commit(ctx); err != nil {
		return fmt.Errorf("v1c_authorization_commit_failed")
	}
	return nil
}

func persistV1CAuthorization(
	ctx context.Context,
	tx pgx.Tx,
	write authentication.NewSandboxAuthorization,
) error {
	if _, err := activeV1CSessionRevision(
		ctx,
		tx,
		write.UserID,
		write.SessionID,
	...
	var counter int64
	err := tx.QueryRow(ctx, `
INSERT INTO v1c_totp_replay_state(user_id,last_used_counter,updated_at)
VALUES ($1,$2,$3)
ON CONFLICT (user_id) DO UPDATE
SET last_used_counter=EXCLUDED.last_used_counter,updated_at=EXCLUDED.updated_at
WHERE v1c_totp_replay_state.last_used_counter < EXCLUDED.last_used_counter
RETURNING last_used_counter`, write.UserID, write.TOTPCounter, write.CreatedAt).Scan(&counter)
	if err != nil || counter != write.TOTPCounter {
		return fmt.Errorf("v1c_totp_replay_rejected")
	}
...
func (store *V1CAuthenticationStore) ConsumeSandboxAuthorization(
	ctx context.Context,
	...
	tx, err := store.pool.BeginTx(ctx, pgx.TxOptions{IsoLevel: pgx.ReadCommitted})
	if err != nil {
		return authentication.ConsumedAuthorization{}, fmt.Errorf("v1c_authorization_consume_begin_failed")
	}
	defer func() { _ = tx.Rollback(context.Background()) }()
	consumed, err := consumeV1CAuthorization(
		ctx, tx, tokenHash, sessionID, purpose, now,
	)
	if err != nil {
		return authentication.ConsumedAuthorization{}, err
	}
	revision, err := activeV1CSessionRevision(
		ctx,
		tx,
		consumed.UserID,
		consumed.SessionID,
	...
	if err = tx.Commit(ctx); err != nil {
		return authentication.ConsumedAuthorization{}, fmt.Errorf("v1c_authorization_consume_commit_failed")
	}
	consumed.ConsumedAt = consumed.ConsumedAt.UTC()
	return consumed, nil
}

func consumeV1CAuthorization(
	ctx context.Context,
	tx pgx.Tx,
	tokenHash, sessionID string,
	purpose authentication.AuthorizationPurpose,
	...
	err := tx.QueryRow(ctx, `
UPDATE v1c_sandbox_authorizations
SET consumed_at=$4
...
RETURNING id,user_id,session_id,purpose,source_hash,reason_hash,target_revision,consumed_at`,
	...
	if err != nil {
		return authentication.ConsumedAuthorization{}, fmt.Errorf("v1c_authorization_consume_failed")
	}
	return consumed, nil
}

func activeV1CSessionRevision(
	ctx context.Context,
	tx pgx.Tx,
...
FOR SHARE OF session,actor`,
		...
		return 0, fmt.Errorf("v1c_authorization_session_invalid")
	}
	return revision, nil
}

func validateV1CAuthorizationWrite(write authentication.NewSandboxAuthorization) error {
	audit := write.Audit
...
func (store *V1CAuthenticationStore) RevokeAllUserSessions(
	ctx context.Context,
	...
	if authorizationID == "" || userID == "" || actorSessionID == "" ||
		...
		return 0, fmt.Errorf("v1c_revoke_all_invalid")
	}
	tx, err := store.pool.BeginTx(ctx, pgx.TxOptions{IsoLevel: pgx.Serializable})
	if err != nil {
		return 0, fmt.Errorf("v1c_revoke_all_begin_failed")
	}
	defer func() { _ = tx.Rollback(context.Background()) }()
	count, err := revokeAllV1CSessions(
	...
	if err = tx.Commit(ctx); err != nil {
		return 0, fmt.Errorf("v1c_revoke_all_commit_failed")
	}
	return count, nil
}

func revokeAllV1CSessions(
	ctx context.Context,
	tx pgx.Tx,
	authorizationID, userID, actorSessionID, sourceHash, reasonHash string,
	now time.Time,
) (int64, error) {
	revision, err := lockV1CActorSession(ctx, tx, userID, actorSessionID, now)
	if err != nil {
		return 0, err
	}
	beforeHash, err := v1CSessionStateHash(ctx, tx, userID)
	if err != nil {
		return 0, err
	}
	count, err := revokeV1CSessions(ctx, tx, userID, now)
	if err != nil {
		return 0, err
	}
	afterHash, err := v1CSessionStateHash(ctx, tx, userID)
	if err != nil {
		return 0, err
	}
	controlID := "revoke-all-" + actorSessionID + "-" + fmt.Sprint(now.UnixNano())
	if err = insertV1CSessionControl(
