...
func TestExchangeExpansionPostgresCleanInstallQualification(t *testing.T) {
	ctx, pool := openExchangeExpansionTestDatabase(t, "AXIOM_EXCHANGE_EXPANSION_TEST_DSN")
	defer pool.Close()
	...
	assertExchangeExpansionSchema(t, ctx, pool)
	assertExchangeExpansionImmutableEvidence(t, ctx, pool)
	assertExchangeExpansionRoleGrants(t, ctx, pool)
}

func TestExchangeExpansionPostgresTrendFoundationToExchangeExpansionUpgradeQualification(t *testing.T) {
	ctx, pool := openExchangeExpansionTestDatabase(t, "AXIOM_EXCHANGE_EXPANSION_UPGRADE_TEST_DSN")
	defer pool.Close()
	assertPostgres18(t, ctx, pool)
	assertEmptyTestDatabase(t, ctx, pool)
	migrations, err := Migrations()
	...
	for _, migration := range migrations[:11] {
		...
		if applyErr != nil || !changed {
			connection.Release()
			t.Fatalf("initial trend migration %s changed=%t error=%v", migration.Name, changed, applyErr)
		}
	}
	connection.Release()
	...
	if applied, applyErr := ApplyMigrations(ctx, pool); applyErr != nil || applied != len(migrations)-11 {
		t.Fatalf("initial-platform-to-exchange-expansion migrations=%d/%d error=%v", applied, len(migrations)-11, applyErr)
	}
	assertExchangeExpansionSchema(t, ctx, pool)
	assertExchangeExpansionImmutableEvidence(t, ctx, pool)
}

func openExchangeExpansionTestDatabase(t *testing.T, environment string) (context.Context, *pgxpool.Pool) {
	t.Helper()
	...
	if err != nil || !strings.HasSuffix(configuration.ConnConfig.Database, "_exchange_expansion_test") {
		t.Fatal("exchange expansion integration requires a dedicated database ending _exchange_expansion_test")
	}
...
func assertExchangeExpansionSchema(t *testing.T, ctx context.Context, pool *pgxpool.Pool) {
	t.Helper()
	var count int
...
WHERE NOT tgisinternal AND tgname IN (
  ...
  'public_clock_samples_immutable','public_connection_events_immutable'
		...
		t.Fatalf("exchange expansion relational/immutable triggers=%d error=%v", count, err)
	}
}

func assertExchangeExpansionImmutableEvidence(t *testing.T, ctx context.Context, pool *pgxpool.Pool) {
	t.Helper()
	statements := []struct {
		sql  string
		args []any
...
func assertExchangeExpansionRoleGrants(t *testing.T, ctx context.Context, pool *pgxpool.Pool) {
	t.Helper()
	roles := []string{"axiom_exchange_expansion_runtime", "axiom_exchange_expansion_recorder", "axiom_exchange_expansion_readonly"}
