...
func applyOperationalEvidenceIncidentTransition(
	ctx context.Context, tx pgx.Tx, principal authentication.Principal,
	command console.OwnerControlCommand, now time.Time,
) (map[string]any, error) {
	current, revision, err := operationalEvidenceIncidentState(ctx, tx, command.TargetID)
	if err != nil {
		return nil, err
	}
	...
	if command.State == "resolved" {
		if err = operationalEvidenceRecordResolution(ctx, tx, principal, command, now); err != nil {
			return nil, err
		}
	}
	next := revision + 1
...
resolved_at=CASE WHEN $2='resolved' THEN $4 ELSE resolved_at END WHERE id=$1`,
	...
	if err = appendOperationalEvidenceIncidentEvent(ctx, tx, command.TargetID, next, command.State,
		principal.UserID, command.Reason, "", "", operationalEvidenceResolutionDetail(command),
...
func operationalEvidenceRecordResolution(
	ctx context.Context, tx pgx.Tx, principal authentication.Principal,
	command console.OwnerControlCommand, now time.Time,
) error {
	...
	_, err := tx.Exec(ctx, `INSERT INTO owner_console_incident_resolution_evidence(
incident_id,evidence,evidence_hash,recorded_by,recorded_at
) VALUES($1,$2,$3,$4,$5)`, command.TargetID, evidence, ownerConsoleHash([]byte(evidence)), principal.UserID, now)
	return ownerConsoleConstraintError(err)
}

func operationalEvidenceResolutionDetail(command console.OwnerControlCommand) string {
...
func operationalEvidenceIncidentState(ctx context.Context, tx pgx.Tx, id string) (string, int64, error) {
	var state string
	var revision int64
...
func appendOperationalEvidenceIncidentEvent(
	...
	var previous *string
	err := tx.QueryRow(ctx, `SELECT event_hash::text FROM owner_console_incident_events
WHERE incident_id=$1 ORDER BY incident_revision DESC LIMIT 1`, incidentID).Scan(&previous)
	...
	hash := ownerConsoleHash([]byte(fmt.Sprintf("%s\x00%s\x00%d\x00%s\x00%s\x00%s\x00%s\x00%s\x00%s",
		prior, incidentID, revision, eventType, actor, reason, referenceType, referenceID, detail)))
	id, _ := ownerConsoleIdentifier("incident-event")
	_, err = tx.Exec(ctx, `INSERT INTO owner_console_incident_events(
...
func operationalEvidenceActiveUser(ctx context.Context, tx pgx.Tx, id string) (bool, error) {
	var exists bool
	err := tx.QueryRow(ctx, `SELECT EXISTS(SELECT 1 FROM users WHERE id=$1 AND status='active')`, id).Scan(&exists)
	return exists, err
}

func operationalEvidencePayloadRevision(value any) (int64, error) {
...
func operationalEvidenceIncidentSeverity(value string) bool {
