...
const selectSandboxRuntimeClaimSQL = `
SELECT outbox.id FROM sandbox_runtime_submission_outbox outbox
WHERE outbox.account_id=$1 AND outbox.account_epoch=$2 AND (
  ...
  outbox.intent_action<>'ENTRY' OR EXISTS (
    ...
    WHERE plan.id=outbox.plan_id
      AND (plan.execution_expires_at IS NULL OR plan.execution_expires_at>$3)
...
const markSandboxRuntimeSubmittingSQL = `
UPDATE sandbox_runtime_submission_outbox outbox
...
const markSandboxRuntimeCancelPendingSQL = `
UPDATE sandbox_runtime_submission_outbox outbox
...
WHERE outbox.account_id=$1
  ...
  AND EXISTS (
    SELECT 1
    FROM sandbox_runtime_account_leases lease
    JOIN sandbox_runtime_exchange_accounts exchange_account
...
func (store *SandboxRuntimeDispatcherStore) ClaimOutbox(
	ctx context.Context,
	...
	if err != nil {
		return nil, fmt.Errorf("sandbox_runtime_claim_begin_failed")
	}
	defer func() { _ = tx.Rollback(context.Background()) }()
	ids, err := selectSandboxRuntimeClaimIDs(ctx, tx, account, epoch, owner, fence, now, limit)
	if err != nil {
		return nil, err
	}
	result, err := claimSandboxRuntimeOutboxIDs(ctx, tx, ids, owner, fence, now, ttl)
	if err != nil {
		return nil, err
	}
	if err = tx.Commit(ctx); err != nil {
		return nil, fmt.Errorf("sandbox_runtime_claim_commit_failed")
	}
...
func selectSandboxRuntimeClaimIDs(
	ctx context.Context,
	tx pgx.Tx,
	account sandbox.AccountID,
	epoch uint64,
...
FROM sandbox_runtime_account_leases WHERE account_id=$1 FOR UPDATE`,
		account, owner, fence, now).Scan(&leaseOK); err != nil || !leaseOK {
		return nil, fmt.Errorf("sandbox_runtime_stale_fencing_token")
	}
	rows, err := tx.Query(ctx, selectSandboxRuntimeClaimSQL, account, epoch, now, fence, limit)
	if err != nil {
		return nil, fmt.Errorf("sandbox_runtime_claim_select_failed")
	}
	var ids []string
	for rows.Next() {
		var id string
		if err = rows.Scan(&id); err != nil {
			rows.Close()
			return nil, fmt.Errorf("sandbox_runtime_claim_scan_failed")
		}
		ids = append(ids, id)
	}
	rows.Close()
...
func claimSandboxRuntimeOutboxIDs(
	ctx context.Context,
	tx pgx.Tx,
	ids []string,
	owner string,
...
WHERE id=$1`, id, owner, fence, now.Add(ttl), now); err != nil {
			return nil, fmt.Errorf("sandbox_runtime_claim_update_failed")
		}
...
type sandboxRuntimeQueryRower interface {
	QueryRow(context.Context, string, ...any) pgx.Row
}

func readSandboxRuntimeOutbox(ctx context.Context, tx sandboxRuntimeQueryRower, id string) (sandbox.SubmissionOutbox, error) {
...
SELECT id,plan_id,account_id,account_epoch,order_id,client_order_id,strategy_id,
 ...
 leg_index,depends_on_leg_index,state,claim_owner,fencing_token,claim_expires_at,
 attempt,updated_at
FROM sandbox_runtime_submission_outbox WHERE id=$1`, id).Scan(
		...
		&record.Submission.PolicyHash, &record.Submission.ApprovedAt, &legIndex, &dependsOn,
		&record.State,
		&claimOwner, &fencingToken, &claimExpiresAt, &record.Attempt, &record.UpdatedAt)
	if err != nil {
		return sandbox.SubmissionOutbox{}, fmt.Errorf("sandbox_runtime_outbox_read_failed")
	}
	if legIndex < 0 {
		return sandbox.SubmissionOutbox{}, fmt.Errorf("sandbox_runtime_outbox_read_failed")
	}
	record.LegIndex = uint32(legIndex)
	if dependsOn != nil {
		if *dependsOn < 0 {
			return sandbox.SubmissionOutbox{}, fmt.Errorf("sandbox_runtime_outbox_read_failed")
		}
		value := uint32(*dependsOn)
		record.DependsOn = &value
	}
	return decodeSandboxRuntimeOutbox(
...
func decodeSandboxRuntimeOutbox(
	...
	setSandboxRuntimeClaimMetadata(&record, claimOwner, fencingToken, claimExpiresAt)
	...
	if !ok {
		return sandbox.SubmissionOutbox{}, fmt.Errorf("sandbox_runtime_instrument_invalid")
	}
...
func setSandboxRuntimeClaimMetadata(
	record *sandbox.SubmissionOutbox,
	claimOwner *string,
	fencingToken *int64,
	claimExpiresAt *time.Time,
...
func (store *SandboxRuntimeDispatcherStore) MarkSubmitting(
	ctx context.Context,
	id string,
	fence uint64,
	now time.Time,
	...
	if err != nil {
		return fmt.Errorf("sandbox_runtime_submitting_begin_failed")
	}
	defer func() { _ = tx.Rollback(context.Background()) }()
	var planID string
	err = tx.QueryRow(ctx, markSandboxRuntimeSubmittingSQL, id, fence, now).Scan(&planID)
	if err != nil {
...
func (store *SandboxRuntimeDispatcherStore) MarkUnknown(
	...
	tx, err := store.pool.BeginTx(ctx, pgx.TxOptions{IsoLevel: pgx.Serializable})
	if err != nil {
		return fmt.Errorf("sandbox_runtime_unknown_begin_failed")
	}
	defer func() { _ = tx.Rollback(context.Background()) }()
	var planID string
	err = tx.QueryRow(ctx, `
...
RETURNING plan_id`, id, fence, now).Scan(&planID)
	if err != nil {
		return fmt.Errorf("sandbox_runtime_unknown_transition_failed")
	}
	if err = updateSandboxRuntimePlanState(ctx, tx, planID); err != nil {
		return err
	}
	if err = tx.Commit(ctx); err != nil {
		return fmt.Errorf("sandbox_runtime_unknown_commit_failed")
	}
	return kill.Hit(ctx, sandbox.KillAfterReducerUpdate)
}
