...
func (store *OwnerConsoleStore) populateRunLabShadow(ctx context.Context, item *generated.ShadowSessionResource) error {
	if item.RunId == nil || *item.RunId == "" {
		return nil
	}
	runID := *item.RunId
	decisions, err := store.runLabShadowDecisions(ctx, runID)
	if err != nil {
		return err
	}
	balances, positions, err := store.runLabShadowInventory(ctx, runID)
	if err != nil {
		return err
	}
	pnl, err := store.runLabShadowPnl(ctx, runID)
	if err != nil {
		return err
	}
	...
	if item.ExchangeId != nil {
		health, healthErr := store.runLabShadowHealth(ctx, *item.ExchangeId)
...
func (store *OwnerConsoleStore) runLabShadowDecisions(ctx context.Context, runID string) ([]generated.ShadowDecisionSummary, error) {
...
func (store *OwnerConsoleStore) runLabShadowInventory(
	ctx context.Context,
	runID string,
...
func (store *OwnerConsoleStore) runLabShadowPnl(ctx context.Context, runID string) (generated.ShadowPnlAttribution, error) {
...
func (store *OwnerConsoleStore) runLabShadowHealth(ctx context.Context, exchange string) (generated.ShadowDataHealth, error) {
