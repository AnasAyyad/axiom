...
func (store *SandboxRuntimeDispatcherStore) StopCanarySession(
	ctx context.Context,
	...
	if session == "" || account == "" || now.IsZero() ||
		...
		return fmt.Errorf("sandbox_runtime_canary_stop_invalid")
	}
	tx, err := store.pool.BeginTx(ctx, pgx.TxOptions{IsoLevel: pgx.Serializable})
	if err != nil {
		return fmt.Errorf("sandbox_runtime_canary_stop_begin_failed")
	}
	...
	if err = tx.Commit(ctx); err != nil {
		return fmt.Errorf("sandbox_runtime_canary_stop_commit_failed")
	}
	return nil
}
...
WHERE sandbox_session_id=$1
  ...
  AND revoked_at IS NULL`,
		...
		return fmt.Errorf("sandbox_runtime_canary_arm_revoke_failed")
	}
	return nil
}
...
WHERE id=$1 AND state<>'STOPPED'`,
	...
	if err != nil || tag.RowsAffected() > 1 {
		return fmt.Errorf("sandbox_runtime_canary_session_stop_failed")
	}
	return nil
}
...
WHERE id=$1 AND state='ARMED'`,
		...
		return fmt.Errorf("sandbox_runtime_canary_account_lock_failed")
	}
	return nil
}
