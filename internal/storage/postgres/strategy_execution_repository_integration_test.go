...
func TestStrategyExecutionPostgresAtomicOrderFillJournalCheckpoint(t *testing.T) {
	dsn := os.Getenv("AXIOM_STRATEGY_EXECUTION_TEST_DSN")
	if dsn == "" {
		t.Skip("AXIOM_STRATEGY_EXECUTION_TEST_DSN is not set")
	}
	configuration, err := pgxpool.ParseConfig(dsn)
	if err != nil || !strings.HasSuffix(configuration.ConnConfig.Database, "_strategy_execution_test") {
		t.Fatal("strategy execution integration requires a dedicated database ending _strategy_execution_test")
	}
	...
	seedStrategyExecutionReferences(t, ctx, pool)
	repository, err := NewStrategyExecutionRepository(pool)
	if err != nil {
		t.Fatal(err)
	}
	write := strategyExecutionAtomicFixture()
	if err = repository.Commit(ctx, write); err != nil {
		t.Fatal(err)
	}
	assertStrategyExecutionAtomicState(t, ctx, pool, 1)
...
func seedStrategyExecutionReferences(t *testing.T, ctx context.Context, pool *pgxpool.Pool) {
	t.Helper()
	...
	for index, statement := range statements {
		if _, err := pool.Exec(ctx, statement, hash, now); err != nil {
			t.Fatalf("strategy execution reference seed %d failed: %v", index+1, err)
		}
	}
	...
	for index, state := range states {
		...
		if err != nil {
			t.Fatal("strategy execution order seed transition failed")
		}
	}
}

func strategyExecutionAtomicFixture() StrategyExecutionAtomicWrite {
	...
	return StrategyExecutionAtomicWrite{
		...
		Checkpoint: generated.InsertStrategyExecutionCheckpointParams{ID: "checkpoint-a", RunID: "run-a", Revision: 1,
...
func assertStrategyExecutionAtomicState(t *testing.T, ctx context.Context, pool *pgxpool.Pool, expected int) {
	t.Helper()
