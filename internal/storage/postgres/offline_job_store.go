...
const offlineJobLease = 30 * time.Second

var (
	ownerConsoleFailureCode = regexp.MustCompile(`^[a-z][a-z0-9_]{0,95}$`)
	ownerConsoleDecimal     = regexp.MustCompile(`^-?(0|[1-9][0-9]*)(\.[0-9]+)?$`)
)

// OfflineJobMaterializer turns a durable request into verified offline inputs.
// Exchange clients are deliberately absent from this boundary.
type OfflineJobMaterializer func(context.Context, string, string, json.RawMessage) (backtest.JobClaim, error)

// OfflineJobStore implements durable, exclusively leased offline job execution.
type OfflineJobStore struct {
	pool        *pgxpool.Pool
	owner       string
	clock       domain.Clock
	materialize OfflineJobMaterializer
}

// NewOfflineJobStore constructs the PostgreSQL backtest.Worker boundary.
func NewOfflineJobStore(pool *pgxpool.Pool, owner string, clock domain.Clock, materialize OfflineJobMaterializer) (*OfflineJobStore, error) {
	if pool == nil || owner == "" || clock == nil || materialize == nil {
		return nil, fmt.Errorf("owner_console_job_store_dependencies_missing")
	}
	return &OfflineJobStore{pool: pool, owner: owner, clock: clock, materialize: materialize}, nil
}

// Claim exclusively leases the oldest queued or expired offline job.
func (store *OfflineJobStore) Claim(ctx context.Context) (backtest.JobClaim, bool, error) {
	...
	if err != nil {
		...
		return backtest.JobClaim{}, false, fmt.Errorf("owner_console_job_materialization_failed")
	}
	claim.ID = jobID
	...
	if err = store.pool.QueryRow(ctx, `SELECT resume_ordinal,single_step FROM jobs WHERE id=$1 AND state IN ('RUNNING','PAUSE_REQUESTED')
      AND claim_owner=$2`, jobID, store.owner).Scan(&resumeOrdinal, &claim.SingleStep); err != nil || resumeOrdinal < 0 {
		...
		return backtest.JobClaim{}, false, fmt.Errorf("owner_console_job_control_materialization_failed")
	}
	...
	if err = store.attachRun(ctx, kind, payload, claim); err != nil {
		...
		return backtest.JobClaim{}, false, fmt.Errorf("owner_console_job_run_manifest_failed: %w", err)
	}
	return claim, true, nil
}

func (store *OfflineJobStore) claimRow(ctx context.Context, now time.Time) (string, string, json.RawMessage, bool, error) {
	...
	leaseEnd := now.Add(offlineJobLease)
...
func (store *OfflineJobStore) Renew(ctx context.Context, id string) error {
	if id == "" {
		return fmt.Errorf("owner_console_job_renewal_invalid")
	}
	now := store.clock.Now().UTC
	tag, err := store.pool.Exec(ctx, `UPDATE jobs SET claim_expires_at=$1,updated_at=$2,
      ...
      AND claim_expires_at>$2`, now.Add(offlineJobLease), now, id, store.owner)
	if err != nil || tag.RowsAffected() != 1 {
		return fmt.Errorf("owner_console_job_renewal_rejected")
	}
	return nil
}

// Control reads the current fenced replay posture without mutating progress.
func (store *OfflineJobStore) Control(ctx context.Context, id string) (backtest.JobControl, error) {
	var state string
	var resume int64
	var step bool
	...
	if err != nil || resume < 0 {
		return backtest.JobControl{}, fmt.Errorf("owner_console_job_control_unavailable")
	}
...
func (store *OfflineJobStore) Pause(ctx context.Context, id string, ordinal uint64, checkpoint []byte) error {
	if id == "" || ordinal == 0 || ordinal > uint64(^uint64(0)>>1) || !json.Valid(checkpoint) {
		return fmt.Errorf("owner_console_job_pause_invalid")
	}
	now := store.clock.Now().UTC
	tx, err := store.pool.BeginTx(ctx, pgx.TxOptions{IsoLevel: pgx.Serializable})
	if err != nil {
	...
	if err = tx.QueryRow(ctx, `SELECT state,run_id,single_step FROM jobs WHERE id=$1 AND
      state IN ('RUNNING','PAUSE_REQUESTED') AND claim_owner=$2 AND claim_expires_at>$3 FOR UPDATE`,
		...
		return fmt.Errorf("owner_console_job_pause_not_requested")
	}
	if state == "RUNNING" {
	...
	tag, err := tx.Exec(ctx, `UPDATE jobs SET state='PAUSED',claim_owner=NULL,claim_expires_at=NULL,
      ...
      WHERE id=$1 AND state='PAUSE_REQUESTED' AND claim_owner=$5`, id, int64(ordinal), string(checkpoint),
		now, store.owner)
	if err != nil || tag.RowsAffected() != 1 {
		return fmt.Errorf("owner_console_job_pause_conflict")
	}
	runTag, err := tx.Exec(ctx, `UPDATE runs SET state='paused' WHERE id=$1 AND state='running'`, runID)
	if err != nil || runTag.RowsAffected() != 1 {
		return fmt.Errorf("owner_console_run_pause_conflict")
	}
	if err = insertOwnerConsolePauseCheckpoint(ctx, tx, id, runID, ordinal, checkpoint, partial.Events, now); err != nil {
		return err
	}
	return tx.Commit(ctx)
}

func insertOwnerConsolePauseCheckpoint(ctx context.Context, tx pgx.Tx, id, runID string, ordinal uint64,
	...
	checkpointHash := ownerConsoleSHA256(checkpoint)
	...
	return insertOwnerConsoleCanonicalOutputs(ctx, tx, runID, events)
}

// Complete atomically publishes the safe API result projection and releases the lease.
func (store *OfflineJobStore) Complete(ctx context.Context, id string, result backtest.CanonicalResult, canonical []byte) error {
	if id == "" || result.ResultHash == "" || !json.Valid(canonical) {
		return fmt.Errorf("owner_console_job_result_invalid")
	}
	now := store.clock.Now().UTC
	if err := store.completeRun(ctx, id, result, canonical, now); err != nil {
		return fmt.Errorf("owner_console_job_completion_rejected")
	}
	return nil
}

// Fail records one bounded terminal reason and releases the worker lease.
func (store *OfflineJobStore) Fail(ctx context.Context, id, reason string) error {
	if id == "" || !ownerConsoleFailureCode.MatchString(reason) {
		return fmt.Errorf("owner_console_job_failure_invalid")
	}
	now := store.clock.Now().UTC
	if err := store.failRun(ctx, id, reason, now); err != nil {
		return fmt.Errorf("owner_console_job_failure_rejected")
	}
	return nil
}

func offlineJobMetrics(result backtest.CanonicalResult) map[string]generated.Decimal {
	...
	for key, value := range values {
		if ownerConsoleDecimal.MatchString(value) {
...
func offlineJobResult(result backtest.CanonicalResult, reportID, reportHash string) generated.JobResult {
	metrics := offlineJobMetrics(result)
...
var _ backtest.ControlledJobStore = (*OfflineJobStore)(nil)
