...
// TriangularArbitrageCandidateWrite is one complete three-leg immutable candidate record.
type TriangularArbitrageCandidateWrite struct {
	Candidate generated.InsertTriangularCandidateParams
	Legs      []generated.InsertTriangularCandidateLegParams
}

// TriangularArbitrageOutcomeWrite atomically records one terminal simulation, opportunity
// lifetime, and every already-balanced journal linkage.
type TriangularArbitrageOutcomeWrite struct {
...
// TriangularArbitrageRepository owns durable triangular decisions and atomic claim functions.
type TriangularArbitrageRepository struct{ pool *pgxpool.Pool }

// NewTriangularArbitrageRepository constructs the triangular arbitrage persistence boundary.
func NewTriangularArbitrageRepository(pool *pgxpool.Pool) (*TriangularArbitrageRepository, error) {
	if pool == nil {
		return nil, fmt.Errorf("triangular_arbitrage_repository_pool_missing")
	}
	return &TriangularArbitrageRepository{pool: pool}, nil
}

// RecordCandidate commits the parent plus exactly three chained legs or none.
func (repository *TriangularArbitrageRepository) RecordCandidate(ctx context.Context, write TriangularArbitrageCandidateWrite) error {
	if !validTriangularArbitrageCandidateWrite(write) {
		return fmt.Errorf("triangular_arbitrage_candidate_invalid")
	}
	tx, err := repository.pool.BeginTx(ctx, pgx.TxOptions{})
	if err != nil {
		return fmt.Errorf("triangular_arbitrage_candidate_begin_failed")
	}
	defer func() { _ = tx.Rollback(context.Background()) }()
	queries := generated.New(tx)
	if _, err = queries.InsertTriangularCandidate(ctx, write.Candidate); err != nil {
		return fmt.Errorf("triangular_arbitrage_candidate_insert_failed")
	}
	for _, leg := range write.Legs {
		if _, err = queries.InsertTriangularCandidateLeg(ctx, leg); err != nil {
			return fmt.Errorf("triangular_arbitrage_candidate_leg_insert_failed")
		}
	}
	if err = tx.Commit(ctx); err != nil {
		return fmt.Errorf("triangular_arbitrage_candidate_commit_failed")
	}
	return nil
}

// RegisterClaimResource initializes or safely refreshes unheld exact capacity.
func (repository *TriangularArbitrageRepository) RegisterClaimResource(
	ctx context.Context,
	write generated.RegisterTriangularArbitrageClaimResourceParams,
) error {
	if write.PID == "" || write.PAccountID == "" || write.PExchangeID == "" ||
		...
		return fmt.Errorf("triangular_arbitrage_claim_resource_invalid")
	}
	if err := generated.New(repository.pool).RegisterTriangularArbitrageClaimResource(ctx, write); err != nil {
		return fmt.Errorf("triangular_arbitrage_claim_resource_failed: %w", err)
	}
	return nil
}

// Claim atomically acquires every sorted multi-resource requirement.
func (repository *TriangularArbitrageRepository) Claim(
	ctx context.Context,
	write generated.ClaimTriangularArbitrageResourcesParams,
) error {
	if write.GroupID == "" || write.DecisionID == "" || write.AccountID == "" ||
		...
		return fmt.Errorf("triangular_arbitrage_claim_invalid")
	}
	if err := generated.New(repository.pool).ClaimTriangularArbitrageResources(ctx, write); err != nil {
		return fmt.Errorf("triangular_arbitrage_claim_failed: %w", err)
	}
	return nil
}

// Settle applies exact consumption and optional final release atomically.
func (repository *TriangularArbitrageRepository) Settle(
	ctx context.Context,
	write generated.SettleTriangularArbitrageClaimGroupParams,
) error {
	if write.GroupID == "" || write.ExpectedRevision <= 0 || write.FencingToken <= 0 ||
		...
		!write.RecordedAt.Valid {
		return fmt.Errorf("triangular_arbitrage_claim_settlement_invalid")
	}
	if err := generated.New(repository.pool).SettleTriangularArbitrageClaimGroup(ctx, write); err != nil {
		return fmt.Errorf("triangular_arbitrage_claim_settlement_failed: %w", err)
	}
	return nil
}

// Close releases, expires, or quarantines one active claim group.
func (repository *TriangularArbitrageRepository) Close(
	ctx context.Context,
	write generated.CloseTriangularArbitrageClaimGroupParams,
) error {
	if write.PGroupID == "" || write.PExpectedRevision <= 0 || write.PFencingToken <= 0 ||
		...
		!write.PRecordedAt.Valid {
		return fmt.Errorf("triangular_arbitrage_claim_close_invalid")
	}
	if err := generated.New(repository.pool).CloseTriangularArbitrageClaimGroup(ctx, write); err != nil {
		return fmt.Errorf("triangular_arbitrage_claim_close_failed: %w", err)
	}
	return nil
}

// RecordOutcome commits terminal execution, lifetime, and journal evidence.
func (repository *TriangularArbitrageRepository) RecordOutcome(ctx context.Context, write TriangularArbitrageOutcomeWrite) error {
	if !validTriangularArbitrageOutcomeWrite(write) {
		return fmt.Errorf("triangular_arbitrage_outcome_invalid")
	}
	tx, err := repository.pool.BeginTx(ctx, pgx.TxOptions{})
	if err != nil {
		return fmt.Errorf("triangular_arbitrage_outcome_begin_failed")
	}
	defer func() { _ = tx.Rollback(context.Background()) }()
	queries := generated.New(tx)
	if _, err = queries.InsertTriangularSimulationOutcome(ctx, write.Simulation); err != nil {
		return fmt.Errorf("triangular_arbitrage_simulation_insert_failed")
	}
	if _, err = queries.InsertTriangularOpportunityLifetime(ctx, write.Lifetime); err != nil {
		return fmt.Errorf("triangular_arbitrage_lifetime_insert_failed")
	}
	for _, journal := range write.Journals {
		if _, err = queries.InsertTriangularJournalLink(ctx, journal); err != nil {
			return fmt.Errorf("triangular_arbitrage_journal_link_insert_failed")
		}
	}
	if err = tx.Commit(ctx); err != nil {
		return fmt.Errorf("triangular_arbitrage_outcome_commit_failed")
	}
	return nil
}

// LoadCandidate returns immutable parent and canonical leg order.
func (repository *TriangularArbitrageRepository) LoadCandidate(
	ctx context.Context,
	decisionID string,
) (generated.TriangularCandidate, []generated.TriangularCandidateLeg, error) {
	if decisionID == "" {
		return generated.TriangularCandidate{}, nil, fmt.Errorf("triangular_arbitrage_candidate_invalid")
	}
	queries := generated.New(repository.pool)
	candidate, err := queries.GetTriangularCandidate(ctx, decisionID)
	if err != nil {
		return generated.TriangularCandidate{}, nil, fmt.Errorf("triangular_arbitrage_candidate_load_failed")
	}
...
func validTriangularArbitrageCandidateWrite(write TriangularArbitrageCandidateWrite) bool {
...
func validTriangularArbitrageOutcomeWrite(write TriangularArbitrageOutcomeWrite) bool {
