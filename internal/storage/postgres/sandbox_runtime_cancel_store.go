...
func (store *SandboxRuntimeDispatcherStore) MarkCancelPending(
	ctx context.Context,
	...
	var outboxID string
	err := store.pool.QueryRow(ctx, markSandboxRuntimeCancelPendingSQL,
		account,
		epoch,
		clientOrderID,
		owner,
	...
	if err != nil {
		return "", fmt.Errorf("sandbox_runtime_cancel_pending_failed")
	}
...
func (store *SandboxRuntimeDispatcherStore) MarkCancelUnknown(
	ctx context.Context,
	id string,
	fence uint64,
	now time.Time,
...
WHERE id=$1
  ...
  AND fencing_token=$2`, id, fence, now)
	if err != nil || tag.RowsAffected() != 1 {
		return fmt.Errorf("sandbox_runtime_cancel_unknown_failed")
	}
