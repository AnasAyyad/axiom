...
// V1CEngineAccount is the exact durable account generation an engine owns.
type V1CEngineAccount struct {
...
func (store *V1CDispatcherStore) EnsureAttestedAccount(
	ctx context.Context,
	account V1CEngineAccount,
	registeredAt time.Time,
) (V1CEngineAccount, error) {
	if validateEngineAccountRegistration(account, registeredAt) != nil {
		return V1CEngineAccount{}, fmt.Errorf("v1c_engine_account_invalid")
	}
	...
	if err != nil {
		return V1CEngineAccount{}, fmt.Errorf("v1c_engine_account_begin_failed")
	}
	defer func() { _ = tx.Rollback(context.Background()) }()
	if err = insertAttestedEngineAccount(ctx, tx, account, registeredAt); err != nil {
		return V1CEngineAccount{}, err
	}
	current, err := readV1CEngineAccount(ctx, tx, account.AccountID)
	if err != nil ||
		...
		return V1CEngineAccount{}, fmt.Errorf("v1c_engine_account_conflict")
	}
	if err = tx.Commit(ctx); err != nil {
		return V1CEngineAccount{}, fmt.Errorf("v1c_engine_account_commit_failed")
	}
	return current, nil
}
...
ON CONFLICT DO NOTHING`,
		...
		return fmt.Errorf("v1c_engine_account_insert_failed")
	}
	if _, err := tx.Exec(ctx, `
...
ON CONFLICT DO NOTHING`,
		...
		return fmt.Errorf("v1c_engine_epoch_insert_failed")
	}
	return nil
}
...
func (store *V1CDispatcherStore) RecordValidatedEngineIdentity(
	ctx context.Context,
	identity sandbox.AccountIdentity,
) error {
	if identity.Validate() != nil {
		return fmt.Errorf("v1c_engine_identity_invalid")
	}
	tx, err := store.pool.BeginTx(
		ctx,
		pgx.TxOptions{IsoLevel: pgx.Serializable},
	...
	if err != nil {
		return fmt.Errorf("v1c_engine_identity_begin_failed")
	}
	defer func() { _ = tx.Rollback(context.Background()) }()
	...
	if err = tx.Commit(ctx); err != nil {
		return fmt.Errorf("v1c_engine_identity_commit_failed")
	}
	return nil
}
...
func verifyEngineIdentityAccount(
	...
	current, err := readV1CEngineAccount(ctx, tx, identity.AccountID)
	if err != nil ||
		...
		return fmt.Errorf("v1c_engine_identity_conflict")
	}
	return nil
}
...
ON CONFLICT DO NOTHING`,
		...
		return fmt.Errorf("v1c_engine_identity_insert_failed")
	}
	var same bool
	if err := tx.QueryRow(ctx, `
SELECT key_fingerprint=$3 AND account_identity_hash=$4
FROM v1c_credential_generations
...
func (store *V1CDispatcherStore) SetEngineAccountState(
	ctx context.Context,
	...
	if account == "" ||
		...
		return fmt.Errorf("v1c_engine_state_invalid")
	}
...
WHERE id=$1`,
	...
	if err != nil || tag.RowsAffected() != 1 {
		return fmt.Errorf("v1c_engine_state_update_failed")
	}
	return nil
}

// RenewAccountLease extends only the exact live owner/fencing token.
func (store *V1CDispatcherStore) RenewAccountLease(
	ctx context.Context,
	account sandbox.AccountID,
	owner string,
	fence uint64,
	...
	if account == "" || owner == "" || fence == 0 ||
		...
		return fmt.Errorf("v1c_lease_renew_invalid")
	}
	tag, err := store.pool.Exec(ctx, `
UPDATE v1c_account_leases
SET expires_at=$5
WHERE account_id=$1 AND owner=$2 AND fencing_token=$3
  AND expires_at>$4`,
	...
	if err != nil || tag.RowsAffected() != 1 {
		return fmt.Errorf("v1c_lease_renew_failed")
	}
	return nil
}
...
func validateEngineAccountRegistration(
	...
	if account.AccountID == "" ||
		...
		return fmt.Errorf("v1c_engine_account_invalid")
	}
...
type v1CEngineAccountRower interface {
	QueryRow(context.Context, string, ...any) pgx.Row
}

func readV1CEngineAccount(
	ctx context.Context,
	rower v1CEngineAccountRower,
	account sandbox.AccountID,
) (V1CEngineAccount, error) {
	var result V1CEngineAccount
...
WHERE id=$1 FOR UPDATE`,
	...
	if err != nil || epoch <= 0 || generation <= 0 {
		return V1CEngineAccount{}, fmt.Errorf("v1c_engine_account_read_failed")
	}
...
// V1CEngineStartupEvidenceSink durably appends one exact startup cycle.
type V1CEngineStartupEvidenceSink struct {
...
// NewV1CEngineStartupEvidenceSink binds one immutable evidence sink to an
// account, exchange, and fencing cycle.
func NewV1CEngineStartupEvidenceSink(
	store *V1CDispatcherStore,
	account sandbox.AccountID,
	exchange sandbox.Exchange,
	cycle uint64,
) (*V1CEngineStartupEvidenceSink, error) {
	if store == nil || account == "" || cycle == 0 ||
		(exchange != sandbox.ExchangeBinance &&
			exchange != sandbox.ExchangeBybit) {
		return nil, fmt.Errorf("v1c_startup_evidence_sink_invalid")
	}
	return &V1CEngineStartupEvidenceSink{
		store:    store,
		account:  account,
		exchange: exchange,
		cycle:    cycle,
...
func (sink *V1CEngineStartupEvidenceSink) AppendCollectorLifecycle(
	...
	if event.Exchange != string(sink.exchange) ||
		...
		return fmt.Errorf("v1c_startup_evidence_invalid")
	}
	encoded, err := json.Marshal(event)
	if err != nil {
		return fmt.Errorf("v1c_startup_evidence_encode_failed")
	}
	digest := sha256.Sum256(encoded)
	hash := hex.EncodeToString(digest[:])
	id := "v1c-startup-" + hash[:32]
...
func (sink *V1CEngineStartupEvidenceSink) insertStartupEvidence(
	ctx context.Context,
	event exchangecontracts.CollectorLifecycleEvidence,
	id, hash string,
) (bool, error) {
	tag, err := sink.store.pool.Exec(ctx, `
INSERT INTO v1c_engine_startup_evidence(
...
ON CONFLICT DO NOTHING`,
	...
	if err != nil {
		return false, fmt.Errorf("v1c_startup_evidence_insert_failed")
	}
	return tag.RowsAffected() == 1, nil
}

func (sink *V1CEngineStartupEvidenceSink) verifyStartupEvidence(
	ctx context.Context,
	event exchangecontracts.CollectorLifecycleEvidence,
	id, hash string,
) error {
...
FROM v1c_engine_startup_evidence
WHERE account_id=$2 AND startup_cycle=$4 AND stage=$5`,
	...
	if err != nil || !same {
		return fmt.Errorf("v1c_startup_evidence_conflict")
	}
	return nil
}

var _ exchangecontracts.LifecycleEvidenceSink = (*V1CEngineStartupEvidenceSink)(nil)
