...
const multiExchangeConsoleOpportunitiesCTE = `WITH opportunities AS (
...
const multiExchangeConsoleOpportunitiesPageSQL = multiExchangeConsoleOpportunitiesCTE + `
...
const multiExchangeConsoleOpportunityByIDSQL = multiExchangeConsoleOpportunitiesCTE + ` WHERE id=$1`

const multiExchangeConsoleTriangularLegsSQL = `SELECT leg.leg_index,candidate.exchange_id,leg.instrument_id,leg.side,
...
const multiExchangeConsoleCrossExchangeLegsSQL = `SELECT leg.leg_index,leg.exchange_id,leg.instrument_id,leg.side,
  leg.input_quantity::text,leg.trade_quantity::text,leg.gross_output::text,
  leg.net_output::text,leg.fee_asset,leg.fee_quantity::text,
  leg.fee_quote_equivalent::text,leg.vwap::text,leg.spread_depth_cost::text,
...
type multiExchangeConsoleOpportunityRow struct {
...
func (store *OwnerConsoleStore) Opportunities(
	...
	var err error
	if cursor != "" {
		cursorTime, cursorID, err = decodeMultiExchangeConsoleTimeCursor(store.cursor, "multi_exchange_console-opportunities-"+kind, cursor)
		if err != nil {
			return generated.OpportunityPage{}, err
		}
	}
	rows, err := store.pool.Query(ctx, multiExchangeConsoleOpportunitiesPageSQL,
		kind, nullableMultiExchangeConsoleTime(cursorTime), cursorID, limit+1)
	if err != nil {
		return generated.OpportunityPage{}, err
	}
	defer rows.Close()
	...
	for rows.Next() {
		var row multiExchangeConsoleOpportunityRow
		...
		items = append(items, multiExchangeConsoleOpportunitySummary(row, now))
	}
	...
	if len(items) > limit {
		...
		next := encodeMultiExchangeConsoleTimeCursor(store.cursor, "multi_exchange_console-opportunities-"+kind, last.RecordedAt, last.Id)
		page.NextCursor = &next
	}
	return page, nil
}

func nullableMultiExchangeConsoleTime(value time.Time) any {
	if value.IsZero() {
		return nil
	}
	return value
...
func multiExchangeConsoleOpportunitySummary(row multiExchangeConsoleOpportunityRow, now time.Time) generated.OpportunitySummary {
	...
	result := generated.OpportunitySummary{
		...
		Quality: multiExchangeConsoleQuality("immutable_simulation_evidence", row.recordedAt,
	...
	age := generated.Revision(multiExchangeConsolePositiveAge(now, row.recordedAt))
...
func (store *OwnerConsoleStore) Opportunity(ctx context.Context, id string) (generated.OpportunityDetail, error) {
	summary, kind, err := store.multiExchangeConsoleOpportunityByID(ctx, id)
	if err != nil {
		return generated.OpportunityDetail{}, err
	}
	legs, err := store.multiExchangeConsoleOpportunityLegs(ctx, id, kind)
	if err != nil {
		return generated.OpportunityDetail{}, err
	}
	inventory, err := store.multiExchangeConsoleOpportunityInventory(ctx, id, kind)
	if err != nil {
		return generated.OpportunityDetail{}, err
	}
	recovery, costs, outcomeAt, outcomeCorrelation, err := store.multiExchangeConsoleOpportunityOutcome(ctx, id, kind)
	if err != nil {
		return generated.OpportunityDetail{}, err
	}
...
func (store *OwnerConsoleStore) multiExchangeConsoleOpportunityByID(
	ctx context.Context, id string,
) (generated.OpportunitySummary, string, error) {
	var row multiExchangeConsoleOpportunityRow
	err := store.pool.QueryRow(ctx, multiExchangeConsoleOpportunityByIDSQL, id).Scan(&row.id, &row.kind, &row.strategy,
	...
	return multiExchangeConsoleOpportunitySummary(row, store.clock.Now().UTC), row.kind, nil
}

func (store *OwnerConsoleStore) multiExchangeConsoleOpportunityLegs(
	ctx context.Context, id, kind string,
) ([]generated.OpportunityLeg, error) {
	query := multiExchangeConsoleTriangularLegsSQL
	if kind == "cross_exchange" {
		query = multiExchangeConsoleCrossExchangeLegsSQL
	}
...
func (store *OwnerConsoleStore) multiExchangeConsoleOpportunityInventory(
	ctx context.Context, id, kind string,
...
func (store *OwnerConsoleStore) multiExchangeConsoleOpportunityOutcome(
	ctx context.Context, id, kind string,
) (generated.RecoveryAnalysis, map[string]string, time.Time, string, error) {
	if kind == "triangular" {
		return store.multiExchangeConsoleTriangularOpportunityOutcome(ctx, id)
	}
	return store.multiExchangeConsoleCrossExchangeOpportunityOutcome(ctx, id)
}

func (store *OwnerConsoleStore) multiExchangeConsoleTriangularOpportunityOutcome(
	ctx context.Context, id string,
) (generated.RecoveryAnalysis, map[string]string, time.Time, string, error) {
...
func (store *OwnerConsoleStore) multiExchangeConsoleCrossExchangeOpportunityOutcome(
	ctx context.Context, id string,
) (generated.RecoveryAnalysis, map[string]string, time.Time, string, error) {
	recovery := generated.RecoveryAnalysis{Disposition: "not_simulated",
		Explanation: "No immutable simulation outcome has been recorded.", RecoveryLoss: "0"}
