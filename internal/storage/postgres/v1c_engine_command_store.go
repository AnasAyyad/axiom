...
const v1CEngineCommandClaimTTL = 30 * time.Second

// QueueEngineCommand appends an idempotent query, cancel, or reconcile request.
func (store *V1CDispatcherStore) QueueEngineCommand(
	ctx context.Context,
	...
	if command.Validate() != nil {
		return fmt.Errorf("v1c_engine_command_invalid")
	}
	tag, err := store.pool.Exec(ctx, `
INSERT INTO v1c_engine_commands(
...
ON CONFLICT (id) DO NOTHING`,
	...
	if err != nil {
		return fmt.Errorf("v1c_engine_command_insert_failed")
	}
...
WHERE id=$1`,
	...
	if err != nil || !same {
		return fmt.Errorf("v1c_engine_command_identity_conflict")
	}
	return nil
}

// ClaimEngineCommands fences a bounded page to the only live account owner.
func (store *V1CDispatcherStore) ClaimEngineCommands(
	ctx context.Context,
	...
	if account == "" || epoch == 0 || owner == "" || fence == 0 ||
		...
		limit < 1 || limit > 16 {
		return nil, fmt.Errorf("v1c_engine_command_claim_invalid")
	}
	...
	if err != nil {
		return nil, fmt.Errorf("v1c_engine_command_claim_begin_failed")
	}
	...
	if err = tx.Commit(ctx); err != nil {
		return nil, fmt.Errorf("v1c_engine_command_claim_commit_failed")
	}
	return commands, nil
}
...
SELECT EXISTS(
  ...
  WHERE account_id=$1 AND owner=$2 AND fencing_token=$3
    AND expires_at>$4
		...
		return fmt.Errorf("v1c_engine_command_claim_fence_invalid")
	}
	return nil
}
...
UPDATE v1c_engine_commands command
...
RETURNING command.id,command.account_id,command.account_epoch,
		...
		owner,
		fence,
		now.Add(v1CEngineCommandClaimTTL),
	)
	if err != nil {
		return nil, fmt.Errorf("v1c_engine_command_claim_failed")
	}
	return rows, nil
}
...
func (store *V1CDispatcherStore) CompleteEngineCommand(
	ctx context.Context,
	id string,
	fence uint64,
	succeeded bool,
	...
	if id == "" || fence == 0 || len(evidenceHash) != 64 ||
		now.IsZero() || now.Location() != time.UTC {
		return fmt.Errorf("v1c_engine_command_complete_invalid")
	}
...
WHERE command.id=$1
  ...
  AND EXISTS(
	...
	if err != nil || tag.RowsAffected() != 1 {
		return fmt.Errorf("v1c_engine_command_complete_failed")
	}
	return nil
}
