...
// B6FactSetWrite is one immutable reviewed fact set and every versioned route
// fact it contains.
type B6FactSetWrite struct {
	FactSet generated.InsertRebalancingFactSetParams
	Facts   []generated.InsertRebalancingRouteFactParams
}

// B6RecommendationWrite is one complete advisory recommendation, its selected
// provenance links, and the explicit operator checklist.
type B6RecommendationWrite struct {
...
// B6Repository owns immutable B6 reviewed-fact and advisory evidence.
type B6Repository struct{ pool *pgxpool.Pool }

// NewB6Repository constructs the B6 persistence boundary.
func NewB6Repository(pool *pgxpool.Pool) (*B6Repository, error) {
	if pool == nil {
		return nil, fmt.Errorf("b6_repository_pool_missing")
	}
	return &B6Repository{pool: pool}, nil
}

// RecordFactSet atomically commits one immutable reviewed fact graph.
func (repository *B6Repository) RecordFactSet(ctx context.Context, write B6FactSetWrite) error {
	if !validB6FactSetWrite(write) {
		return fmt.Errorf("b6_fact_set_invalid")
	}
	tx, err := repository.pool.BeginTx(ctx, pgx.TxOptions{})
	if err != nil {
		return fmt.Errorf("b6_fact_set_begin_failed")
	}
	defer func() { _ = tx.Rollback(context.Background()) }()
	queries := generated.New(tx)
	if _, err = queries.InsertRebalancingFactSet(ctx, write.FactSet); err != nil {
		return fmt.Errorf("b6_fact_set_insert_failed: %w", err)
	}
	for _, fact := range write.Facts {
		if _, err = queries.InsertRebalancingRouteFact(ctx, fact); err != nil {
			return fmt.Errorf("b6_route_fact_insert_failed: %w", err)
		}
	}
	if err = tx.Commit(ctx); err != nil {
		return fmt.Errorf("b6_fact_set_commit_failed: %w", err)
	}
	return nil
}

// RecordRecommendation atomically commits one complete advisory result.
func (repository *B6Repository) RecordRecommendation(
	ctx context.Context,
	write B6RecommendationWrite,
) error {
	if !validB6RecommendationWrite(write) {
		return fmt.Errorf("b6_recommendation_invalid")
	}
	tx, err := repository.pool.BeginTx(ctx, pgx.TxOptions{})
	if err != nil {
		return fmt.Errorf("b6_recommendation_begin_failed")
	}
	defer func() { _ = tx.Rollback(context.Background()) }()
	queries := generated.New(tx)
	if _, err = queries.InsertRebalancingRecommendation(ctx, write.Recommendation); err != nil {
		return fmt.Errorf("b6_recommendation_insert_failed: %w", err)
	}
	for _, step := range write.Steps {
		if _, err = queries.InsertRebalancingRecommendationStep(ctx, step); err != nil {
			return fmt.Errorf("b6_recommendation_step_insert_failed: %w", err)
		}
	}
	for _, step := range write.Checklist {
		if _, err = queries.InsertRebalancingChecklistStep(ctx, step); err != nil {
			return fmt.Errorf("b6_checklist_step_insert_failed: %w", err)
		}
	}
	if err = tx.Commit(ctx); err != nil {
		return fmt.Errorf("b6_recommendation_commit_failed: %w", err)
	}
	return nil
}

// LoadFactSet returns one immutable fact-set aggregate.
func (repository *B6Repository) LoadFactSet(
	ctx context.Context,
	id string,
) (generated.RebalancingFactSet, []generated.RebalancingRouteFact, error) {
	if id == "" {
		return generated.RebalancingFactSet{}, nil, fmt.Errorf("b6_fact_set_invalid")
	}
	queries := generated.New(repository.pool)
	factSet, err := queries.GetRebalancingFactSet(ctx, id)
	if err != nil {
		return generated.RebalancingFactSet{}, nil, fmt.Errorf("b6_fact_set_load_failed")
	}
...
func (repository *B6Repository) LoadRecommendation(
	ctx context.Context,
	id string,
) (
	...
	if id == "" {
		return generated.RebalancingRecommendation{}, nil, nil, fmt.Errorf("b6_recommendation_invalid")
	}
	queries := generated.New(repository.pool)
	recommendation, err := queries.GetRebalancingRecommendation(ctx, id)
	if err != nil {
		return generated.RebalancingRecommendation{}, nil, nil, fmt.Errorf("b6_recommendation_load_failed")
	}
...
func validB6FactSetWrite(write B6FactSetWrite) bool {
...
func validB6RecommendationWrite(write B6RecommendationWrite) bool {
	...
	if (recommendation.Method == "natural_reverse_arbitrage") !=
		(recommendation.SourceB5DecisionID != nil) ||
