...
const sandboxRuntimeConsoleReconciliationsSQL = `
...
FROM sandbox_runtime_reconciliations reconciliation
...
func (store *OwnerConsoleStore) SandboxReconciliations(
	ctx context.Context,
	cursor string,
	limit int,
	...
	cursorTime, cursorID, _, err := decodeOwnerConsoleTimeCursor(
		store.cursor, sandboxRuntimeConsoleReconciliationScope, cursor,
	)
	if err != nil {
		return generated.SandboxReconciliationPage{}, err
	}
	rows, err := store.pool.Query(
		ctx, sandboxRuntimeConsoleReconciliationsSQL,
	...
	defer rows.Close()
	items, err := store.scanSandboxRuntimeConsoleReconciliations(ctx, rows, limit+1)
	if err != nil {
		return generated.SandboxReconciliationPage{}, err
	}
	resets, err := store.sandboxRuntimeConsoleResetIncidents(ctx, exchangeFilter)
	if err != nil {
		return generated.SandboxReconciliationPage{}, err
	}
	return store.sandboxQualificationReconciliationPage(items, resets, limit), nil
}

func (store *OwnerConsoleStore) scanSandboxRuntimeConsoleReconciliations(
	ctx context.Context,
	rows pgx.Rows,
	capacity int,
	...
	for rows.Next() {
		...
		item.AuditUrl = "/api/v1/audit-events?event_type=sandbox_runtime_reconciliation"
		differences, err := store.sandboxRuntimeConsoleDifferences(ctx, item.Id)
		if err != nil {
			return nil, err
		}
		item.Differences = differences
		countSandboxQualificationDifferenceStates(&item)
		items = append(items, item)
	}
	return items, rows.Err()
}

func countSandboxQualificationDifferenceStates(item *generated.SandboxReconciliation) {
...
func (store *OwnerConsoleStore) sandboxQualificationReconciliationPage(
	...
	if hasMore && len(items) > 0 {
		...
		value := encodeOwnerConsoleTimeCursor(
			store.cursor, sandboxRuntimeConsoleReconciliationScope,
...
func (store *OwnerConsoleStore) sandboxRuntimeConsoleDifferences(
	ctx context.Context,
...
FROM sandbox_runtime_reconciliation_differences
...
ORDER BY recorded_at,id`, reconciliationID)
	...
	for rows.Next() {
		...
		item.AuditUrl = "/api/v1/audit-events?event_type=sandbox_runtime_difference"
		result = append(result, item)
	}
	return result, rows.Err()
}

func (store *OwnerConsoleStore) sandboxRuntimeConsoleResetIncidents(
	ctx context.Context,
	exchangeFilter string,
...
FROM sandbox_runtime_reset_incidents incident
JOIN sandbox_runtime_exchange_accounts account ON account.id=incident.account_id
WHERE ($1='' OR account.exchange=$1)
...
LIMIT 50`, exchangeFilter)
	...
	for rows.Next() {
		...
		item.AuditUrl = "/api/v1/audit-events?event_type=sandbox_runtime_reset"
		item.Adjustments, err = store.sandboxRuntimeConsoleAdjustments(ctx, item.Id)
		if err != nil {
			return nil, err
		}
		result = append(result, item)
...
func (store *OwnerConsoleStore) sandboxRuntimeConsoleAdjustments(
	ctx context.Context,
	incidentID string,
) ([]struct {
...
FROM sandbox_runtime_external_adjustments
