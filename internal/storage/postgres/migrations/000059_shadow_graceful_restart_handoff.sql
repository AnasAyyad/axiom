...
-- A graceful restart may return only a checkpointed, entries-disabled shadow
-- session to the queue. The complete prior lease tuple remains as immutable
-- fencing evidence until the next claim atomically advances its epoch.
  ...
  IF NEW.state <> OLD.state THEN
    IF NEW.revision <> OLD.revision + 1 OR NOT (
      ...
      (OLD.state IN ('PAUSED','RUNNING') AND NEW.state='QUEUED' AND NOT NEW.entries_enabled AND
       ...
       EXISTS (SELECT 1 FROM run_checkpoints checkpoint WHERE checkpoint.run_id=OLD.run_id))
