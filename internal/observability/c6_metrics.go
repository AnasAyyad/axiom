...
func (metrics *Metrics) RecordSandboxOrder(exchange, state string) error {
	if !slices.Contains(metrics.catalog.Exchanges, exchange) ||
		!slices.Contains(c6OrderStates, state) {
...
// SetC6Soak publishes a single bounded qualification state.
func (metrics *Metrics) SetC6Soak(
	mode, state string,
	duration time.Duration,
) error {
	if !slices.Contains([]string{"smoke", "formal"}, mode) ||
		...
		return fmt.Errorf("metric_value_rejected:c6_soak")
	}
	metrics.c6SoakState.WithLabelValues(mode, state).Set(1)
	metrics.c6SoakDuration.WithLabelValues(mode).Set(duration.Seconds())
	return nil
}

// RecordC6Failure records one closed qualification failure.
func (metrics *Metrics) RecordC6Failure(reason string) error {
	if !slices.Contains(c6FailureReasons, reason) {
...
// SetC6MemoryTrend records the signed resident-memory delta after warm-up.
func (metrics *Metrics) SetC6MemoryTrend(window string, deltaBytes int64) error {
	if !slices.Contains([]string{"15m", "1h", "run"}, window) {
		return fmt.Errorf("metric_label_rejected:c6_memory_window")
	}
	metrics.c6MemoryTrend.WithLabelValues(window).Set(float64(deltaBytes))
	return nil
}
