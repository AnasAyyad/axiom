...
func qualificationSourceCommit() (string, error) {
	return qualificationSourceCommitFromEnv("AXIOM_PUBLIC_DATA_SOURCE_COMMIT")
}
...
func captureSoakStatus(
	...
	return soakStatus{SchemaVersion: "axiom.public-data-soak-status.v4", SourceCommit: evidence.SourceCommit,
...
func writeSoakStatus(root string, status soakStatus) error {
	return writeAtomicJSON(filepath.Join(root, "public-data-soak-status.json"), status)
}
