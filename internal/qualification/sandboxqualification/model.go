...
// FormalDuration, AlertSLO, and RecoveryRTO define the immutable formal SandboxQualification
...
// sandbox qualification qualification modes.
const (
	ModeSmoke  Mode = "smoke"
	ModeFormal Mode = "formal"
)

// TerminalState is the immutable outcome recorded in sandbox qualification evidence.
type TerminalState string

// sandbox qualification terminal evidence states.
const (
...
// Identity binds a sandbox qualification run to exact source, build, executable, image,
...
// Config contains the explicit, default-off sandbox qualification runner configuration.
type Config struct {
	Enabled        bool
	Identity       Identity
...
// Sample is one bounded, redacted observation of sandbox qualification safety and SLO state.
type Sample struct {
...
// AccountObservation is the redacted per-account qualification recovery projection.
...
// immutable binding to the formal qualification run. It contains no raw exchange data.
...
// RecoveryEvent is one immutable, redacted qualification recovery lifecycle fact.
...
// Evidence is the immutable terminal sandbox qualification qualification document.
...
// otherwise non-canonical sandbox qualification runner configurations.
func ValidateConfig(configuration Config) error {
	if !configuration.Enabled || configuration.Identity.RunID == "" ||
		...
		return fmt.Errorf("sandbox_qualification_configuration_rejected")
	}
	...
	case ModeFormal:
		if configuration.Duration != FormalDuration ||
			...
			!imagePattern.MatchString(configuration.Identity.ImageHash) {
			return fmt.Errorf("sandbox_qualification_formal_configuration_rejected")
		}
	case ModeSmoke:
		if configuration.Duration < time.Second ||
			configuration.Duration > 15*time.Minute {
			return fmt.Errorf("sandbox_qualification_smoke_configuration_rejected")
		}
	default:
		return fmt.Errorf("sandbox_qualification_mode_rejected")
	}
	if len(configuration.Identity.Accounts) != 2 {
		return fmt.Errorf("sandbox_qualification_account_set_rejected")
	}
