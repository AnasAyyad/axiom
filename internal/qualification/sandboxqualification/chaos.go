...
// RequiredChaosScenarios is the exact closed sandbox qualification deterministic fault set.
...
func RunDeterministicChaos(
	...
	if harness == nil || seed == "" || now.IsZero() ||
		...
		return nil, fmt.Errorf("sandbox_qualification_chaos_rejected")
	}
	...
	for index, scenario := range RequiredChaosScenarios {
		...
		if err != nil || fact == "" {
			return nil, fmt.Errorf("sandbox_qualification_chaos_harness_failed:%s", scenario)
		}
		outcome := "FAILED"
		if passed {
			outcome = "PASSED"
...
func ValidateChaosEvidence(events []ChaosEvent) error {
	if len(events) != len(RequiredChaosScenarios) {
		return fmt.Errorf("sandbox_qualification_chaos_set_incomplete")
	}
	...
	for _, event := range events {
		if !slices.Contains(RequiredChaosScenarios, event.Scenario) ||
			...
			return fmt.Errorf("sandbox_qualification_chaos_evidence_invalid")
		}
