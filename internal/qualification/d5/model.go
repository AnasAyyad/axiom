// Package d5 implements the separate authenticated V1D seven-day readiness qualification.
package d5
...
// Immutable D5 qualification durations and service-level thresholds.
const (
	// FormalDuration is the exact uninterrupted D5 qualification clock.
...
// Supported D5 qualification modes.
const (
	// ModeSmoke can test runner mechanics but can never qualify D5.
...
// Closed terminal verdict states written by the D5 runner.
const (
...
// DeclaredLoad proves the official run exercises the full D5 platform surface.
...
// Evidence is the immutable authenticated D5 terminal verdict.
...
func ValidateConfig(configuration Config) error {
	...
	if !configuration.Enabled || identity.RunID == "" || !commitPattern.MatchString(identity.SourceSHA) ||
		...
		return fmt.Errorf("d5_configuration_rejected")
	}
	...
	case ModeFormal:
		if configuration.Duration != FormalDuration || identity.SourceDirty ||
			...
			return fmt.Errorf("d5_formal_configuration_rejected")
		}
	case ModeSmoke:
		if configuration.Duration < time.Second || configuration.Duration > MaximumSmokeDuration ||
			...
			return fmt.Errorf("d5_smoke_configuration_rejected")
		}
	default:
		return fmt.Errorf("d5_mode_rejected")
	}
	return nil
}
...
func validateImages(images map[string]string) error {
	if len(images) != len(requiredImages) {
		return fmt.Errorf("d5_image_set_rejected")
	}
...
func validateSchedule(schedule FaultSchedule, duration time.Duration) error {
	if schedule.SchemaVersion != "axiom.d5.fault-schedule.v1" || len(schedule.Faults) < 5 || duration <= 0 {
		return fmt.Errorf("d5_fault_schedule_rejected")
	}
