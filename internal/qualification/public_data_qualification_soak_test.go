...
func TestPublicDataQualificationContinuous72HourPublicSoak(t *testing.T) {
	if os.Getenv("AXIOM_PUBLIC_DATA_SOAK") != "1" {
		t.Skip("set AXIOM_PUBLIC_DATA_SOAK=1 and AXIOM_PUBLIC_DATA_SOAK_OUTPUT to run the formal 72-hour gate")
	}
	root := os.Getenv("AXIOM_PUBLIC_DATA_SOAK_OUTPUT")
	if !filepath.IsAbs(root) || filepath.Clean(root) == string(filepath.Separator) {
		t.Fatal("AXIOM_PUBLIC_DATA_SOAK_OUTPUT must be a dedicated absolute directory")
	}
	runPublicDataQualificationSoak(t, root, formalSoakDuration, formalFlushEvery, formalSampleEvery, true)
}

func TestPublicDataQualificationPublicSoakHarnessSmoke(t *testing.T) {
	if os.Getenv("AXIOM_PUBLIC_DATA_SOAK_SMOKE") != "1" {
		t.Skip("set AXIOM_PUBLIC_DATA_SOAK_SMOKE=1 to exercise the public qualification harness")
	}
	runPublicDataQualificationSoak(t, t.TempDir(), 20*time.Second, 5*time.Second, 2*time.Second, false)
}
...
func runPublicDataQualificationSoak(t *testing.T, root string, duration, flushEvery, sampleEvery time.Duration, formal bool) {
	t.Helper()
	...
	if err != nil {
		...
		t.Fatal("public-data qualification qualification event journal could not be created")
	}
	if !appendQualificationEvent(journal, &evidence, qualificationEvent{RecordedAt: started,
		Phase: "preflight", Outcome: "passed"}) {
		_ = journal.Close()
		t.Fatal("public-data qualification qualification event journal preflight failed")
	}
	components := newSoakComponents(t, root, qualificationLifecycleSink{journal: journal})
	if !appendQualificationEvent(journal, &evidence, qualificationEvent{Phase: "startup", Outcome: "passed"}) {
		_ = journal.Close()
		t.Fatal("public-data qualification qualification startup evidence failed")
	}
	...
	monitorFailure := monitorSoakFailClosedUntil(ctx, officialEnd.C, root, components.client, components.recorder,
	...
	finishPublicDataQualificationSoak(t, root, sourceCommit, started, formal, components, manifest, &evidence, journal)
}
...
func finishPublicDataQualificationSoak(
	t *testing.T,
	root string,
	sourceCommit string,
	started time.Time,
	...
	finalizePublicDataQualificationJournal(sourceCommit, formal, manifest.Revision, evidence, journal)
	...
	if !evidence.Qualified {
		t.Fatalf("public-data qualification public soak did not qualify: %v", evidence.Failures)
	}
}

func finalizePublicDataQualificationJournal(
	sourceCommit string,
	formal bool,
	...
	appendQualificationEvent(journal, evidence, qualificationEvent{
...
func newSoakComponents(t *testing.T, root string,
	lifecycleSink exchangecontracts.LifecycleEvidenceSink) soakComponents {
	t.Helper()
	clock := &domain.SystemClock{}
	client, err := binance.NewRecorderPublicClientWithMonotonic("market-data-only-v1", clock,
		exchangecontracts.NewProcessMonotonicSource())
	if err != nil {
		t.Fatal(err)
	}
	streamRecorder, err := recorder.New(root, "public-data-public-soak", "public-data-public-soak", "binance",
...
func newSoakEvidence(started time.Time, flushEvery time.Duration, formal bool, sourceCommit, root string) soakEvidence {
	return soakEvidence{SchemaVersion: "axiom.public-data-soak.v5", SourceCommit: sourceCommit,
		...
		EventJournal:     qualificationJournalEvidence{Path: "public-data-soak-events.jsonl"}, root: root}
}
...
func completeSoakEvidence(
	...
	for symbol, collector := range collectors {
		...
		appendFormalCollectorFailures(evidence, symbol, sloPassed, book.Eligible)
	}
...
func writeSoakEvidence(root string, evidence soakEvidence) error {
	return writeAtomicJSON(filepath.Join(root, "public-data-soak-evidence.json"), evidence)
}
