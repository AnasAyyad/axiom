...
func TestCoherentMarketDataProductionPublicRecordOnlyAndCoherentQualification(t *testing.T) {
	if os.Getenv("AXIOM_COHERENT_MARKET_DATA_LIVE_PUBLIC") != "1" {
		t.Skip("AXIOM_COHERENT_MARKET_DATA_LIVE_PUBLIC=1 is required")
	}
	...
	joined, joinErr := views.CoherentAsOf([]runtimecore.MarketKey{
		...
		UTC: time.Now().UTC()}, runtimecore.InitialCoherentMarketDataCoherentPolicy())
	path, tierA, records := persistLiveTierA(t, qualification)
	t.Logf("COHERENT_MARKET_DATA_LIVE_DATASET_EVIDENCE root=%s manifest=%s tier_a_hash=%s records=%d",
		qualification.root, filepath.Base(path), tierA.Hash, records)
	if joinErr != nil {
		t.Fatalf("live coherent view rejected: %v", joinErr)
	}
	t.Logf("COHERENT_MARKET_DATA_LIVE_QUALIFICATION root=%s manifest=%s coherent_view=%s records=%d",
...
func newLiveQualification(t *testing.T) liveQualification {
	t.Helper()
	evidenceRoot := os.Getenv("AXIOM_COHERENT_MARKET_DATA_LIVE_EVIDENCE_ROOT")
	if evidenceRoot == "" {
	...
	session := fmt.Sprintf("coherent_market_data-live-%d", time.Now().UTC().UnixNano())
	qualification := liveQualification{root: filepath.Join(evidenceRoot, session),
		profile: recorder.CollectorProfile{Instance: "coherent_market_data-live-collector", Region: region,
	...
	qualification.binanceRecorder = liveCoherentMarketDataRecorder(t, filepath.Join(qualification.root, "binance"),
		"binance-coherent_market_data-live", session+"-binance", "binance", ordinals, qualification.profile)
	qualification.bybitRecorder = liveCoherentMarketDataRecorder(t, filepath.Join(qualification.root, "bybit"),
		"bybit-coherent_market_data-live", session+"-bybit", "bybit", ordinals, qualification.profile)
	var err error
...
func persistLiveTierA(t *testing.T, qualification liveQualification) (string, recorder.TierAManifest, uint64) {
	...
	tierA, err := recorder.BuildTierAManifest("coherent_market_data-live-tier-a", time.Now().UTC(),
	...
	go func() {
		...
		binanceResult.health, binanceResult.samples, binanceResult.err = sampleBoundedClock(ctx, 10, 10, func() (exchangecontracts.ClockHealth, error) {
			health, _, sampleErr := binanceClient.SampleServerTimeRecorded(ctx,
				instrument, "coherent_market_data-live-binance", 1, binanceSink)
	...
	go func() {
		...
		bybitResult.health, bybitResult.samples, bybitResult.err = sampleBoundedClock(ctx, 5, 4, func() (exchangecontracts.ClockHealth, error) {
			health, _, sampleErr := bybitClient.SampleServerTimeRecorded(ctx,
				instrument, "coherent_market_data-live-bybit", 1, bybitSink)
			return health, sampleErr
		})
	}()
	group.Wait()
...
func sampleLiveSnapshots(
	...
	go func() {
		...
		binanceResult.snapshot, binanceResult.token, binanceResult.err = binanceClient.SnapshotRecorded(ctx,
			exchangecontracts.SnapshotRequest{Instrument: instrument, Depth: 100}, "coherent_market_data-live-binance", 1, binanceSink)
	}()
	go func() {
		defer group.Done()
		bybitResult.snapshot, bybitResult.token, bybitResult.err = bybitClient.SnapshotRecorded(ctx,
			exchangecontracts.SnapshotRequest{Instrument: instrument, Depth: 50}, "coherent_market_data-live-bybit", 1, bybitSink)
	}()
	group.Wait()
}
...
func liveCoherentInput(
	...
	if err != nil || book.BeginGeneration("coherent_market_data-live-"+evidence.exchange, 1) != nil {
		t.Fatal(err)
	}
	...
	observation := marketdata.Observation{
		...
		ConnectionID: "coherent_market_data-live-" + evidence.exchange, ConnectionGeneration: 1,
...
func liveCoherentMarketDataRecorder(
	t *testing.T,
	...
	t.Helper()
	value, err := recorder.NewCoherentMarketData(root, datasetID, sessionID, exchange, ordinals,
