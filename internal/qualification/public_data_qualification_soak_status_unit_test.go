...
func TestQualificationSourceCommitIsExactAndSanitized(t *testing.T) {
	t.Setenv("AXIOM_PUBLIC_DATA_SOURCE_COMMIT", testSourceCommit)
	commit, err := qualificationSourceCommit()
	if err != nil || commit != testSourceCommit {
		t.Fatalf("commit=%q err=%v", commit, err)
	}
	t.Setenv("AXIOM_PUBLIC_DATA_SOURCE_COMMIT", "not-a-commit")
...
func TestWriteSoakStatusAtomicallyReplacesOneFile(t *testing.T) {
	root := t.TempDir()
	status := soakStatus{SchemaVersion: "axiom.public-data-soak-status.v4", SourceCommit: testSourceCommit,
		ManifestRevision: 1}
	...
	payload, err := os.ReadFile(filepath.Join(root, "public-data-soak-status.json"))
	if err != nil {
		t.Fatal(err)
	}
	...
	temporary, err := filepath.Glob(filepath.Join(root, ".public-data-soak-status.json.*.tmp"))
...
func TestWriteSoakEvidenceUsesAtomicTerminalReport(t *testing.T) {
	...
	payload, err := os.ReadFile(filepath.Join(root, "public-data-soak-evidence.json"))
	if err != nil {
		t.Fatal(err)
	}
	...
	if decoded.SchemaVersion != "axiom.public-data-soak.v5" {
