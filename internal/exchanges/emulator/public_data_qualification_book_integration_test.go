...
func TestPublicDataQualificationEmulatorSnapshotBridgeAndGapInvalidation(t *testing.T) {
	...
	scenario := Scenario{Name: "public-data-book", Seed: "public-data-book-v1",
	...
	exercisePublicDataQualificationBookScenario(t, adapter, server, clock, instrument)
}

func exercisePublicDataQualificationBookScenario(
	t *testing.T,
	adapter *Adapter,
	server *Server,
	clock domain.Clock,
	...
	assertPublicDataQualificationGapInvalidates(t, stream, book, server, clock)
}

func assertPublicDataQualificationGapInvalidates(
	t *testing.T,
	...
	if !server.Complete() {
		t.Fatal("public-data qualification emulator scenario was not fully consumed")
	}
}
