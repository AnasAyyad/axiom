...
func TestC6ChaosIsDefaultOffAndFormalOnly(t *testing.T) {
	for _, test := range []struct {
		...
		t.Run(test.name, func(t *testing.T) {
			t.Setenv("AXIOM_C6_CHAOS_ENABLED", test.enabled)
			t.Setenv("AXIOM_C6_CHAOS_MODE", test.mode)
...
func TestQualificationEnvironmentDropsCredentialsAndDatabaseAccess(t *testing.T) {
	...
	for _, forbidden := range []string{
		"DB_PASSWORD_FILE", "BINANCE", "BYBIT", "AXIOM_C6_RUN_ID",
	} {
