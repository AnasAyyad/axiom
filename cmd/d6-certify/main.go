...
func main() {
	if err := run(context.Background(), os.Stdout); err != nil {
		_, _ = fmt.Fprintln(os.Stderr, "d6-certify:", err)
		os.Exit(1)
	}
}

func run(_ context.Context, output io.Writer) error {
	if os.Getenv("AXIOM_D6_FINAL_CERTIFICATION_ENABLED") != "1" {
		return fmt.Errorf("final certification is default-off")
	}
	var candidate certification.Candidate
	if err := certification.ReadStrictJSON(os.Getenv("AXIOM_D6_CANDIDATE_FILE"), &candidate); err != nil {
		return err
	}
	var trust certification.TrustStore
	if err := certification.ReadStrictJSON(os.Getenv("AXIOM_D6_TRUSTED_REVIEWERS_FILE"), &trust); err != nil {
		return err
	}
	...
	key, err := signingKey(os.Getenv("AXIOM_D6_RELEASE_SIGNING_KEY_FILE"))
	if err != nil {
		return err
	}
	...
	if _, err = certification.WriteVerdictNoReplace(os.Getenv("AXIOM_D6_VERDICT_DIRECTORY"), verdict); err != nil {
		return err
	}
...
func signingKey(path string) (ed25519.PrivateKey, error) {
	...
	if err != nil || len(seed) != ed25519.SeedSize {
		return nil, fmt.Errorf("d6 release signing key invalid")
	}
