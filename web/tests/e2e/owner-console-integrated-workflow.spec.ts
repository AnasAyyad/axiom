...
const configuration = required("AXIOM_OWNER_CONSOLE_E2E_CONFIGURATION_ID");
const dataset = required("AXIOM_OWNER_CONSOLE_E2E_DATASET_ID");
const generation = required("AXIOM_OWNER_CONSOLE_E2E_RESEARCH_GENERATION_ID");
const portfolio = required("AXIOM_OWNER_CONSOLE_E2E_PORTFOLIO_ID");
const evidenceShadow = required("AXIOM_OWNER_CONSOLE_E2E_EVIDENCE_SHADOW_ID");
const email = process.env.AXIOM_OWNER_CONSOLE_E2E_EMAIL ?? "owner@example.test";
const password = required("AXIOM_OWNER_CONSOLE_E2E_PASSWORD");
...
test("real authenticated API drives the complete virtual research workflow", async ({
  ...
  await expect(page.getByText("namespace-owner_console")).toBeVisible();
...
function required(name: string): string {
  ...
  if (!value)
    throw new Error(`${name} is required for integrated owner-console E2E`);
  return value;
}
