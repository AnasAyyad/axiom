...
describe("GettingStartedPage", () => {
  it("guides an owner through the required safe-first checklist", () => {
    ...
    expect(
      screen.getByRole("link", { name: /Run a guided proof demonstration/ }),
    ).toHaveAttribute("href", "/guided-demonstrations");
    expect(
      screen.getByText(/does not predict returns or prove/i),
    ).toBeInTheDocument();
    expect(
      screen.getByText(
        /only after the required strategy-session workflow is installed and armed/i,
      ),
    ).toBeVisible();
  });
});
  ...
  it("takes the owner through every installed walkthrough without creating a run", async () => {
    ...
    fireEvent.click(
      screen.getByRole("button", {
        name: "Continue to Inventory Rebalancing basics",
      }),
    );
    expect(
      await screen.findByRole("heading", { name: "Tour step 2 of 2" }),
    ).toBeVisible();
    expect(screen.getByText("Advisory recommendation evidence")).toBeVisible();
  });

  it("shows virtual fills and accounting evidence for a multi-leg walkthrough", async () => {
    vi.stubGlobal("fetch", vi.fn(guidedDemonstrationFetch));
    const client = new QueryClient({
      defaultOptions: { queries: { retry: false } },
    });
    ...
    await screen.findByText("Trend Following basics");
    fireEvent.click(
      screen.getAllByRole("button", { name: "Run walkthrough" })[0]!,
    );

    expect(
      await screen.findByText("Accepted shared-pipeline event"),
    ).toBeVisible();
    expect(screen.getByText("Rejected shared-pipeline event")).toBeVisible();
    expect(screen.getAllByText("Virtual fills")).toHaveLength(2);
    expect(
      screen.getAllByText("Virtual portfolio and accounting"),
    ).toHaveLength(2);
    expect(
      screen.queryByText("Advisory recommendation evidence"),
...
function guidedDemonstrationFetch(input: RequestInfo | URL) {
  ...
  return Promise.resolve(
    jsonResponse({
      ...
      ...(advisory
        ? { advisory_evidence: '{"recommendation":"read-only"}' }
        : {}),
