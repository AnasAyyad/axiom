...
import {
  ...
  runRiskProjection,
} from "./ownerControlRunProjectionValidation";
...
const run = z
  .object({
    ...
    strategy_id: z.string().min(1),
    strategy_version: z.string().min(1),
    mode: z.enum(["backtest", "replay", "shadow", "sandbox"]),
    environment: z.enum([
      "recorded_data",
      "production_public",
      "binance_spot_testnet",
      "bybit_demo",
      "paired_exchange_sandbox",
    ]),
    exchanges: z
      .array(z.enum(["binance", "bybit"]))
      .min(1)
      .max(2)
      .optional(),
    instrument: z.string().min(3).max(32).optional(),
    state: z.string().min(1),
    order_capable: z.boolean(),
    available_actions: z
      ...
      z
        .object({
          ...
          exchanges: z
            ...
            .min(1)
            .max(2),
          instruments: z.array(z.string().min(1).max(64)),
          coverage_types: z.array(z.string().min(1).max(64)),
...
export const ownerControlResponseSchemas: ReadonlyArray<
  readonly [RegExp, z.ZodType]
> = [
  [ownerControlListPaths, resourcePage],
  [ownerControlDetailPaths, resource],
  ...
  [ownerControlCommandPaths, command],
];
