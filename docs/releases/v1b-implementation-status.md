...
| Active implementation phase | B6 locally verified for every specified non-soak gate; B7 is next |
...
checkpoints; this changes only the intermediate merge boundary, not the B4 or
B5 gates. A phase may be `Locally verified` after every non-soak gate passes;
