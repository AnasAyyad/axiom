...
| AX-V1D-D05-008 exact seven-day runner | `internal/qualification/operationalreadiness`, `cmd/operational-readiness` | non-qualifying preflight check; runner smoke, failure, no-replace, signature, and formal server evidence |
| AX-V1D-D05-009 approved fault schedule and declared load | `deploy/config/operational-readiness-fault-schedule-v1.json`; operational-readiness test manifest | checked-in manifest contract test; boundary check and formal fault evidence |
