...
|---|---|---|
| AX-V1D-D05-001 current fresh high/critical pressure and immutable history | `internal/storage/pressure`, migration `000027`, `d5_storage_pressure.go`; two-minute freshness gates | pressure unit tests; D5 PostgreSQL qualification |
| AX-V1D-D05-002 high rejects heavy jobs | `d5_pressure_gates.go`, job/export/report/shadow creation gates | D5 PostgreSQL pressure lifecycle |
...
| AX-V1D-D05-008 exact seven-day runner | `internal/qualification/operationalreadiness`, `cmd/operational-readiness` | runner smoke, failure, no-replace, signature, and formal server evidence |
| AX-V1D-D05-009 approved fault schedule and declared load | `deploy/config/d5-fault-schedule-v1.json`; D5 test manifest | boundary check and formal fault evidence |
